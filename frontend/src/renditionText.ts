...
export async function resolveRenditionText(
  ...
  const raw = object(await generated.resolveRenditionText({
...
export async function readVerifiedRenditionText(
  ...
  const response = await generated.readRenditionText({
    node_id: source.nodeID, revision: authorizationRevision,
    version_id: source.versionID, blob_hash: source.blobHash, size: source.size,
    ...
    ...(ready.generationID ? { generation_id: ready.generationID } : {}),
  }, { session, headers: { Accept: "text/markdown" }, signal });
  ...
  let artifact: string;
  try {
    artifact = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
  } catch {
    throw new Error("The verified text rendition is not valid UTF-8.");
  }
  const parsed = parseRenditionMarkdown(artifact);
  if (parsed.buildID !== ready.buildID || parsed.source.sha256 !== source.blobHash) {
    throw new Error("The received text disagreed with the selected rendition.");
  }

<script lang="ts">
  ...
  import { maxPageSourceBytes } from "./pages.js";
  import { Button, Chip, SelectDropdown, Spinner } from "@kenn-io/kit-ui";
  import { PageViewerSession, pdfPageDPI, pageFailureMessages, type PageViewerState } from "./pageViewer.js";
  ...
  const sizeError = $derived(source.size <= 0 ? "This document is empty. Download the verified original."
    : source.size > maxPageSourceBytes ? "Page previews support documents up to 64 MiB. Download the verified original." : "");
  $effect(() => {
    const key = identity;
    if (sizeError) return;
    const inputs = untrack(() => ({ session, source, authorizationRevision, onauthfailure }));
    viewer = new PageViewerSession(inputs.session, inputs.source, inputs.authorizationRevision, key,
      next => { viewState = next; }, inputs.onauthfailure);
    zoom = "fit";
    void viewer.load();
...
{#if sizeError}
  <p class="notice" role="status">{sizeError}</p>
{:else if source.mimeType === "image/png" && shown.inventory?.inventory.frames.length === 0}
  <OriginalPreview {session} {source} {authorizationRevision} {onauthfailure} />
{:else}
<section class="page-viewer" aria-label={`Pages of ${source.name}`}>
  ...
  {#if shown.url && shown.frame}
    {#if source.mimeType === "application/pdf" && shown.dpi !== pdfPageDPI && shown.inventory?.runtime_available}
      <Button size="sm" tone="info" onclick={() => void viewer?.render()}>Render page {shown.page} at {pdfPageDPI} DPI</Button>
    {/if}
  ...
  {:else if shown.status === "canceled" || shown.status === "failed"}
    <p class="notice" role="status">{shown.status === "canceled" ? "Render canceled." : `Render failed. ${pageFailureMessages[shown.message ?? ""] ?? ""}`}</p>
    <Button size="sm" onclick={() => void viewer?.load()}>Refresh pages</Button>
  ...
  {:else}
    ...
    {:else}<p class="notice" role="status">{pageFailureMessages.unavailable}</p>{/if}
  {/if}
  {#if (shown.status === "rendering" || shown.status === "error") && shown.job && (shown.job.state === "queued" || shown.job.state === "running")}
    <Button size="sm" onclick={() => void viewer?.cancel()}>Cancel render</Button>
  {/if}
</section>
{/if}

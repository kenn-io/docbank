...
it.each(["none", "partial"])("preserves decoded children with %s processing coverage", (state) => {
  const entry = { ...item(), state, reason: "" };
  const result = validateRelationPage(page([entry]), parent, "outgoing");

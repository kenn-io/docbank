...
export function mailboxJSON<T>(
  ...
  return sessionJSON<T>(`/api/v1/mailbox${path}`, {
    session,
...
export async function uploadMailboxArchive(
  ...
  const saved = new Map(c.chunks.map((chunk) => [chunk.index, chunk]));
  ...
  for (let offset = 0, index = 0; offset < file.size; offset += chunkBytes, index++) {
    ...
    const existing = saved.get(index);
    if (existing) {
      if (existing.sha256 !== digest || existing.size !== data.size)
        throw new Error(`Saved chunk ${index} did not match the selected file.`);
      onprogress("Verified saved source", { processed: offset + data.size, total: file.size });
      continue;
    }

...
describe("snapshot request-epoch state", () => {
  ...
  it("retains the accepted page and paging after a failed rerun", async () => {
    ...
    await rerun;
    expect(states.at(-1)).toMatchObject(accepted);
    expect(states.at(-1)!.error?.message).toBe("Query unavailable");
    expect(states.at(-1)!.page).toBe(accepted.page);

    await session.page("previous");
    expect(states.at(-1)).toMatchObject({ status: "ready", offset: 0, query });
    await session.run(otherQuery, { page_size: 50 });
    expect(states.at(-1)).toMatchObject({ status: "ready", offset: 0, query: otherQuery });
    expect(states.at(-1)!.firstPage).not.toBe(accepted.firstPage);
  });

...
export type EmailPDFReceipt = generated.EmailPDFReceiptV1;
...
export async function retainedEmailPDFs(session: string, versionID: string, signal: AbortSignal): Promise<EmailPDFReceipt[]> {
  const values = await generated.listEmailPDFs(versionID, { session, signal });
...
export async function renderEmailPDF(session: string, versionID: string, paper: string, signal: AbortSignal): Promise<EmailPDFReceipt> {
  const job = await generated.renderEmailPDF({ version_id: versionID, generation_id: "", paper }, { session, signal });
  ...
  while (job.state !== "completed") {
    ...
    const next = await generated.getEmailPDFJob(job.job_id, { session, signal });
    job.state = next.state;
  }
  const receipt = await generated.getEmailPDF(versionID, job.profile_fingerprint, { session, signal });

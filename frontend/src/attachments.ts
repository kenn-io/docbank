...
export function validateRelationPage(raw: unknown, selected: AttachmentIdentity, direction: RelationDirection,
  ...
  const items = value.items.map((raw) => {
    ...
    if (!["decoded", "pending", "indexed", "partial", "none", "failed", "unsupported", "encrypted", "unavailable"].includes(state) ||
...
async function readBoundedJSON(session: string, route: string, signal: AbortSignal): Promise<unknown> {
  const response = await sessionResponse<Response>(route, { session, signal, method: "GET" });
...
export async function readRelationPage(session: string, selected: AttachmentIdentity, direction: RelationDirection,
  ...
  const params: generated.ListEmailDocumentRelationsParams = {
    [direction === "outgoing" ? "parent_version_id" : "child_version_id"]: selected.version_id, limit: relationPageSize,
    ...(cursor ? { after_operation_id: cursor.operationID, after_order: cursor.order } : {}),
  };
  return validateRelationPage(await readBoundedJSON(session, generated.getListEmailDocumentRelationsUrl(params), signal), selected, direction, cursor);
}
export async function readPublication(session: string, sample: AttachmentRelation, signal: AbortSignal): Promise<PublicationReceipt> {
  return validatePublication(await readBoundedJSON(session, generated.getGetEmailDocumentPublicationUrl(sample.operation_id), signal), sample);
}
export async function readRelatedSource(session: string, target: AttachmentIdentity, signal: AbortSignal): Promise<RelatedSelectedSource> {
  identity(target);
  const version = object(await readBoundedJSON(session, generated.getGetContentVersionUrl(target.version_id), signal));
  ...
  const node = object(await readBoundedJSON(session, generated.getGetNodeUrl(target.node_id), signal));

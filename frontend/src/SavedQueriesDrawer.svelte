<script lang="ts">
  ...
  import { createSavedQuery, deleteSavedQuery, listSavedQueries, updateSavedQuery, type Definition, type SavedQuery, type SavedQueryKind } from "./savedQueries.js";
  ...
  async function save(asNew: boolean): Promise<void> {
    if (pending || (!asNew && !selected)) return;
    failure = "";
    notice = "";
    pending = true;
    try {
      ...
      if (!alive) return;
      edit(saved);
      if (saved.kind === "query") onload(saved.payload);
  ...
  <div class="workspace">
    ...
    {#if deleting}
      <section aria-label="Delete saved definition" class="editor">
        ...
        <p>This permanently removes only this saved definition. Documents and other saved definitions are unchanged. A loaded query draft remains in the tab until discarded.</p>
        <div class="actions">
    ...
    {:else}
      ...
      <section class="editor" aria-label="Definition editor">
        ...
        {/if}
        <div class="actions">
          <Button tone="info" surface="solid" disabled={pending || !name} onclick={() => void save(true)}>Save as new</Button>
          {#if selected}<Button disabled={pending || !name} onclick={() => void save(false)}>Save changes</Button>{/if}

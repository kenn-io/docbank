...
export class SnapshotSession {
  ...
  private accepted?: Readonly<SnapshotState>;
  private epoch = 0;
  ...
  private begin(
    ...
    this.emit(this.accepted
      ? { ...this.accepted, status: "loading", error: undefined }
      : { status: "loading", query: acceptedQuery, options: acceptedOptions, offset: 0 });
  ...
  private emit(state: SnapshotState): void {
    ...
    if (state.page && state.status !== "loading") this.accepted = this.state;
    this.publish(this.state);
  }
}

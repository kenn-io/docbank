<script lang="ts">
  ...
  import { Button, Card, Chip, DetailDrawer, IconButton, SearchInput, Spinner, type ChipTone } from "@kenn-io/kit-ui";
  import {
    APIError,
    documentCoverage,
    documentSearch,
  ...
  async function execute(): Promise<void> {
    ...
    error = "";
    try {
      const reportProgress = (event: ProcessingJobEvent): void => {
        ...
        if (event.job) {
          run = {
            ...
            status: {
              ...
              state: "running",
              phase: "queued",
              embedding_job_ids: event.job.embedding_job_ids,
              completed_bindings: 0,
            },
          };
        } else if (event.status && run) {
          run = { job: run.job, status: event.status };
        }
      };
      ...
      if (processingController !== active) return;
      run = completed;
      coverage = await documentCoverage(
        session,
        profileName,
        plan.vault_uid,
  ...
  function processingStatusTone(state: ProcessingState): ChipTone {
    ...
    {:else if plan}
      ...
      {#if run}
        <section aria-live="polite">
          <div class="section-heading"><div><span>DURABLE JOB</span><strong>{run.status.state}</strong></div><Chip size="xs" tone={processingStatusTone(run.status.state)}>{run.status.phase}</Chip></div>

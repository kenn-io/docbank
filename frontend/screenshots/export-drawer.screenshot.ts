...
test("downloads the exact 1001-member frozen source through the real export worker", async ({ page }) => {
  ...
  try {
    ...
    const api = async <T>(route: string, body?: unknown): Promise<T> => {
      const response = await fetch(`http://127.0.0.1:${url.port}${route}`, { method: body === undefined ? "GET" : "POST", signal: AbortSignal.timeout(60_000), headers: { Host: url.host, "X-Docbank-Web-Session": session, "Content-Type": "application/json" }, ...(body === undefined ? {} : { body: JSON.stringify(body) }) });
    ...
    await drawer.getByLabel("Downloaded ZIP filename", { exact: true }).fill("Renamed frozen review.zip");
    await expect(drawer.getByRole("button", { name: "Start reviewed export", exact: true })).toBeEnabled();
    await expect(drawer.getByTestId("export-plan-fingerprint")).toHaveText(fingerprint);
    expect(chunks).toEqual([1000, 1]);
    ...
    expect(download.suggestedFilename()).toBe("Renamed frozen review.zip");
...
`, archive], { maxBuffer: 4 * 1024 * 1024 })).stdout) as { total: number; member_hash: string; fingerprint: string; entries: number; role_bytes: number; members: [number, string, string, number][] };
    ...
    await page.getByRole("checkbox", { name: `Select ${frozen[0]!.path}`, exact: true }).check();

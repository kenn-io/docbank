...
// RequestEmailDocumentProcessing runs one exact receipt occurrence through its
// configured provider under explicit consent, waiting for completion.
func (v *Vault) RequestEmailDocumentProcessing(
	...
	return v.processing.RequestEmailDocumentProcessing(ctx, request)
}
...
func (v *Vault) PublishEmailDocuments(
...
// EmailDocumentPublication reads the immutable receipt for one operation.
func (v *Vault) EmailDocumentPublication(
...
// EmailDocumentRelations reads a bounded page of exact parent or child relationships.
func (v *Vault) EmailDocumentRelations(
	ctx context.Context, query document.EmailDocumentRelationQuery,
) (document.EmailDocumentRelationPage, error) {
	if err := v.begin(); err != nil {

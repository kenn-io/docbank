...
func (runeTokenizer) Identity() document.TokenizerIdentity {
	return document.TokenizerIdentity{Name: "private-acceptance-runes", Revision: "v1"}
}
...
func run() error {
	...
	vault, err := docbank.New(context.Background(), docbank.Config{Root: root, Processing: docbank.ProcessingOptions{
		Profiles: map[string]docbank.ProcessingProfileConfig{profileName: {
			...
			RenditionDisclosure: docbank.ProcessingRuntimeDisclosure{Endpoint: config.doclingURL},
			EmbeddingDisclosures: map[string]docbank.ProcessingRuntimeDisclosure{bindingName: {Endpoint: config.embeddingURL}},
...
func newEmbeddingProvider(config settings, transport *http.Transport, recorder *destinationRecorder) (document.EmbeddingProvider, document.EmbeddingDescriptor, error) {
	...
	descriptor, err := document.NewEmbeddingDescriptor(document.EmbeddingDescriptor{
		ID: openaicompat.ProviderID, ContractVersion: document.EmbeddingProviderContractVersion,
		...
		Metric: document.VectorMetricCosine, Normalization: document.VectorNormalizationNone, ScalarEncoding: openaicompat.ScalarEncodingFloat32,
		DocumentFormatter: openaicompat.DocumentFormatterV1, QueryFormatter: openaicompat.QueryFormatterV1,
	...
	profile := openaicompat.Profile{Origin: config.embeddingURL, Descriptor: descriptor, ModelInput: contract, SecretBinding: "embedding",
	...
	policy, err := openaicompat.PolicyFingerprint(profile)
	if err != nil {
		return nil, document.EmbeddingDescriptor{}, err
	}
	...
	provider, err := openaicompat.New(profile, secretResolver{"embedding": config.embeddingKey}, &http.Client{Transport: trackingTransport{
...
func processingProfile(rendition document.RenditionDescriptor, embedding document.EmbeddingDescriptor) (document.ProcessingProfileV1, error) {
	profile := document.ProcessingProfileV1{
		...
		Rendition: &document.RenditionBindingV1{AdapterContract: "docling.serve/v1", AuthorizationFingerprint: digest("docling-authorization"),
			...
			RequestedArtifacts: []document.EvidenceArtifactRole{document.EvidenceArtifactMarkdown, document.EvidenceArtifactStructured}, TrustBoundary: string(rendition.TrustBoundary), UploadOptionsFingerprint: digest("docling-upload")},
		Embeddings: []document.EmbeddingBindingV1{{Activation: document.EmbeddingRequired, AuthorizationFingerprint: digest("embedding-authorization"),
			...
			InputKind: document.EmbeddingInputRenditionChunk, MaxInputTokens: 512, MaxBatchItems: 16, MaxInputBytes: 1 << 20, MaxResponseBytes: 1 << 20,
			...
			Chunk: &document.EmbeddingChunkPolicyV1{ContextFingerprint: digest("chunk-context"), Formatter: "rendition-chunk/v1", MaxTokens: 512, OverlapTokens: 32,
				Tokenizer: "private-acceptance-runes", TokenizerRevision: "v1", TruncationPolicy: document.TruncationPolicyReject}}},
		EvidenceLexical: document.EvidenceLexicalPolicyV1{CompletenessFingerprint: digest("completeness"), LexicalSegmenterFingerprint: digest("segments"),
			MaxDocumentChars: 100000, MaxSegmentRunes: 1000, MaxUnitRunes: 100000, NormalizedEvidenceContract: document.NormalizedEvidenceContractV1,

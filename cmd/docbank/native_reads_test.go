...
func TestNativeReadCLISubprocessCatalogCoverageAndBounds(t *testing.T) {
	...
	scopedCapabilitiesJSON, stderr, err := nativeReadSubprocessWithSession(t, sessionFile,
		"agent", "capabilities", "--json")
	require.NoError(t, err, stderr)
	var scopedCapabilities api.AgentCapabilities
	require.NoError(t, json.Unmarshal([]byte(scopedCapabilitiesJSON), &scopedCapabilities))
	for _, operation := range scopedCapabilities.Server.Operations {
		assert.NotEqual(t, "export_job_start", operation.ID)
	}
	for _, route := range scopedCapabilities.Server.Routes {
	...
	capabilityJSON, stderr, err := nativeReadSubprocess(t, "agent", "capabilities", "--json")
	require.NoError(t, err, stderr)
	var agentCapabilities api.AgentCapabilities
	require.NoError(t, json.Unmarshal([]byte(capabilityJSON), &agentCapabilities))
	assert.Equal(t, "docbank-agent/1", agentCapabilities.Contract)
	assert.Equal(t, info.VaultID, agentCapabilities.Server.VaultID)
	assert.NotEmpty(t, agentCapabilities.Server.Operations)

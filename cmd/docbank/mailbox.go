...
func uploadMailboxChunks(ctx context.Context, c *daemonconn.Connection, source io.ReaderAt, container store.MailboxContainer, progress io.Writer) error {
	saved := make(map[int]store.MailboxChunk, len(container.Chunks))
	for _, chunk := range container.Chunks {
		saved[chunk.Index] = chunk
	}
	for index, offset := 0, int64(0); offset < container.Size && container.State != "sealed"; index, offset = index+1, offset+store.MailboxChunkBytes {
		size := min(store.MailboxChunkBytes, container.Size-offset)
		chunkHash, err := hashMailboxSource(ctx, io.NewSectionReader(source, offset, size), size)
		if err != nil {
			return err
		}
		if chunk, ok := saved[index]; ok {
			if chunk.SHA256 != chunkHash || chunk.Size != size {
				return fmt.Errorf("saved chunk %d differs from source: %w", index, store.ErrMailboxConflict)
			}
			_, _ = fmt.Fprintf(progress, "Verified %d of %d saved bytes\n", offset+size, container.Size)
			continue
		}
		if err = c.UploadMailboxChunk(ctx, container.ID, index, chunkHash, size, io.NewSectionReader(source, offset, size)); err != nil {
			return err
		}
		_, _ = fmt.Fprintf(progress, "Uploaded %d of %d bytes\n", offset+size, container.Size)
	}
	return nil
}

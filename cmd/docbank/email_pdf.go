...
func init() {
	...
	command.RunE = func(cmd *cobra.Command, args []string) (retErr error) {
		...
		job, err := c.RequestEmailPDF(cmd.Context(), document.EmailPDFRequest{VersionID: args[0], Paper: paper})
		if err != nil {
			return err
		}
		ticker := time.NewTicker(250 * time.Millisecond)
		defer ticker.Stop()
		for store.RenditionJobState(job.State) != store.RenditionJobCompleted {
			switch store.RenditionJobState(job.State) {
			case store.RenditionJobFailed, store.RenditionJobOperatorRequired:
				return fmt.Errorf("email PDF job %s ended %s; inspect processing jobs", job.JobID, job.State)
			case store.RenditionJobQueued, store.RenditionJobRunning, store.RenditionJobRetryWait, store.RenditionJobCompleted:
			}
			select {

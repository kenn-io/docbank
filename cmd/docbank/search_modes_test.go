...
func TestSearchAutoDoesNotSelectEmbeddingBinding(t *testing.T) {
	for _, bindings := range [][]string{nil, {"general"}, {"general", "multilingual"}} {
		t.Run(strings.Join(bindings, "+"), func(t *testing.T) {
			binding, err := selectDocumentSearchBinding("auto", "", bindings)
			require.NoError(t, err)
			assert.Empty(t, binding)
			_, err = selectDocumentSearchBinding("auto", "general", bindings)
			require.ErrorContains(t, err, "--binding")
		})
	}
}

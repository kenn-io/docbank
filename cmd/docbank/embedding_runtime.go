...
func configureEmbeddingRuntimeBundle(cfg config.Config, blobs embeddingRuntimeBlobStore,
	...
	for name, configured := range cfg.EmbeddingProfiles {
		...
		if value, ok := os.LookupEnv(variable); !ok || value == "" {
			return embeddingRuntimeBundle{}, fmt.Errorf("embedding credential %q environment variable is missing", configured.CredentialBinding)
		}
...
func executableProcessingProfiles(cfg config.Config,
	...
	for _, name := range names {
		portable, err := cfg.PortableProcessingProfile(name)
		if err != nil {
			return nil, err
		}
		...
		for _, binding := range portable.Embeddings {
			...
			if binding.InputKind == document.EmbeddingInputRenditionChunk {
				tokenizer := configuredEmbeddingTokenizer(binding.Chunk.Tokenizer)
				if tokenizer == nil {
					executable = false
					break
				}

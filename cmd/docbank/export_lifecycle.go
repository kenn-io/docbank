...
func newExportLifecycleCommands() []*cobra.Command {
	...
	showPlan := &cobra.Command{Use: "show-plan <plan-id>", Short: "Read one frozen export plan header", Args: cobra.ExactArgs(1)}
	showPlan.RunE = func(cmd *cobra.Command, args []string) error {
		if err := requireExportID(args[0]); err != nil {
			return err
		}
		connection, err := exportConnection(cmd.Context())
		...
		result, err := connection.API().GetExportPlan(cmd.Context(), &apiclient.GetExportPlanRequestOptions{
			PathParams: &apiclient.GetExportPlanPath{ID: args[0]},
		})
		if err != nil {
			return fmt.Errorf("read export plan: %w", err)
		}
		if err := json.MarshalWrite(cmd.OutOrStdout(), result); err != nil {
			return fmt.Errorf("write export plan: %w", err)
		}
	...
	problems := &cobra.Command{Use: "problems <plan-id>", Short: "Read one page of unavailable export outputs", Args: cobra.ExactArgs(1)}
	var after int64
	problems.Flags().Int64Var(&after, "after", 0, "Start after this output problem offset")
	problems.RunE = func(cmd *cobra.Command, args []string) error {
		if err := requireExportID(args[0]); err != nil {
			return err
		}
		if after < 0 || after > bundle.MaxOutputProblems {
			return usageError(errors.New("export problems --after is outside the supported range"))
		}
		connection, err := exportConnection(cmd.Context())
		if err != nil {
			return err
		}
		result, err := connection.API().GetExportOutputProblems(cmd.Context(), &apiclient.GetExportOutputProblemsRequestOptions{
			PathParams: &apiclient.GetExportOutputProblemsPath{ID: args[0]},
			Query:      &apiclient.GetExportOutputProblemsQuery{After: &after},
		})
		if err != nil {
			return fmt.Errorf("read export output problems: %w", err)
		}
		if err := json.MarshalWrite(cmd.OutOrStdout(), result); err != nil {
			return fmt.Errorf("write export output problems: %w", err)
		}
		return nil
	}
	...
	return []*cobra.Command{source, plan, preview, showPlan, problems, start, status, cancel}
}

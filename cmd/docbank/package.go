...
var packageMembersCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, item := range page.Items {
			if _, err = fmt.Fprintf(cmd.OutOrStdout(), "%d\t%s\t%s\t%s\n", item.Ordinal, item.RowID, item.DocumentKind, strconv.QuoteToASCII(item.DisplayName)); err != nil {
...
var packageRecordCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		_, err = fmt.Fprintf(cmd.OutOrStdout(), "%s row %d · %s\n%s\n", strconv.QuoteToASCII(result.LoadFile), result.RowOrdinal, result.RowID, strconv.QuoteToASCII(string(result.RawJSON)))
		if err != nil {

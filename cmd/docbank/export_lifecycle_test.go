...
func TestExportLifecycleCLIRejectsUnknownAndOversizedRequestBeforeDaemon(t *testing.T) {
	...
	_, err = runCLI(t, "export", "show-plan", "not-a-plan")
	require.ErrorContains(t, err, "canonical UUIDv4")
	_, err = runCLI(t, "export", "problems", "not-a-plan")
	require.ErrorContains(t, err, "canonical UUIDv4")
	_, err = runCLI(t, "export", "problems", "11111111-1111-4111-8111-111111111111", "--after", "3000001")
	require.ErrorContains(t, err, "outside the supported range")
	_, err = runCLI(t, "export", "problems", "11111111-1111-4111-8111-111111111111", "--after", "-1")
	require.ErrorContains(t, err, "outside the supported range")
}
...
func TestExportLifecycleCLIProducesFrozenPDFArchive(t *testing.T) {
	...
	require.NoError(t, err, output)
	var retainedPlan bundle.Plan
	require.NoError(t, json.Unmarshal([]byte(output), &retainedPlan))
	require.Equal(t, plan.ID, retainedPlan.ID)
	require.Equal(t, plan.Fingerprint, retainedPlan.Fingerprint)
	require.Equal(t, source.MemberHash, retainedPlan.Source.MemberHash)
	output, err = runCLI(t, "export", "problems", plan.ID, "--after", "0")
	require.NoError(t, err, output)
	var problems bundle.OutputProblems
	require.NoError(t, json.Unmarshal([]byte(output), &problems))
	require.Equal(t, plan.ID, problems.PlanID)
	require.Equal(t, plan.Fingerprint, problems.Fingerprint)
	require.LessOrEqual(t, len(problems.Items), bundle.InspectionPageSize)
	...
	mcpPlan := callMCP("get_export_plan", map[string]any{"plan_id": plan.ID})
	require.Equal(t, plan.Fingerprint, mcpPlan["fingerprint"])
	require.Equal(t, source.MemberHash, mcpPlan["member_hash"])
	mcpProblems := callMCP("list_export_output_problems", map[string]any{"plan_id": plan.ID, "after": 0})
	require.Equal(t, plan.Fingerprint, mcpProblems["fingerprint"])
	require.EqualValues(t, problems.Total, mcpProblems["total"])

...
func exerciseQueryFunctions(t *testing.T, driver docsqlite.Driver) {
	...
	var matches bool
	require.NoError(t, second.QueryRowContext(t.Context(), `SELECT
		docbank_query_timestamp_v1(?) < docbank_query_timestamp_v1(?)`,
		"2026-01-02T03:04:05Z", "2026-01-02T04:04:05.000000001+01:00").Scan(&matches))
	require.True(t, matches, "SQL comparison must retain nanosecond precision across zone offsets")
	err = second.QueryRowContext(t.Context(), `SELECT docbank_query_timestamp_v1(?)`, "invalid").Scan(&family)
	require.Error(t, err, "malformed stored timestamps must not silently match")
}

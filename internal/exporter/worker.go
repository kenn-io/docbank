...
func (w *Worker) Run(ctx context.Context) error {
	if err := w.retryCatalogMutation(ctx, func() error { return w.catalog.RevokeAbandonedExportSessions(ctx) }); err != nil {
		return err
	}
	...
	for _, file := range files {
		...
		if id, ok := strings.CutSuffix(file.Name(), ".zip"); ok {
			if parsed, e := uuid.Parse(id); e == nil && parsed.String() == id {
				var retained bool
				e := w.retryCatalog(ctx, func() error {
					var err error
					retained, err = w.catalog.ExportArchiveRetained(ctx, id)
					return err
				})
				if e != nil {
					return e
...
func (w *Worker) retryCatalogMutation(ctx context.Context, operation func() error) error {
	return w.retryCatalog(ctx, func() error { return w.gate.MutateContext(ctx, operation) })
}
...
func (w *Worker) RunOne(ctx context.Context) (bool, error) {
	...
	err := w.retryCatalogMutation(ctx, func() error { var err error; claim, err = w.catalog.ClaimExportJob(ctx); return err })
	...
	go func() {
		...
		for {
			...
			case <-work.Done():
				return
			case <-ticker.C:
				if err := w.catalog.CheckExportClaim(work, claim); err != nil && !w.catalog.RenditionJobErrorRetryable(err) {
					stopWork(err)
					return
				}
			}
	...
	if err == nil {
		...
		receipt, err = bundle.Write(work, file, plan, func(visit func(bundle.Document) error) error {
			...
			e := w.retryCatalogMutation(work, func() error { return w.catalog.AdvanceExportJob(work, claim, roles, bytes) })
			if e == nil {
	...
	if err == nil {
		err = w.gate.MutateContext(work, func() error {
			if err := w.retryCatalog(work, func() error { return w.catalog.CheckExportClaim(work, claim) }); err != nil {
				return err
			}
			...
			if err := w.retryCatalog(work, func() error {
				return w.catalog.FinishExportJob(work, claim, &receipt, claim.Job.ID+".zip", "")
			}); err != nil {
	...
	if err != nil {
		if errors.Is(context.Cause(work), errOwnerRevoked) {
			return true, w.retryCatalogMutation(ctx, func() error { return w.catalog.CancelExportJob(ctx, claim.Owner, claim.Job.ID) })
		}
		finish := w.retryCatalogMutation(ctx, func() error { return w.catalog.FinishExportJob(ctx, claim, nil, "", "archive_failed") })
...
func (w *Worker) Cleanup(ctx context.Context) error {
	...
	return w.gate.MutateContext(ctx, func() error {
		var ids []string
		err := w.retryCatalog(ctx, func() error {
			var err error
		...
		for _, id := range ids {
			...
			if err = w.retryCatalog(ctx, func() error { return w.catalog.DeleteExpiredExportJob(ctx, id) }); err != nil {
				return err
			}
		...
		return w.retryCatalog(ctx, func() error { return w.catalog.CleanupExportAuthority(ctx) })
	})
}

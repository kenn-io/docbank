...
func (worker *IndexWorker) rebuild(ctx context.Context, vectorSpaceID string) (store.VectorIndexGenerationRecord, error) {
	...
	encoded := built.Bytes()
	record := store.VectorIndexGenerationRecord{ID: store.VectorIndexGenerationID(source.ManifestChecksum, encoded),
...
func (worker *IndexWorker) loadVectorSets(ctx context.Context, source store.VectorIndexSource,
	...
	sets := make([]document.VectorSetV1, 0, len(manifest.SetIDs))
	rowCount := 0
	...
	for _, setID := range manifest.SetIDs {
		...
		remainingRows := worker.buildOptions.MaxRows - rowCount
		set, checksum, err := document.DecodeVectorSetV1(payload, document.VectorBounds{
		...
		canonical, encodedChecksum, err := document.EncodeVectorSetV1(set)
		if err != nil || checksum != setID || encodedChecksum != setID || !bytes.Equal(canonical, payload) ||
	...
	if len(coverage.Missing) != 0 {
		return nil, nil, coverage, nil
	}
	built, err := vectorindex.BuildGeneration(manifest, sets, worker.buildOptions)
...
func validateStoredVectorIndex(record store.VectorIndexGenerationRecord, source store.VectorIndexSource,
	...
	if store.VectorIndexGenerationID(record.SourceManifestChecksum, record.Bytes) != record.ID ||
		record.VectorSpaceID != source.VectorSpaceID ||
...
func (worker *IndexWorker) runPass(ctx context.Context) (bool, error) {
	...
	if _, err := worker.catalog.ReclaimVectorIndexGenerations(ctx, worker.clock().UTC()); err != nil {
		permanent = errors.Join(permanent, fmt.Errorf("reclaiming vector index generations: %w", err))
	}

...
func processingCoverageLine(style styles, item api.CoverageClass, width int) string {
	...
	return style.muted.Render(pad(fit(fmt.Sprintf(" %s · %s · %s · %d/%d complete · unavailable: %d · stale: %d · ineligible: %d", name, requirement, item.State, item.Complete, item.Total, item.Unavailable, item.Stale, item.Ineligible), width), width))
}

...
func (m Model) loadPackages(requestID uint64) tea.Cmd {
	ctx, backend := m.ctx, m.backend
	return func() tea.Msg {
		page, err := backend.Packages(ctx, "", "", 250)
		return packagesLoadedMsg{requestID: requestID, page: page, err: err}
	}
}

func (m Model) loadPackageMembers(packageID string, requestID uint64) tea.Cmd {
	ctx, backend := m.ctx, m.backend
	return func() tea.Msg {
		page, err := backend.PackageMembers(ctx, packageID, 0, 250)
		return packageMembersLoadedMsg{requestID: requestID, packageID: packageID, page: page, err: err}
	}
}

func (m Model) loadPackageLabel(label, packageID string, requestID uint64) tea.Cmd {
	ctx, backend := m.ctx, m.backend
	return func() tea.Msg {
		page, err := backend.LookupLabel(ctx, label, packageID, 100)
...
func (m Model) updatePackagesKeys(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	case "r":
		...
		m.packagesRequestID++
		if m.packageMembersOpen {
			return m, tea.Batch(m.startSpinner(), m.loadPackageMembers(m.packageMembersPackage.PackageID, m.packagesRequestID))
		}
	...
	case keyEnter:
		...
		m.packagesErr = nil
		m.packagesRequestID++
		return m, tea.Batch(m.startSpinner(), m.loadPackageMembers(selected.PackageID, m.packagesRequestID))
	case "l":
	...
	case keyPageDown:
		m.movePackageCursor(m.visiblePackageRows())
	case keyHome, "g":
		m.movePackageCursor(-len(m.packages) - len(m.packageMembers))
	case keyEnd, "G":
		m.movePackageCursor(len(m.packages) + len(m.packageMembers))
	}
	return m, nil
...
func (m Model) renderPackagesLocation() string {
	...
	if m.packagesTruncated {
		right = fmt.Sprintf("%d+ package(s)", len(m.packages))
	}
	if m.packageMembersOpen {
		left = " Load-file package · " + m.packageMembersPackage.PackageName
		right = fmt.Sprintf("%d document(s)", len(m.packageMembers))
		if m.packageMembersTruncated {
			right = fmt.Sprintf("%d+ document(s)", len(m.packageMembers))
		}
	}
...
func (m Model) renderPackages(height int) string {
	...
	if m.packagesErr != nil && visible > 0 {
		...
		for _, item := range m.packageLabelMatches[:min(len(m.packageLabelMatches), visible)] {
			page := "unknown"
			if item.PageState == "verified" && item.PageNumber > 0 {
				page = strconv.Itoa(item.PageNumber)
			}
			line := fmt.Sprintf("  %-40s %-12s %-11s %s", item.Label, item.LabelSet, item.Provenance, page)
			lines = append(lines, pad(fit(line, m.width), m.width))
		}
		...
		for index := m.packageMembersOffset; index < end; index++ {
			...
			line := fmt.Sprintf("  %-3d %-45s %-11s %d", member.Ordinal, member.DisplayName, member.DocumentKind, pages)
		...
		for index := m.packagesOffset; index < end; index++ {
			...
			left := fmt.Sprintf("  %s  %s", item.PackageName, item.Direction)
...
func (m Model) renderPackagesFooter() string {
	hints := []hint{{text: "↑/↓ move", priority: 100}, {text: "r refresh", priority: 75},
	...
	position := ""
	if m.packageMembersOpen && len(m.packageMembers) > 0 {

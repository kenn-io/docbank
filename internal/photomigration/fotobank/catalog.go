...
func readCatalog(ctx context.Context, db *sql.DB, catalog string, version int64, layouts []tableLayout) (photomigration.Report, []photomigration.MapEntry, error) {
	...
	queries := map[string]*int64{
		...
		"album_media": &report.Counts.AlbumMemberships,
		"scopes":      &report.Counts.Shares, "checkouts": &report.Counts.Checkouts,
		"checkout_entries": &report.Counts.CheckoutEntries,
...
func validateCatalog(ctx context.Context, db *sql.DB) (int64, []tableLayout, error) {
	var version int64
	if err := db.QueryRowContext(ctx, `SELECT COALESCE(MAX(version),0) FROM schema_migrations`).Scan(&version); err != nil {
		return 0, nil, fmt.Errorf("%w: reading Fotobank schema version: %w", ErrSchemaMismatch, err)
	}
	if version != 1 {
		return version, nil, fmt.Errorf("%w: schema version %d, expected 1", ErrSchemaMismatch, version)
	}
	var dirty bool
	if err := db.QueryRowContext(ctx, `SELECT dirty FROM schema_migrations ORDER BY version DESC LIMIT 1`).Scan(&dirty); err != nil {
		return version, nil, fmt.Errorf("%w: reading Fotobank migration marker: %w", ErrSchemaMismatch, err)
	}
	if dirty {
		return version, nil, fmt.Errorf("%w: schema_migrations is dirty", ErrSchemaMismatch)
	}
	layouts, err := catalogLayout(ctx, db)
	if err != nil {
		return version, nil, fmt.Errorf("%w: %w", ErrSchemaMismatch, err)
	}
...
func validateEmbeddedDocbank(ctx context.Context, driver docsqlite.Driver, path string) (int64, error) {
	db, err := driver.Open(path, docsqlite.OpenOptions{Access: docsqlite.ReadOnlyImmutable, TransactionMode: docsqlite.Deferred})
	if err != nil {
		return 0, fmt.Errorf("opening embedded Docbank database: %w", err)
	}
	db.SetMaxOpenConns(1)
	defer func() { _ = db.Close() }()
	var version int64
	if err := db.QueryRowContext(ctx, `SELECT schema_version FROM vault_metadata WHERE singleton=1`).Scan(&version); err != nil {
		return 0, fmt.Errorf("%w: reading schema version: %w", ErrEmbeddedSchemaMismatch, err)
	}
	if version != 16 {
		return 0, fmt.Errorf("%w: version %d, expected 16", ErrEmbeddedSchemaMismatch, version)
	}
	...
	for table, expected := range expectedColumns {
		...
		if err != nil {
			return 0, fmt.Errorf("%w: %s: %w", ErrEmbeddedSchemaMismatch, table, err)
		}
		if !sameStrings(columnNames(got), expected) {
			return 0, fmt.Errorf("%w: %s columns %v, expected %v", ErrEmbeddedSchemaMismatch, table, columnNames(got), expected)
		}
		if gotMetadata := encodeColumns(got); gotMetadata != expectedMetadata[table] {
			return 0, fmt.Errorf("%w: %s column metadata %q, expected %q", ErrEmbeddedSchemaMismatch, table, gotMetadata, expectedMetadata[table])
		}
	}
	var uniqueBlobBytes int64
	if err := db.QueryRowContext(ctx, `SELECT COALESCE(SUM(size),0) FROM blobs`).Scan(&uniqueBlobBytes); err != nil {
		return 0, fmt.Errorf("reading embedded Docbank blob bytes: %w", err)
	}
	return uniqueBlobBytes, nil
}

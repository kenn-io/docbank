...
// the exact primary content version. A text-only record uses its supplied text
// as both source and text; a native and its supplied text retain separate
// immutable versions. Publication decodes the text without extracting the source.

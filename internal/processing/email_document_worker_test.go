...
func TestEmailDocumentsRemoteConsentBindingAndRevocation(t *testing.T) {
	...
	worker, err := NewRenditionWorker(RenditionWorkerConfig{Catalog: f.catalog, Blobs: f.blobs, Runtime: emailRealRuntime{provider, f.blobs}, Gate: newWorkerTestGate(), Owner: "email-consent-worker", LeaseDuration: time.Minute, IdleDelay: time.Millisecond})
	require.NoError(t, err)
...
func TestEmailDocumentsRealCSVProviderSearchAndConsent(t *testing.T) {
	...
	request := emailProcessingRequest(t, receipt, provider.Descriptor())
	request.Profile.Rendition.TrustBoundary = string(provider.Descriptor().TrustBoundary)
	request.Profile.RetentionDisclosure.TrustBoundary = string(provider.Descriptor().TrustBoundary)
	service, err := NewService(ServiceConfig{Catalog: f.catalog, Blobs: f.blobs,
		Gate: newWorkerTestGate(), SpoolDirectory: f.spool,
		Profiles: map[string]ProfileConfig{"email": {Profile: request.Profile, RenditionProvider: provider}},
	})
	require.NoError(t, err)
	node, version, profile, err := service.resolve(t.Context(), Selector{
		NodeID: receipt.Relations[0].Child.NodeID, ContentVersionID: receipt.Relations[0].Child.VersionID, Profile: "email",
	})
	require.NoError(t, err)
	prepared, err := service.prepareRendition(t.Context(), node, version, profile, false)
	require.NoError(t, err)
	request.ExecutionIdentity, request.CapturedArtifactPolicy = prepared.identity, prepared.capturedPolicy
	_, err = service.EnqueueEmailDocumentProcessing(t.Context(), request)
	require.ErrorIs(t, err, store.ErrProcessingConsentRequired)
	hits, _, err := f.catalog.SearchPage(t.Context(), "attachmentquasar", 10)
	require.NoError(t, err)
	require.Empty(t, hits)
	grantEmailProcessingConsent(t, f.catalog, request)
	for _, field := range []string{"media_family", "media_type", "capability_checksum"} {
		t.Run(field, func(t *testing.T) {
			forged := request
			switch field {
			case "media_family":
				forged.ExecutionIdentity.Upload.MediaFamily = "pdf"
				forged.ExecutionIdentity.Authorization.MediaFamily = "pdf"
			case "media_type":
				forged.ExecutionIdentity.Upload.MediaType = "application/pdf"
				forged.ExecutionIdentity.Authorization.MediaType = "application/pdf"
			case "capability_checksum":
				forged.ExecutionIdentity.Upload.CapabilityRecordChecksum = processingHash("forged-capability")
				forged.ExecutionIdentity.Authorization.CapabilityRecordChecksum = processingHash("forged-capability")
			}
			_, err := service.EnqueueEmailDocumentProcessing(t.Context(), forged)
			require.ErrorIs(t, err, store.ErrInvalidEmailDocumentRequest)
		})
	}
	_, queued, err := f.catalog.ClaimNextRenditionJob(t.Context(), "email-probe", time.Now(), time.Minute)
	require.NoError(t, err)
	require.False(t, queued, "invalid metadata must not leave queued work")
	job, err := service.EnqueueEmailDocumentProcessing(t.Context(), request)
	require.NoError(t, err)
	page, err := f.catalog.EmailDocumentRelations(t.Context(), document.EmailDocumentRelationQuery{ChildVersionID: job.Child.VersionID})
	require.NoError(t, err)
	require.Equal(t, "pending", page.Items[0].State)
	worker, err := NewRenditionWorker(RenditionWorkerConfig{Catalog: f.catalog, Blobs: f.blobs, Runtime: service.renditions, Gate: newWorkerTestGate(), Owner: "email-csv-worker", LeaseDuration: time.Minute, IdleDelay: time.Millisecond})
	require.NoError(t, err)
	worked, err := worker.RunOne(t.Context())
	require.NoError(t, err)
	require.True(t, worked)

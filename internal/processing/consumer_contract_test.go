...
func TestConsumerSourceFence(t *testing.T) {
	...
	assert.Equal(t, []string{
		...
		"lexical", "vector_authority", "vector_embed", "vector_candidates",
		"lexical", "vector_authority", "vector_embed", "vector_candidates",
...
func newConsumerSource(t *testing.T, consumer externalConsumer, revoked retrieval.DocumentIdentity) *consumerSource {
	...
	neighbors, err := generation.Search([]float32{1, 0}, len(documents))
	require.NoError(t, err)
...
func (source *consumerSource) RevalidateSearchCandidates(_ context.Context, candidates []store.SearchCandidateIdentity,
	...
	allowed := make([]store.RevalidatedSearchCandidate, 0, len(candidates))
	for _, candidate := range candidates {
		...
		if _, current := allowedDocuments[document]; current &&
			...
			allowed = append(allowed, store.RevalidatedSearchCandidate{
				NodeID: candidate.NodeID, ContentVersionID: candidate.ContentVersionID,
			})
		}
	}
...
func (source *consumerSource) NormalizeSearchOptions(_ context.Context, scope store.SearchOptions) (store.SearchOptions, error) {
	return scope, source.requireFence(scope)
}

func (source *consumerSource) AuthorizeExpansion(_ context.Context, operation retrieval.ProviderOperation) (retrieval.ProviderEgressLease, error) {
...
func (source *consumerSource) AuthorizeReranking(_ context.Context, operation retrieval.ProviderOperation) (retrieval.ProviderEgressLease, error) {
	source.stages = append(source.stages, "rerank_authorize")
	if err := source.requireFence(operation.Scope); err != nil {
		return nil, err
	}
	if slices.Contains(source.observed, source.revoked) {
		return nil, errors.New("revoked document reached reranking authorization")
	}
	return source, nil
}

func (source *consumerSource) Close() {}
...
func newConsumerSearcher(t *testing.T, source *consumerSource) *retrieval.Searcher {
	...
	searcher, err := retrieval.NewSearcher(retrieval.SearcherConfig{Backend: source,
		...
		Owner:    "synthetic-external-consumer", LeaseDuration: time.Minute,

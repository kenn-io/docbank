...
func (service *Service) Coverage(ctx context.Context, profileName string, fence SourceFence) (Coverage, error) {
	...
	if errors.Is(err, store.ErrNotFound) {
		// A configured profile is not durable catalog authority until its first
		// processing run. Before then, absence means zero coverage rather than an
		// invalid plan.
		for _, binding := range bindings {
			report.Embeddings = append(report.Embeddings, CoverageClass{
				Name: binding.BindingID, Required: binding.Required,
				State: string(store.CoverageUnavailable), Unavailable: len(ids), Total: len(ids),
			})
		}
		report.State = string(store.CoveragePartial)
		return report, nil
	}
	if err != nil {
		return Coverage{}, err

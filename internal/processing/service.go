...
type configuredProfile struct {
	...
	vectorSpaces     map[string]string
	tokenizers       map[string]document.Tokenizer
}
...
func NewService(config ServiceConfig) (*Service, error) {
	...
	for name, supplied := range config.Profiles {
		...
		configured := configuredProfile{portable: profile, provider: supplied.RenditionProvider,
			...
			vectorSpaces:     make(map[string]string, len(fingerprints.VectorSpace)),
		...
		maps.Copy(configured.vectorSpaces, fingerprints.VectorSpace)
...
func (service *Service) Plan(ctx context.Context, selector Selector) (Plan, error) {
	...
	plan.ConsentState, err = service.profileConsentState(ctx, profile)
	if err != nil {
		return Plan{}, err
	}
...
func (service *Service) StartWithProgress(ctx context.Context, request StartRequest,
	...
	embeddingCtx := ctx
	if announced.ID != "" && onEnqueued != nil {
		embeddingCtx = service.lifecycle
	}
...
type providerConsentOperation struct {
	disclosure string
	inputs     []string
	retained   []string
}

func profileConsentOperations(profile configuredProfile) []providerConsentOperation {
	operations := make([]providerConsentOperation, 0, 1+2*len(profile.portable.Embeddings))
	if profile.portable.Rendition != nil {
		operations = append(operations, providerConsentOperation{
			disclosure: profile.record.RenditionDisclosureFingerprint,
			inputs:     []string{string(document.RenditionInputOriginalFile)},
			retained:   retainedRenditionClasses(profile.portable),
		})
	}
	for _, binding := range profile.portable.Embeddings {
		operations = append(operations, providerConsentOperation{disclosure: binding.DisclosureFingerprint,
			inputs: []string{string(binding.InputKind)}, retained: []string{"embedding_vector_set"}})
		if profile.embedders[binding.Name].Descriptor().SupportsTextQuery {
			operations = append(operations, providerConsentOperation{disclosure: binding.DisclosureFingerprint,
				inputs: []string{"query_text"}, retained: []string{}})
		}
	}
	return operations
}

func (service *Service) profileConsentState(ctx context.Context, profile configuredProfile) (string, error) {
	state := "active"
	for _, operation := range profileConsentOperations(profile) {
		_, err := service.catalog.AuthorizeProviderOperation(ctx, store.ProviderOperationAuthorizationRequest{
			...
			DisclosureFingerprint: operation.disclosure, InputClasses: operation.inputs,
			RetainedArtifactClasses: operation.retained})
		switch {
		case err == nil:
...
func (service *Service) grantProfileConsent(ctx context.Context, profile configuredProfile, expiresAt *time.Time) error {
	grant := func(operation providerConsentOperation) error {
		_, err := service.catalog.GrantConsent(ctx, store.ProcessingConsentGrantRequest{
			Principal: service.principal, Scope: service.scope, ProfileFingerprint: profile.record.Fingerprint,
			DisclosureFingerprint: operation.disclosure, InputClasses: operation.inputs,
			RetainedArtifactClasses: operation.retained, ExpiresAt: expiresAt})
		return err
	}
	for _, operation := range profileConsentOperations(profile) {
		if err := grant(operation); err != nil {
			return err
		}
	}
	return nil
...
func (service *Service) runRendition(ctx context.Context, node store.Node, version store.ContentVersion,
	...
	run := renditionRun{jobID: job.ID, waiterID: waiter.ID, attachmentID: waiter.AttachmentID}
	if onEnqueued != nil {
		onEnqueued(run)
	}
	executionCtx := ctx
	if onEnqueued != nil {
		executionCtx = service.lifecycle
	}
	...
	if err != nil {
		if current, statusErr := service.catalog.RenditionJobByID(executionCtx, job.ID); statusErr == nil &&
			current.State == store.RenditionJobCompleted {
			return run, nil
		}
		return run, err
	...
	if !processed {
		if err := service.waitRenditionTerminal(executionCtx, job.ID); err != nil {
			return run, err
		}
	}
	return run, nil
...
func (service *Service) waitRenditionTerminal(ctx context.Context, jobID string) error {
	ticker := time.NewTicker(10 * time.Millisecond)
	defer ticker.Stop()
	for {
		job, err := service.catalog.RenditionJobByID(ctx, jobID)
		if err != nil {
			return err
		}
		switch job.State {
		case store.RenditionJobCompleted, store.RenditionJobFailed, store.RenditionJobOperatorRequired:
			return nil
...
func (service *Service) Coverage(ctx context.Context, profileName string, fence SourceFence) (Coverage, error) {
	...
	coverage, err := service.catalog.ProcessingCoverage(ctx, store.ProcessingCoverageScope{
	...
	states := make([]store.CoverageState, 0, 1+len(bindings))
	if profile.portable.Rendition != nil {
		item := coverage.Renditions
		report.Renditions = CoverageClass{Name: "rendition", Required: true, State: string(item.State),
	...
	for _, item := range append(coverage.Embeddings.Required, coverage.Embeddings.Optional...) {
		report.Embeddings = append(report.Embeddings, CoverageClass{Name: item.Binding.BindingID,
			...
			Unavailable: item.Unavailable, Stale: item.Stale, Ineligible: item.Ineligible,
			Rebuilding: item.Rebuilding, PreviousServing: item.PreviousGenerationServing, Total: item.Total})
		states = append(states, item.State)
	}
	report.State = summarizeProcessingCoverageStates(states)
	return report, nil
}
...
func summarizeProcessingCoverageStates(states []store.CoverageState) string {
	if len(states) == 0 {
		return string(store.CoverageComplete)
	}
	allComplete, allIneligible := true, true
	for _, state := range states {
		if state == store.CoverageRebuilding {
			return string(store.CoverageRebuilding)
		}
		allComplete = allComplete && state == store.CoverageComplete
		allIneligible = allIneligible && state == store.CoverageIneligible
	}
	if allComplete {
		return string(store.CoverageComplete)
	}
	if allIneligible {
		return string(store.CoverageIneligible)
	}
	return string(store.CoveragePartial)
}
...
func (service *Service) prepareSearch(
	...
	if strings.TrimSpace(request.Query) == "" {
		return preparedSearch{}, errors.New("document search query is required")
	}
...
func (service *Service) waitEmbeddingTerminal(ctx context.Context, jobID string) error {
	...
	defer ticker.Stop()
	for {
		job, err := service.catalog.EmbeddingJobByID(ctx, jobID)
		if err != nil {
			return err
		}
...
func sourceFenceCanonicalBytes(fence SourceFence) ([]byte, error) {
	...
	if err != nil || vaultID.Version() != 4 || vaultID.String() != fence.VaultUID {
	...
	for index, id := range ids {
		...
		if parseErr != nil || parsed.Version() != 4 || parsed.String() != id {
			return nil, errors.New("source fence contains an invalid content version ID")
		}
...
func renditionRuntimeDisclosure(supplied RuntimeDisclosure, binding document.RenditionBindingV1,
	...
	roles := []string{"normalized_evidence"}
	if retention.RetainSanitizedMarkdown {
		roles = append(roles, "sanitized_markdown")
	}
	if retention.RetainProviderMarkdown {
		roles = append(roles, "provider_markdown")
	}
	if retention.RetainTypedArtifacts {

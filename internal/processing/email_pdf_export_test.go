...
func TestExportEmailPDFPinsRetainedReceiptAndArchive(t *testing.T) {
	...
	runtime := &EmailPDFRuntime{Catalog: f.catalog, Blobs: f.blobs, Recipe: document.EmailPDFRecipeV1{
		Contract: document.EmailPDFContract, RendererVersion: "synthetic-test", RendererSHA256: strings.Repeat("a", 64), FontsSHA256: strings.Repeat("b", 64), WorkerSHA256: strings.Repeat("c", 64), BubblewrapSHA256: strings.Repeat("d", 64), Paper: "A4",
	}}
	...
	require.NoError(t, err)
	worker, err := NewRenditionWorker(RenditionWorkerConfig{Catalog: f.catalog, Blobs: f.blobs, Runtime: runtime, Gate: newTestOperationGate(), Owner: "synthetic-export", LeaseDuration: time.Minute, IdleDelay: time.Millisecond})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	// Failed admission is atomic, so retry can freeze the now-retained PDF.
...
func TestExportEmailPDFRecipeAcrossDistinctMessages(t *testing.T) {
	...
	var rendered bytes.Buffer
	require.NoError(t, pdf.Output(&rendered))
	recipe := document.EmailPDFRecipeV1{Contract: document.EmailPDFContract, RendererVersion: "synthetic-batch", RendererSHA256: strings.Repeat("a", 64), FontsSHA256: strings.Repeat("b", 64), WorkerSHA256: strings.Repeat("c", 64), BubblewrapSHA256: strings.Repeat("d", 64), Paper: "A4"}
	...
	for _, name := range []string{"first.eml", "second.eml"} {
		...
		job, _, err := f.catalog.EnqueueRenditionJob(t.Context(), request)
		require.NoError(t, err)
		worker, err := NewRenditionWorker(RenditionWorkerConfig{Catalog: f.catalog, Blobs: f.blobs, Runtime: runtime, Gate: newTestOperationGate(), Owner: "synthetic-batch", LeaseDuration: time.Minute, IdleDelay: time.Millisecond})
		require.NoError(t, err)
		_, err = worker.RunOne(t.Context())
		require.NoError(t, err)
		status, err := f.catalog.RenditionJobByID(t.Context(), job.ID)

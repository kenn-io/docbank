...
func (worker *EmbeddingWorker) ScanOnce(ctx context.Context) (int, error) {
	...
	for {
		...
		processed++
		if err := worker.processOwnedClaim(ctx, claim, work); err != nil {
			return processed, err
		}
	}
}

// RunJob processes at most one exact embedding job without reconciling or
// consuming any other queued work.
func (worker *EmbeddingWorker) RunJob(ctx context.Context, jobID string) (bool, error) {
	...
	return true, worker.processOwnedClaim(ctx, claim, work)
}

func (worker *EmbeddingWorker) processOwnedClaim(
	ctx context.Context, claim EmbeddingWorkClaim, work EmbeddingWork,
) error {
	...
	if err := worker.gate.MutateContext(ctx, func() error {
		...
		return fmt.Errorf("abandoning embedding work: %w", ErrEmbeddingPersistence)
	}
	return nil
}
...
func (worker *EmbeddingWorker) processClaim(ctx context.Context, claim EmbeddingWorkClaim, work EmbeddingWork) (retErr error) {
	...
	if err := worker.gate.MutateContext(attemptCtx, func() error {
		_, err := worker.authority.PublishEmbeddingHeadWithLease(attemptCtx, head, work.Consent,
			authorization, claim.AttemptID, claim.Epoch, worker.clock().UTC())
		return err
	}); err != nil {
		if isEmbeddingWorkFence(err) {
			return ErrEmbeddingWorkFenced
	...
	if err := worker.gate.MutateContext(leaseCtx, func() error {
		return worker.catalog.FinishEmbeddingWork(leaseCtx, claim, receipt, worker.clock().UTC())
	}); err != nil {
		if isEmbeddingWorkFence(err) {
			return ErrEmbeddingWorkFenced
		}

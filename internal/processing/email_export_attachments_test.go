...
func TestExportAttachmentVolumeJobRetryAndReadback(t *testing.T) {
	...
	require.NoError(t, err)
	worker, err := exporter.New(f.catalog, f.blobs, t.TempDir(), newTestOperationGate())
	require.NoError(t, err)
...
func TestExportQualifiedAttachmentPDFAndUnsupportedChild(t *testing.T) {
	...
	runtime := &EmailPDFRuntime{Catalog: f.catalog, Blobs: f.blobs, Renderer: pdfTestRenderer{rendered.Bytes()}, Recipe: document.EmailPDFRecipeV1{Contract: document.EmailPDFContract, RendererVersion: "synthetic-child-test", RendererSHA256: strings.Repeat("a", 64), WorkerSHA256: strings.Repeat("b", 64), FontsSHA256: strings.Repeat("c", 64), BubblewrapSHA256: strings.Repeat("d", 64), Paper: "A4"}}
	...
	require.NoError(t, err)
	worker, err := NewRenditionWorker(RenditionWorkerConfig{Catalog: f.catalog, Blobs: f.blobs, Runtime: runtime, Gate: newTestOperationGate(), Owner: "attachment-export", LeaseDuration: time.Minute, IdleDelay: time.Millisecond})
	require.NoError(t, err)

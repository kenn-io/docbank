...
func TestIndexWorkerMembershipOnlyDriftUsesDistinctGenerationIdentity(t *testing.T) {
	catalog, space := newIndexCatalogFixture(t)
	worker := newIndexWorkerForTest(t, catalog)
	...
	catalog.mu.Lock()
	source := catalog.sources[space]
	secondMember := source.Members[0]
	secondMember.EmbeddingSetID = indexWorkerHash("second-logical-member")
	source.Members = append(source.Members, secondMember)
	source.ManifestChecksum = indexWorkerHash("membership-only-source")
	...
	require.Equal(t, first.Bytes, second.Bytes,
		"deduplicated vector-set membership must retain identical index bytes")
	require.NotEqual(t, first.ID, second.ID,
		"generation identity must also bind the exact logical source membership")
}
...
func TestIndexWorkerRebuildHoldsDaemonPreservationGateAcrossBlobReads(t *testing.T) {
	catalog, space := newIndexCatalogFixture(t)
	catalog.readStarted = make(chan struct{})
	catalog.allowRead = make(chan struct{})
	gate := api.NewOperationGate()
	worker := newIndexWorkerWithConfig(t, catalog, IndexWorkerConfig{Gate: gate})

	rebuildDone := make(chan error, 1)
	go func() { _, err := worker.Rebuild(t.Context(), space); rebuildDone <- err }()
	<-catalog.readStarted

	commitStarted := make(chan struct{})
	commitDone := make(chan error, 1)
	go func() {
		commitDone <- gate.PhysicalMutate(func() error {
			close(commitStarted)
			return nil
		})
	}()
	select {
	case <-commitStarted:
		t.Fatal("physical authority changed while the vector worker was reading it")
	...
	case <-time.After(time.Second):
		t.Fatal("physical authority commit did not resume after the vector build")
	}

...
// RunEmailPDFRetentionTest keeps the existing processing fixture available to
// the external backup test without introducing a backupapp import cycle.
func RunEmailPDFRetentionTest(t *testing.T, released bool,
	restore func(*store.Store, *blob.Store) (*store.Store, *blob.Store),
) {
	t.Helper()
	...
	r.Recipe.BubblewrapSHA256 = strings.Repeat("d", 64)
	...
	require.NoError(t, err)
	worker, err := NewRenditionWorker(RenditionWorkerConfig{Catalog: f.catalog, Blobs: f.blobs, Runtime: r, Gate: newTestOperationGate(), Owner: "email-pdf-test", LeaseDuration: 60_000_000_000, IdleDelay: 1_000_000})
	require.NoError(t, err)
	...
	grantWorkerConsent(t, f.catalog, changed)
	letterJob, _, err := f.catalog.EnqueueRenditionJob(t.Context(), changed)
	require.NoError(t, err, "retained historical versions remain eligible for email PDF rendering")
	_, err = worker.RunJob(t.Context(), letterJob.ID)
	require.NoError(t, err)
	letter, err := f.catalog.EmailPDFReceipt(t.Context(), target.Version.ID, changed.Profile.Fingerprint)
	require.NoError(t, err)
	require.Equal(t, target.Version.ID, letter.Source.VersionID)
	require.Equal(t, "Letter", letter.Binding.Recipe.Paper)
	_, err = f.catalog.ActiveRendition(t.Context(), target.Version.ID, changed.Profile.Fingerprint)
	require.ErrorIs(t, err, store.ErrNotFound)
	_, _, err = f.catalog.Trash(t.Context(), target.Version.NodeID, store.UnconditionalRev)
	require.NoError(t, err)
	_, _, err = f.catalog.EnqueueRenditionJob(t.Context(), changed)
	require.ErrorIs(t, err, store.ErrRenditionJobStaleAuthority, "PDF rendering still requires a live source node")
	_, _, err = f.catalog.Restore(t.Context(), target.Version.NodeID, store.UnconditionalRev)
	require.NoError(t, err)
	restored, bs := restore(f.catalog, f.blobs)

...
func TestEmbeddingWorkerUsesRuntimeRetentionClassificationBeforeRetry(t *testing.T) {
	tests := []struct {
		name      string
		failure   error
		wantCalls int
		...
		{
			name: "joined retention is permanent",
			failure: errors.Join(embeddingTransientError{retryAfter: time.Millisecond},
				geminiembed.ErrRemoteRetentionUnconfirmed),
			wantCalls: 1,
		},
		{name: "ordinary transient retries", failure: embeddingTransientError{}, wantCalls: 2, wantHead: true},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			fixture := newEmbeddingWorkerFixture(t)
			work := fixture.work("runtime-retention-"+test.name, document.EmbeddingInputOriginalFile, "direct")
			...
			fixture.runtime.failures[work.Binding.Name] = []error{test.failure}
			...
			runtime, err := NewProviderEmbeddingRuntime(provider, blobs, t.TempDir(),
				func(error) (EmbeddingProviderFailure, time.Duration) {
					return EmbeddingProviderTransient, time.Millisecond
				})
			require.NoError(t, err)
			fixture.catalog.enqueue(work)
			worker := fixture.worker(t)
			worker.runtime = runtime
			worker.generationBlobs = blobs

			processed, err := worker.ScanOnce(t.Context())
			require.NoError(t, err)
			assert.Equal(t, 1, processed)
			assert.Equal(t, test.wantCalls, fixture.runtime.callCount(work.Binding.Name))
			assert.Equal(t, test.wantCalls, blobs.opens)
			if test.wantHead {
				assert.Contains(t, fixture.catalog.heads, headKey(work))
			} else {
				assert.NotContains(t, fixture.catalog.heads, headKey(work))
...
func (s *unavailableEmbeddingStore) PublishEmbeddingHeadWithLease(ctx context.Context, head store.EmbeddingHeadRecord, consent store.ProviderOperationAuthorizationRequest, prior store.ProviderOperationAuthorization, rootID string, token int64, at time.Time) (store.ProviderOperationAuthorization, error) {
	if s.publishErr != nil {
		err := s.publishErr
		s.publishErr = nil
		return store.ProviderOperationAuthorization{}, err
	}
	return s.Store.PublishEmbeddingHeadWithLease(ctx, head, consent, prior, rootID, token, at)
}

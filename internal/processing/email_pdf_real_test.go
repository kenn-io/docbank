...
func RunEmailPDFRealBackupFixture(t *testing.T, export func(string, *store.Store, *blob.Store)) {
	t.Helper()
	...
	require.NoError(t, err)
	bubblewrapHash, err := emailpdf.FileSHA256(bubblewrapPath)
	require.NoError(t, err)
	f := newEmailPipelineFixture(t)
	renderer, err := emailpdf.NewRuntime(emailpdf.RuntimeConfig{Worker: workerPath, WorkerSHA256: workerHash, Bubblewrap: bubblewrapPath, BubblewrapSHA256: bubblewrapHash, Chromium: filepath.Join(bundle, "chrome"), Bundle: bundle, BundleSHA256: bundleHash, Fonts: fonts, FontsSHA256: fontsHash, Version: "151.0.7922.34", Spool: f.spool})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	r := &EmailPDFRuntime{Catalog: f.catalog, Blobs: f.blobs, Renderer: renderer, Spool: f.spool, Recipe: document.EmailPDFRecipeV1{Contract: document.EmailPDFContract, RendererVersion: "151.0.7922.34", RendererSHA256: bundleHash, WorkerSHA256: workerHash, BubblewrapSHA256: bubblewrapHash, FontsSHA256: fontsHash, Paper: "A4"}}
	...
	require.NoError(t, err)
	worker, err := NewRenditionWorker(RenditionWorkerConfig{Catalog: f.catalog, Blobs: f.blobs, Runtime: r, Gate: newTestOperationGate(), Owner: "synthetic-pdf-proof", LeaseDuration: time.Minute, IdleDelay: time.Millisecond})
	require.NoError(t, err)
	...
	export(repositoryPath, f.catalog, f.blobs)

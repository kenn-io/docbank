...
func zipDirectory(r io.ReaderAt, size int64, limits ArchiveLimits) (*zip.Reader, []Entry, error) {
	if limits.EntryBytes <= 0 || limits.EntryBytes > 50<<30 || limits.ExpandedBytes <= 0 || limits.ExpandedBytes > 200<<30 {
		return nil, nil, store.ErrMailboxInvalid
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("open preflighted ZIP directory: %w", err)
	}
	names := map[string]bool{}
	entries := []Entry{}
	var total int64
	// Preview inspects only bounded directory metadata and sampled messages.
	for i, f := range archive.File {
		name := strings.ReplaceAll(f.Name, "\\", "/")
		clean := path.Clean(name)
		if !utf8.ValidString(name) || len(name) > 4096 || strings.ContainsAny(name, "\x00:") ||
			strings.HasPrefix(name, "/") || clean == ".." || strings.HasPrefix(clean, "../") ||
			strings.TrimSuffix(name, "/") != clean || names[clean] ||
			f.Mode()&os.ModeSymlink != 0 || (!f.FileInfo().IsDir() && !f.Mode().IsRegular()) {
			return nil, nil, store.ErrMailboxInvalid
		}
		names[clean] = true
		...
		size := int64(f.UncompressedSize64) //nolint:gosec // Bounded by EntryBytes above.
		total += size
		if !f.FileInfo().IsDir() && strings.EqualFold(path.Ext(f.Name), ".mbox") {
			entries = append(entries, Entry{Index: i, Name: name, Size: size, File: f})
		}
	}
	if len(entries) == 0 {
		return nil, nil, store.ErrMailboxInvalid
...
func PreflightZIP(ctx context.Context, r io.ReaderAt, size int64, limits ArchiveLimits) ([]Entry, error) {
	archive, entries, err := zipDirectory(r, size, limits)
	if err != nil {
		return nil, err
	}
	var total int64
	...
	for i, f := range archive.File {
		...
		total += n
		if entry < len(entries) && entries[entry].Index == i {
			entries[entry].SHA256 = hex.EncodeToString(h.Sum(nil))
			entry++
		}
	}
	return entries, nil

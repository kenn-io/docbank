...
func TestAPIKeyExclusionPolicyChecksDaemonKeyWithoutRetainingRawSecret(t *testing.T) {
	...
	policy := daemonconn.NewAPIKeyExclusionPolicy(daemonKey)

	assert.False(t, policy.Allows(daemonconn.New("http://127.0.0.1:1", daemonKey)))
	assert.True(t, policy.Allows(daemonconn.New("http://127.0.0.1:1", "independent-daemon-key")))
	assert.False(t, policy.Allows(daemonconn.New("http://127.0.0.1:1", "")))
...
func TestClientPreservesSourceMetadataOnStatAndVersionDetail(t *testing.T) {
	...
	c := daemonconn.New(server.URL, "")
	node, err := c.API().ResolvePath(t.Context(), &apiclient.ResolvePathRequestOptions{Query: &apiclient.ResolvePathQuery{Path: "/report.pdf"}})

	require.NoError(t, err)
	require.NotNil(t, node.SourceMetadata)
	assert.Equal(t, "Synthetic", *node.SourceMetadata.Fields[0].Value.String)
	version, err := c.API().GetContentVersion(t.Context(), &apiclient.GetContentVersionRequestOptions{PathParams: &apiclient.GetContentVersionPath{VersionID: "11111111-1111-4111-8111-111111111111"}})
...
func newClient(t *testing.T, clientKey string) (*daemonconn.Connection, *store.Store) {
	t.Helper()
	dir := t.TempDir()
	...
	t.Cleanup(ts.Close)
	return daemonconn.New(ts.URL, clientKey), s
}
...
func TestRoundTrip(t *testing.T) {
	...
	dir, err := c.API().CreateNode(ctx, &apiclient.CreateNodeRequestOptions{Body: &apiclient.CreateNodeBody{ParentID: s.RootID(), Name: "docs", Kind: "dir"}})
	...
	got, err := c.API().ResolvePath(ctx, &apiclient.ResolvePathRequestOptions{Query: &apiclient.ResolvePathQuery{Path: "/docs"}})

	require.NoError(t, err)
	assert.Equal(t, dir.ID, got.ID)

	renamed, err := c.API().MoveNode(ctx, &apiclient.MoveNodeRequestOptions{PathParams: &apiclient.MoveNodePath{ID: dir.ID}, Header: &apiclient.MoveNodeHeaders{IfMatch: strconv.Quote(strconv.FormatInt(got.Revision, 10))}, Body: &apiclient.MoveNodeBody{NewName: new("papers"), NewParentID: nil}})

	require.NoError(t, err)
	assert.Equal(t, "/papers", renamed.Path)

	moved, err := c.API().MovePath(ctx, &apiclient.MovePathRequestOptions{Body: &apiclient.MovePathBody{SrcPath: "/papers", DestPath: "/docs"}})

	require.NoError(t, err)
	assert.Equal(t, "/docs", moved.Path)

	movedAgain, err := c.MoveToPath(ctx, moved.ID, moved.Revision, "/filed")
	require.NoError(t, err)
	assert.Equal(t, "/filed", movedAgain.Path)

	kids, err := c.Children(ctx, s.RootID())
	require.NoError(t, err)
	assert.Len(t, kids, 1)

	trashed, err := c.API().TrashPath(ctx, &apiclient.TrashPathRequestOptions{Body: &apiclient.TrashPathBody{Path: "/filed"}})

	require.NoError(t, err)
	restored, err := c.API().RestoreNode(ctx, &apiclient.RestoreNodeRequestOptions{PathParams: &apiclient.RestoreNodePath{ID: dir.ID}, Header: &apiclient.RestoreNodeHeaders{IfMatch: strconv.Quote(strconv.FormatInt(trashed.Revision, 10))}})
...
func TestProvenanceRejectsMalformedAuthority(t *testing.T) {
	...
	_, err := daemonconn.New(ts.URL, "key").Provenance(t.Context(), 42, 10, 0)
...
func TestSearchWithOptionsUsesStableTagIdentity(t *testing.T) {
	...
	report, err := c.SearchWithOptions(
		ctx, "insurance", 10, daemonconn.SearchOptions{
	...
	filterReport, err := c.SearchWithOptions(ctx, "", 10, daemonconn.SearchOptions{TagID: tag.ID})
	require.NoError(t, err)
	require.Len(t, filterReport.Hits, 1)
	assert.Equal(t, "filter", filterReport.Hits[0].Match)
	_, err = c.SearchWithOptions(ctx, "", 10, daemonconn.SearchOptions{})
	...
	_, err = c.SearchWithOptions(ctx, "insurance", 10, daemonconn.SearchOptions{TagID: "bad"})
	require.ErrorContains(t, err, "canonical UUIDv4")
	_, err = c.SearchWithOptions(ctx, "insurance", 10, daemonconn.SearchOptions{
		MIMEType: "application/pdf; version=1",
	})
	require.ErrorContains(t, err, "must not include parameters")
	_, err = c.SearchWithOptions(ctx, "insurance", 10, daemonconn.SearchOptions{UnderNodeID: -1})
...
func TestMoveToPathValidatesRequestBeforeTransport(t *testing.T) {
	c := daemonconn.New("http://127.0.0.1:1", serverKey)
...
func TestAuditStatusBindsOnlyScopeTargetToEnrollmentBaseline(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			t.Cleanup(ts.Close)
			_, err := daemonconn.New(ts.URL, "key").AuditStatus(t.Context(), "", test.nodeID)
			if test.wantErr {
...
func TestEnableAuditRequiresEnabledScopeIdentity(t *testing.T) {
	...
	_, err := daemonconn.New(ts.URL, "key").EnableAudit(t.Context(), "preview-token", true)
...
func TestTagRoundTrip(t *testing.T) {
	c, s := newClient(t, serverKey)
	ctx := t.Context()
	node, err := c.API().CreateNode(ctx, &apiclient.CreateNodeRequestOptions{Body: &apiclient.CreateNodeBody{ParentID: s.RootID(), Name: "records", Kind: "dir"}})
	...
	node, err = c.API().ResolvePath(ctx, &apiclient.ResolvePathRequestOptions{Query: &apiclient.ResolvePathQuery{Path: "/records"}})
...
func TestErrorMapping(t *testing.T) {
	...
	_, err := c.API().ResolvePath(ctx, &apiclient.ResolvePathRequestOptions{Query: &apiclient.ResolvePathQuery{Path: "/missing"}})

	require.ErrorIs(t, err, store.ErrNotFound)

	d, err := c.API().CreateNode(ctx, &apiclient.CreateNodeRequestOptions{Body: &apiclient.CreateNodeBody{ParentID: s.RootID(), Name: "dup", Kind: "dir"}})

	require.NoError(t, err)
	_, err = c.API().CreateNode(ctx, &apiclient.CreateNodeRequestOptions{Body: &apiclient.CreateNodeBody{ParentID: s.RootID(), Name: "dup", Kind: "dir"}})

	require.ErrorIs(t, err, store.ErrExists)

	_, err = c.API().MoveNode(ctx, &apiclient.MoveNodeRequestOptions{PathParams: &apiclient.MoveNodePath{ID: d.ID}, Header: &apiclient.MoveNodeHeaders{IfMatch: strconv.Quote(strconv.FormatInt(d.Revision+99, 10))}, Body: &apiclient.MoveNodeBody{NewName: new("x"), NewParentID: nil}})
...
func TestDeferredRetirementProblemKeepsTypedClientError(t *testing.T) {
	...
	_, err := c.API().StorageRepack(t.Context(), &apiclient.StorageRepackRequestOptions{Body: &apiclient.StorageRepackBody{MaxBytes: new(int64(0)), MinAge: new(time.Duration(0).String()), MinDeadBytes: new(int64(0))}})
...
func TestAPIKeySent(t *testing.T) {
	...
	_, err := c.API().ListTrash(t.Context(), &apiclient.ListTrashRequestOptions{})

	// authed route succeeds only with the key
	require.NoError(t, err)
}

func TestJobsRoundTrip(t *testing.T) {
	c, _ := newClient(t, serverKey)
	items, err := c.API().ListJobs(t.Context())
...
func TestWatchedInboxesRoundTrip(t *testing.T) {
	c, _ := newClient(t, serverKey)
	items, err := c.API().ListWatchedInboxes(t.Context())
...
func TestStoragePackRoundTrip(t *testing.T) {
	...
	ingested, err := c.API().Ingest(t.Context(), &apiclient.IngestRequestOptions{Body: &apiclient.IngestBody{Paths: []string{src}, Dest: "/inbox"}})

	require.NoError(t, err)
	require.Equal(t, 1, ingested.Added)

	report, err := c.API().StoragePack(t.Context(), &apiclient.StoragePackRequestOptions{Body: &apiclient.StoragePackBody{MaxBytes: new(int64(1))}})
...
func TestIngestStreamRoundTrip(t *testing.T) {
	...
	report, err := c.IngestStream(t.Context(), []string{src}, "/inbox", daemonconn.IngestOptions{},
...
func TestIngestClientCarriesSelectionRulesToEveryRoute(t *testing.T) {
	...
	_, err := c.API().PreflightIngest(t.Context(), &apiclient.PreflightIngestRequestOptions{Body: &apiclient.PreflightIngestBody{Paths: []string{"/source"}, Include: include, Exclude: exclude}})

	require.NoError(t, err)
	_, err = c.API().Ingest(t.Context(), &apiclient.IngestRequestOptions{Body: &apiclient.IngestBody{Paths: []string{"/source"}, Dest: "/inbox", Include: include, Exclude: exclude, Replace: false, CollectionLabel: nil}})

	require.NoError(t, err)
	_, err = c.IngestStream(t.Context(), []string{"/source"}, "/inbox", daemonconn.IngestOptions{Include: include, Exclude: exclude}, nil)
	require.NoError(t, err)
...
func TestIngestOptionsSendReplaceToJSONAndStream(t *testing.T) {
	...
	c := daemonconn.New(ts.URL, serverKey)
	label := "Review set"
	opts := daemonconn.IngestOptions{
		Exclude: []string{"cache"}, Replace: true, CollectionLabel: &label,
	}
	jsonReport, err := c.API().Ingest(t.Context(), &apiclient.IngestRequestOptions{Body: &apiclient.IngestBody{Paths: []string{"/source"}, Dest: "/inbox", Include: opts.Include, Exclude: opts.Exclude, Replace: opts.Replace, CollectionLabel: opts.CollectionLabel}})
...
func TestProgressStreamPreservesProblemCode(t *testing.T) {
	...
	_, err := daemonconn.New(ts.URL, "key").IngestStream(
		t.Context(), []string{"/source"}, "/inbox", daemonconn.IngestOptions{}, nil)
	require.Error(t, err)
	code, ok := daemonconn.ProblemCode(err)
	assert.True(t, ok)
...
func TestMaintenanceBusyProblemIsRetryableAndTyped(t *testing.T) {
	...
	_, err := daemonconn.New(ts.URL, "key").API().VaultInfo(t.Context())

	require.ErrorIs(t, err, daemonconn.ErrMaintenanceBusy)
	code, ok := daemonconn.ProblemCode(err)
	assert.True(t, ok)
...
func TestJSONMethodsRejectInvalidUTF8BeforeRequest(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "ordinary", call: func() error {
			_, err := c.API().Ingest(t.Context(), &apiclient.IngestRequestOptions{Body: &apiclient.IngestBody{Paths: []string{invalidPath}, Dest: "/inbox", Include: nil, Exclude: nil, Replace: false, CollectionLabel: nil}})

			return err
		}},
		{name: "stream", call: func() error {
			_, err := c.IngestStream(t.Context(), []string{invalidPath}, "/inbox", daemonconn.IngestOptions{}, nil)
			return err
		}},
		{name: "preflight", call: func() error {
			_, err := c.API().PreflightIngest(t.Context(), &apiclient.PreflightIngestRequestOptions{Body: &apiclient.PreflightIngestBody{Paths: []string{invalidPath}, Include: nil, Exclude: nil}})

			return err
		}},
		{name: "mkdir name", call: func() error {
			_, err := c.API().CreateNode(t.Context(), &apiclient.CreateNodeRequestOptions{Body: &apiclient.CreateNodeBody{ParentID: 1, Name: invalidPath, Kind: "dir"}})
		...
		{name: "move name", call: func() error {
			_, err := c.API().MoveNode(t.Context(), &apiclient.MoveNodeRequestOptions{PathParams: &apiclient.MoveNodePath{ID: 2}, Header: &apiclient.MoveNodeHeaders{IfMatch: strconv.Quote(strconv.FormatInt(1, 10))}, Body: &apiclient.MoveNodeBody{NewName: new(invalidPath), NewParentID: nil}})

			return err
		}},
		{name: "move source path", call: func() error {
			_, err := c.API().MovePath(t.Context(), &apiclient.MovePathRequestOptions{Body: &apiclient.MovePathBody{SrcPath: invalidPath, DestPath: "/dest"}})

			return err
		}},
		{name: "move destination path", call: func() error {
			_, err := c.API().MovePath(t.Context(), &apiclient.MovePathRequestOptions{Body: &apiclient.MovePathBody{SrcPath: "/source", DestPath: invalidPath}})

			return err
		}},
		{name: "trash path", call: func() error {
			_, err := c.API().TrashPath(t.Context(), &apiclient.TrashPathRequestOptions{Body: &apiclient.TrashPathBody{Path: invalidPath}})
...
func TestBackupCreateStreamRoundTripAndTypedError(t *testing.T) {
	...
	_, err := c.API().InitBackupRepository(t.Context(), &apiclient.InitBackupRepositoryRequestOptions{Body: &apiclient.InitBackupRepositoryBody{Repo: new(repoPath)}})

	require.NoError(t, err)
	src := filepath.Join(t.TempDir(), "backup.txt")
	require.NoError(t, os.WriteFile(src, []byte("stream this backup"), 0o600))
	_, err = c.API().Ingest(t.Context(), &apiclient.IngestRequestOptions{Body: &apiclient.IngestBody{Paths: []string{src}, Dest: "/inbox"}})

	require.NoError(t, err)

	var events []api.BackupProgress
	snapshot, err := c.BackupCreateStream(t.Context(), daemonconn.BackupCreateOptions{
	...
	_, err = c.BackupCreateStream(cancelCtx, daemonconn.BackupCreateOptions{Repo: repoPath},
	...
	quick, err := c.API().VerifyBackupRepository(t.Context(), &apiclient.VerifyBackupRepositoryRequestOptions{Body: &apiclient.VerifyBackupRepositoryBody{Repo: new(repoPath), SnapshotID: new(snapshot.ID), All: new(false), Quick: new(true), Jobs: new(int64(0)), ForceUnlock: new(false)}})
	...
	_, err = c.BackupRestoreStream(restoreCtx, daemonconn.BackupRestoreOptions{
	...
	restored, err := c.BackupRestoreStream(t.Context(), daemonconn.BackupRestoreOptions{
...
func TestContentIdentityAndVerificationRoundTrip(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(src, content, 0o600))
	report, err := c.API().Ingest(t.Context(), &apiclient.IngestRequestOptions{Body: &apiclient.IngestBody{Paths: []string{src}, Dest: "/inbox"}})

	require.NoError(t, err)
	require.Equal(t, 1, report.Added)

	node, err := c.API().ResolvePath(t.Context(), &apiclient.ResolvePathRequestOptions{Query: &apiclient.ResolvePathQuery{Path: "/inbox/evidence.txt"}})
	...
	page, err := c.API().ListContentVersions(t.Context(), &apiclient.ListContentVersionsRequestOptions{PathParams: &apiclient.ListContentVersionsPath{ID: node.ID}, Query: &apiclient.ListContentVersionsQuery{Limit: new(int64(10)), Offset: new(int64(0))}})
	...
	version, err := c.API().GetContentVersion(t.Context(), &apiclient.GetContentVersionRequestOptions{PathParams: &apiclient.GetContentVersionPath{VersionID: node.CurrentVersionID}})
	...
	verified, err := c.API().VerifyNodeContent(t.Context(), &apiclient.VerifyNodeContentRequestOptions{PathParams: &apiclient.VerifyNodeContentPath{ID: node.ID}, Header: &apiclient.VerifyNodeContentHeaders{IfMatch: strconv.Quote(strconv.FormatInt(node.Revision, 10))}})

	require.NoError(t, err)
	assert.True(t, verified.Verified)
	assert.Equal(t, wantHash, verified.ComputedHash)

	packed, err := c.API().StoragePack(t.Context(), &apiclient.StoragePackRequestOptions{Body: &apiclient.StoragePackBody{MaxBytes: new(int64(0))}})
...
func TestContentReferencesRejectsInconsistentResponses(t *testing.T) {
	...
	for name, mutate := range tests {
		t.Run(name, func(t *testing.T) {
			...
			t.Cleanup(ts.Close)
			_, err := daemonconn.New(ts.URL, "").ContentReferences(t.Context(), hash, 10, 0)
...
func TestVersionContentRejectsUnprovenResponses(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			t.Cleanup(ts.Close)
			stream, err := daemonconn.New(ts.URL, "key").VersionContent(t.Context(), requestedVersion)
			if err != nil {
				require.ErrorContains(t, err, tt.wantError)
				assert.ErrorIs(t, err, daemonconn.ErrIntegrity)
				return
			}
...
func TestContentReplacementRoundTrip(t *testing.T) {
	...
	versions, err := c.API().ListContentVersions(t.Context(), &apiclient.ListContentVersionsRequestOptions{PathParams: &apiclient.ListContentVersionsPath{ID: created.Node.ID}, Query: &apiclient.ListContentVersionsQuery{Limit: new(int64(10)), Offset: new(int64(0))}})
...
func TestSavedQueryClientRequiresExactETagForEveryItemResponse(t *testing.T) {
	for _, responseETag := range []struct {
		...
		for _, operation := range []struct {
			name string
			call func(*daemonconn.Connection) error
		}{
			{"create", func(c *daemonconn.Connection) error {
			...
			t.Run(responseETag.name+" "+operation.name, func(t *testing.T) {
				...
				t.Cleanup(ts.Close)
				err := operation.call(daemonconn.New(ts.URL, serverKey))
...
func TestSavedQueryClientValidatesOffsetAwarePageCardinality(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			t.Cleanup(ts.Close)
			_, err := daemonconn.New(ts.URL, serverKey).SavedQueries(
...
func TestProvenanceAppendMapsInvalidTimeError(t *testing.T) {
	...
	t.Cleanup(ts.Close)
	_, err := daemonconn.New(ts.URL, serverKey).AppendProvenance(t.Context(), 7, 3,
...
func TestProvenanceAppendRejectsUnboundReceipt(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Cleanup(ts.Close)
			_, err := daemonconn.New(ts.URL, serverKey).AppendProvenance(t.Context(), 7, 3, request)
...
func TestContentReplacementRejectsUnprovenReceipt(t *testing.T) {
	...
	for name, mutate := range tests {
		t.Run(name, func(t *testing.T) {
			...
			t.Cleanup(ts.Close)
			_, err := daemonconn.New(ts.URL, "key").ReplaceContent(t.Context(), 7, 3, "text/plain",
	...
	*base.Version.SourceVersionID = sourceID
	...
	for name, mutate := range tests {
		t.Run(name, func(t *testing.T) {
			...
			t.Cleanup(ts.Close)
			_, err := daemonconn.New(ts.URL, "key").RevertContent(t.Context(), 7, 3, sourceID)
			require.Error(t, err)
		})
	}
}
...
func TestContentVersionPruneValidatesReceipt(t *testing.T) {
	...
	t.Run("visual preview outputs", func(t *testing.T) {
		...
		t.Cleanup(ts.Close)
		_, err := daemonconn.New(ts.URL, "key").PruneContentVersions(
...
func TestStorageRepackRoundTrip(t *testing.T) {
	...
	_, err := c.API().StoragePack(t.Context(), &apiclient.StoragePackRequestOptions{Body: &apiclient.StoragePackBody{MaxBytes: new(int64(0))}})

	require.NoError(t, err)
	for _, path := range []string{"/inbox/drop-a.txt", "/inbox/drop-b.txt"} {
		drop, statErr := c.API().ResolvePath(t.Context(), &apiclient.ResolvePathRequestOptions{Query: &apiclient.ResolvePathQuery{Path: path}})

		require.NoError(t, statErr)
		_, err = c.API().TrashNode(t.Context(), &apiclient.TrashNodeRequestOptions{PathParams: &apiclient.TrashNodePath{ID: drop.ID}, Header: &apiclient.TrashNodeHeaders{IfMatch: strconv.Quote(strconv.FormatInt(drop.Revision, 10))}})

		require.NoError(t, err)
	}
	_, err = c.API().EmptyTrash(t.Context(), &apiclient.EmptyTrashRequestOptions{Body: &apiclient.EmptyTrashBody{OlderThan: new(""), Run: new(true)}})

	require.NoError(t, err)
	_, err = c.API().Gc(t.Context(), &apiclient.GcRequestOptions{Body: &apiclient.GcBody{Run: true}})
	...
	require.Eventually(t, func() bool {
		report, repackErr = c.API().StorageRepack(t.Context(), &apiclient.StorageRepackRequestOptions{Body: &apiclient.StorageRepackBody{MaxBytes: new(int64(0)), MinAge: new(time.Nanosecond.String()), MinDeadBytes: new(int64(1))}})
...
func TestRestoreTargetContentionRemainsTyped(t *testing.T) {
	...
	_, err := daemonconn.New(ts.URL, "key").API().ListTrash(t.Context(), &apiclient.ListTrashRequestOptions{})

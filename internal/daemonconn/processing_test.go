...
func TestProcessingClientUsesTypedRoutesAndVerifiesRenditionStream(t *testing.T) {
	...
	profiles, err := c.API().ListDocumentProcessingProfiles(t.Context())
	...
	grant, err := c.API().GrantDocumentProcessingConsent(t.Context(), &apiclient.GrantDocumentProcessingConsentRequestOptions{Body: &api.ProcessingConsentGrantRequest{Selector: selector, PlanFingerprint: plan.Fingerprint}})
	...
	headerCompleteness = "complete"
	revocation, err := c.API().RevokeDocumentProcessingConsent(t.Context())

	require.NoError(t, err)
	assert.Equal(t, "2026-08-28T00:00:00Z", revocation.RevokedAt)
	purgePlan, err := c.API().PlanDerivativePurge(t.Context(), &apiclient.PlanDerivativePurgeRequestOptions{Body: &api.DerivativePurgePlanRequest{AttachmentIDs: []string{attachmentID}}})
...
func TestProcessingClientPreservesRenditionOutcomes(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.code, func(t *testing.T) {
			...
			defer server.Close()
			_, err := daemonconn.New(server.URL, serverKey).StartProcessing(t.Context(), api.StartProcessingRequest{Selector: api.ProcessingSelector{ContentVersionID: processingStreamVersionID}}, processingStreamProfile)
...
func TestProcessingClientRejectsFailedStatesAndPreservesOptionalResults(t *testing.T) {
	for _, test := range []struct {
		...
		{"failed", "processing_consent_expired", daemonconn.ErrProcessingConsent},
		...
		t.Run(test.state+"/"+test.code, func(t *testing.T) {
			...
			t.Cleanup(server.Close)
			got, err := daemonconn.New(server.URL, serverKey).StartProcessing(t.Context(), api.StartProcessingRequest{Selector: api.ProcessingSelector{ContentVersionID: processingStreamVersionID}}, processingStreamProfile)
...
func TestProcessingClientPreservesCompletedJobWhenStatusIsUnavailable(t *testing.T) {
	...
	t.Cleanup(server.Close)
	job, err := daemonconn.New(server.URL, serverKey).StartProcessing(t.Context(), api.StartProcessingRequest{Selector: api.ProcessingSelector{ContentVersionID: processingStreamVersionID}}, processingStreamProfile)
	require.Error(t, err)
	code, ok := daemonconn.ProblemCode(err)
	require.True(t, ok)
...
func TestProcessingClientRejectsRevocationBeforeFirstJobEvent(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/x-ndjson")
		assert.NoError(t, json.MarshalWrite(w, api.ProcessingJobEvent{
			Sequence: 1, Type: "error", Terminal: true,
			Error: api.NewError(http.StatusForbidden, "operation_denied", "processing access was revoked"),
		}))
	}))
	t.Cleanup(server.Close)
	job, err := daemonconn.New(server.URL, serverKey).StartProcessing(t.Context(),
		api.StartProcessingRequest{Selector: api.ProcessingSelector{ContentVersionID: processingStreamVersionID}},
		processingStreamProfile)
	require.Error(t, err)
	code, ok := daemonconn.ProblemCode(err)
...
func TestDerivativePurgeClientPreservesDeferredReceipt(t *testing.T) {
	...
	t.Cleanup(server.Close)
	receipt, err := daemonconn.New(server.URL, serverKey).RunDerivativePurge(t.Context(), api.DerivativePurgeJobRequest{})
	require.Error(t, err)
	code, ok := daemonconn.ProblemCode(err)
	require.True(t, ok)
...
func TestProcessingClientDeliversDurableJobBeforeBlockedTerminal(t *testing.T) {
	...
	t.Cleanup(server.Close)
	stream, err := daemonconn.New(server.URL, serverKey).StartProcessingStream(t.Context(), api.StartProcessingRequest{Selector: api.ProcessingSelector{ContentVersionID: processingStreamVersionID}}, processingStreamProfile)
	require.NoError(t, err)
...
func TestProcessingClientValidatesSearchResponseAgainstExactFence(t *testing.T) {
	...
	t.Cleanup(server.Close)
	report, err := daemonconn.New(server.URL, serverKey).SearchDocuments(t.Context(), request)
	require.NoError(t, err)
...
func TestProcessingClientValidatesSourceFenceResponseAuthority(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			t.Cleanup(server.Close)
			result, err := daemonconn.New(server.URL, serverKey).ResolveDocumentSourceFence(t.Context(),
...
func TestProcessingClientPreservesTypedFilteredScopeOverflow(t *testing.T) {
	for _, observed := range []int{4097, 4096, 0} {
		t.Run(strconv.Itoa(observed), func(t *testing.T) {
			...
			var overflow *daemonconn.SourceFenceScopeTooLargeError
			if observed > 4096 {
...
func TestProcessingClientRejectsResultRevokedFromConsumerFence(t *testing.T) {
	...
	t.Cleanup(server.Close)
	report, err := daemonconn.New(server.URL, serverKey).SearchDocuments(t.Context(), api.DocumentSearchRequest{
	...
	for _, test := range []struct {
		...
		for _, terminal := range []bool{false, true} {
			t.Run(test.name+"/terminal="+strconv.FormatBool(terminal), func(t *testing.T) {
				...
				t.Cleanup(server.Close)
				stream, err := daemonconn.New(server.URL, serverKey).StartProcessingStream(t.Context(), api.StartProcessingRequest{
				...
				if test.mutate == nil {
					...
					_, problem := daemonconn.ProblemCode(err)
...
func TestProcessingClientValidatesStatusEmbeddingIdentities(t *testing.T) {
	for _, ids := range [][]string{{"invalid"}, {strings.Repeat("a", 64), strings.Repeat("a", 64)}} {
		t.Run(strings.Join(ids, ","), func(t *testing.T) {
			...
			t.Cleanup(server.Close)
			got, err := daemonconn.New(server.URL, serverKey).StartProcessing(t.Context(), api.StartProcessingRequest{

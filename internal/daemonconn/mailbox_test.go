...
func TestMailboxClientReadsFullBoundedJobPage(t *testing.T) {
	...
	require.NoError(t, err)
	tag, err := s.CreateTag(ctx, "Synthetic mailbox label")
	require.NoError(t, err)
	mapping := map[string]string{}
	for i := range 100 {
		mapping[fmt.Sprintf("%03d-%s", i, strings.Repeat("x", 248))] = tag.ID
	}
	for i := range 80 {
...
func TestMailboxClientStreamsEMLWithDurableReceipt(t *testing.T) {
	c, s := newClient(t, serverKey)
	ctx := t.Context()
	require.NoError(t, c.RegisterMailboxArchive(ctx, store.MailboxArchive{ID: "synthetic-archive", Description: "Synthetic exported EML"}))
	raw := []byte("Subject: Synthetic transfer\r\nContent-Type: text/plain\r\n\r\nTransferred body\r\n")
	sum := sha256.Sum256(raw)
	request := store.MailboxTransferRequest{
		ArchiveID: "synthetic-archive", Reference: "message-1", SHA256: hex.EncodeToString(sum[:]),
		Size: int64(len(raw)), Settings: "docbank-eml-transfer/v1", DestinationID: s.RootID(), Name: "message.eml",
	}
	receipt, err := c.TransferMailboxEML(ctx, request, bytes.NewReader(raw))
	require.NoError(t, err)
	stored, err := s.MailboxTransfer(ctx, "vault:"+s.VaultID(), request.ArchiveID, request.Reference)
	require.NoError(t, err)
	require.Equal(t, request.SHA256, stored.Target.SHA256)
	require.Equal(t, stored, receipt)
}

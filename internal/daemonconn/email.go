...
func (c *Connection) EmailMetadata(ctx context.Context, versionID string) (api.EmailMetadata, error) {
...
func (c *Connection) EnsureEmailMetadata(ctx context.Context, versionID string) (api.EmailMetadata, error) {
	var metadata api.EmailMetadata
	if !validUUIDv4(versionID) {
		return metadata, errors.New("email ensure requires a canonical UUIDv4 version ID")
	}
	response, err := c.API().EnsureEmailMetadata(ctx, &apiclient.EnsureEmailMetadataRequestOptions{PathParams: &apiclient.EnsureEmailMetadataPath{VersionID: uuid.MustParse(versionID)}, Body: new(apiclient.EnsureEmailMetadataBody{})})
	if err != nil {
		return api.EmailMetadata{}, err
	}
	metadata = *response
...
func (c *Connection) EmailMetadataGeneration(
	...
	response, err := c.API().GetEmailMetadataGeneration(ctx, &apiclient.GetEmailMetadataGenerationRequestOptions{PathParams: &apiclient.GetEmailMetadataGenerationPath{VersionID: uuid.MustParse(versionID), GenerationID: generationID}})
	if err != nil {
		return api.EmailMetadata{}, err
	}
	metadata = *response
...
func (c *Connection) OpenEmailPart(

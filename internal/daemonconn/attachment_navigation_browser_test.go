...
func TestAttachmentNavigationRealDaemonBrowser(t *testing.T) {
	...
	record, _, found, err := daemonconn.Find(t.Context(), vault)
	require.NoError(t, err)
	require.True(t, found)
	c := daemonconn.New("http://"+record.Address, record.Metadata["api_key"])
	...
	publish := func(versionID, operation string) document.EmailDocumentPublicationReceipt {
		...
		dir, readErr := c.API().GetNode(t.Context(), &apiclient.GetNodeRequestOptions{PathParams: &apiclient.GetNodePath{ID: root.ID}})
		require.NoError(t, readErr)

...
func TestClientEmailDocumentProcessingConsentErrors(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.code, func(t *testing.T) {
			...
			_, err := daemonconn.New(server.URL, serverKey).RequestEmailDocumentProcessing(t.Context(), document.EmailDocumentProcessingRequest{})
...
func TestClientEmailDocumentsRoundTrip(t *testing.T) {
	...
	for _, failure := range []struct {
		...
		t.Run("committed_response_"+failure.name, func(t *testing.T) {
			...
			t.Cleanup(server.Close)
			_, err = daemonconn.New(server.URL, serverKey).PublishEmailDocuments(t.Context(), request)
			require.Error(t, err)
			...
			require.True(t, daemonconn.IsResponseDecodeError(err), "committed mutation must have unknown outcome: %v", err)

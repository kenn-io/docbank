...
func TestMCPRequestPathsDoNotClassifyRedirectResponseAsTransport(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "JSON", call: func(ctx context.Context, c *Connection) error { _, err := c.API().Health(ctx); return err }},
		{name: "processing start", call: func(ctx context.Context, c *Connection) error {
			_, err := c.StartProcessing(ctx, api.StartProcessingRequest{}, strings.Repeat("b", 64))
			return err
		}},
		{name: "rendition", call: func(ctx context.Context, c *Connection) error {
			_, err := c.Rendition(ctx, attachmentID, maxBytes)
			return err
		}},
		{name: "rendition selector", call: func(ctx context.Context, c *Connection) error {
			_, err := c.RenditionForSelector(ctx, api.ProcessingSelector{}, maxBytes)
			return err
		}},
		{name: "rendition range", call: func(ctx context.Context, c *Connection) error {
...
func TestStartProcessingClassifiesTruncatedStreamAsReceivedResponse(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			_, err := New(server.URL, "synthetic-key").StartProcessing(t.Context(), api.StartProcessingRequest{}, strings.Repeat("b", 64))
			require.Error(t, err)
...
func TestRequestOutcomeClassifiesOnlyFailureBeforeResponseAsTransport(t *testing.T) {
	...
	t.Run("partial successful response", func(t *testing.T) {
		...
		_, err := New(server.URL, "synthetic-key").API().GetNode(t.Context(), &apiclient.GetNodeRequestOptions{PathParams: &apiclient.GetNodePath{ID: 1}})

...
func TestListDocumentsUsesStoreUnicodePathNormalization(t *testing.T) {
	...
	page, err := daemonconn.New(ts.URL, "daemon-key").ListDocuments(t.Context(), api.DocumentQuery{
		PathPrefix: "/Cafe\u0301",
	})
	require.NoError(t, err)
...
func TestResolveDocumentSummariesRejectsDuplicateIdentity(t *testing.T) {
	...
	_, err := daemonconn.New("http://127.0.0.1:1", "synthetic-key").ResolveDocumentSummaries(t.Context(),
		api.DocumentSummaryResolveRequest{Identities: []api.DocumentIdentity{identity, identity}})
	require.ErrorIs(t, err, store.ErrInvalidDocumentQuery)
}

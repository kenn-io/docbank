...
type ValidateInput struct {
	...
	Resolver  *Resolver
	Head      []byte
	PageCount func(file *os.File) (int, error)
}
	...
	for _, record := range in.Records {
		...
		for _, fileRef := range record.Files {
			...
			if in.PageCount != nil && (fileRef.Role == "native" || fileRef.Role == "produced_pdf") && strings.EqualFold(filepath.Ext(fileRef.RelPath), ".pdf") {
				...
				} else if declared := declaredPagesFor(record.DocID, in.Images); declared > 0 && actual != declared {
...
func declaredPagesFor(docID string, images []ImageRef) int {
	for _, image := range images {
		if image.ImageKey == docID && image.DocumentBreak && image.DeclaredPageCount > 0 {
			return image.DeclaredPageCount
		}
	}
	return 0
}

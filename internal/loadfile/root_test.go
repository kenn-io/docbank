...
func TestResolverDiscoversLoadFilesAndEnforcesVolumeBound(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, resolver.Close()) })
	dat, opt, volumes, err := resolver.DiscoverPackageFiles("dat-concordance-v1")
	require.ErrorIs(t, err, ErrLoadfileLimit)
	assert.Empty(t, dat)
	assert.Empty(t, opt)
	...
	dat, opt, volumes, err = bounded.DiscoverPackageFiles("dat-concordance-v1")
	require.NoError(t, err)
...
func TestDiscoveryExplainsRootLoadFilesAndRejectsCompetingPageMaps(t *testing.T) {
	...
	resolver, err := NewResolver(t.Context(), root, nil)
	require.NoError(t, err)
	dat, pageMap, volumes, err := resolver.DiscoverPackageFiles("dat-concordance-v1")
	assert.Empty(t, dat)
...
func TestResolverSelectsLoadFileForDeclaredDialect(t *testing.T) {
	root := t.TempDir()
	require.NoError(t, os.Mkdir(filepath.Join(root, "VOL001"), 0o700))
	require.NoError(t, os.WriteFile(filepath.Join(root, "VOL001", "package.dat"), []byte("data"), 0o600))
	require.NoError(t, os.WriteFile(filepath.Join(root, "VOL001", "metadata.csv"), []byte("metadata"), 0o600))
	resolver, err := NewResolver(t.Context(), root, nil)
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, resolver.Close()) })
	dat, _, _, err := resolver.DiscoverPackageFiles("dat-concordance-v1")
	require.NoError(t, err)
	assert.Equal(t, "VOL001/package.dat", dat)
	csv, _, _, err := resolver.DiscoverPackageFiles("csv-rfc4180-v1")
	require.NoError(t, err)

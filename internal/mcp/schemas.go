...
func listPackagePreflightDiagnosticsSchemas() (schema, schema) {
	return rootObjectSchema(schema{
			"preflight_id": uuidSchema(), "cursor": stringSchema(maxCursorCharacters), "limit": integerSchema(1, maxPackageDiagnostics), //nolint:goconst // Published JSON field name.
...
func listTagsSchemas() (schema, schema) {
	input := rootObjectSchema(schema{
		"limit": integerSchema(1, 250), tagOffsetField: integerSchema(0, 1_000_000),
	})
	tag := objectSchema(schema{
		"id": uuidSchema(), "name": stringSchema(0),
		"revision": integerSchema(1, 0), "assignment_count": integerSchema(0, 0),
	}, "id", "name", "revision", "assignment_count")
	output := rootObjectSchema(withPrivateCache(schema{
		"items": arraySchema(tag, 250), "total": integerSchema(0, 0),
		"limit": integerSchema(1, 250), tagOffsetField: integerSchema(0, 1_000_000),
	}), cacheRequired("items", "total", "limit", tagOffsetField)...)
	return input, output
}
...
func listProcessingProfilesSchemas() (schema, schema) {
	input := rootObjectSchema(schema{})
	profile := objectSchema(schema{
		"name":                     stringSchema(128),
		"fingerprint":              sha256Schema(),
		"rendition":                booleanSchema(),
		"embedding_bindings":       arraySchema(stringSchema(128), 64),
		"query_embedding_bindings": arraySchema(stringSchema(128), 64),
		"reranking_available":      booleanSchema(),
	}, "name", "fingerprint", "rendition", "embedding_bindings", "query_embedding_bindings")
	output := rootObjectSchema(withPrivateCache(schema{
		"profiles": arraySchema(profile, 128),
	}), cacheRequired("profiles")...)
	return input, output
}

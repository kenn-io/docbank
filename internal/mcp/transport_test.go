...
func TestStdioRejectsOversizedFrameBeforeDecodeWithoutEchoingIt(t *testing.T) {
	...
	err := ServeStdio(t.Context(), newTestServer(), io.NopCloser(strings.NewReader(frame)),
		testWriteCloser{Writer: &stdout}, slog.New(slog.NewTextHandler(&diagnostics, nil)))
...
func TestHTTPTransportReturnsJSONWithoutSessions(t *testing.T) {
	handler, err := newTestServer().HTTPTransportHandler(HTTPOptions{BearerToken: testMCPBearer})
	require.NoError(t, err)
	request := newHTTPDiscoverRequest(t)
	request.Header.Set("Authorization", "Bearer "+testMCPBearer)
	...
	assert.Contains(t, response.Body.String(), `"result"`)
	assert.Empty(t, response.Header().Get("Mcp-Session-Id"))
	assert.Equal(t, "no-store", response.Header().Get("Cache-Control"))
}

func TestHTTPSubscriptionWithoutCapabilitiesCompletesOverSSE(t *testing.T) {
	handler, err := newTestServer().HTTPTransportHandler(HTTPOptions{BearerToken: testMCPBearer})
	require.NoError(t, err)
	body, err := json.Marshal(map[string]any{
		"jsonrpc": "2.0", "id": 1, "method": "subscriptions/listen",
		"params": map[string]any{"_meta": testRequestMeta(), "notifications": map[string]any{}},
	})
	require.NoError(t, err)
	request := httptest.NewRequest(http.MethodPost, "http://127.0.0.1/mcp", bytes.NewReader(body))
	request.Header = protocolHeaders("subscriptions/listen", "")
	request.Header.Set("Authorization", "Bearer "+testMCPBearer)
	response := httptest.NewRecorder()
	...
	assert.Equal(t, "no-store", response.Header().Get("Cache-Control"))
	assert.Empty(t, response.Header().Get("Mcp-Session-Id"))
	assert.Contains(t, response.Body.String(), `"method":"notifications/subscriptions/acknowledged"`)
	assert.Contains(t, response.Body.String(), `"resultType":"complete"`)
	for event := range strings.SplitSeq(strings.TrimSpace(response.Body.String()), "\n\n") {
		_, payload, ok := strings.Cut(event, "data: ")
		require.True(t, ok, "each event must carry one JSON-RPC message")
...
func TestHTTPTransportBoundsConcurrentActivity(t *testing.T) {
	...
	handler, err := wrapHTTPTransport(inner, HTTPOptions{
		BearerToken: testMCPBearer,
		limits:      httpLimits{MaxConcurrentRequests: 2},
	})
	require.NoError(t, err)
	...
	go func() {
		response := httptest.NewRecorder()
		handler.ServeHTTP(response, authenticatedRequest("127.0.0.1:41002"))
		secondDone <- response
	}()
	<-entered
...
func TestHTTPTransportCapsResponsesAndLogsOnlyStableTelemetry(t *testing.T) {
	...
	handler, err := wrapHTTPTransport(inner, HTTPOptions{
		...
		limits: httpLimits{
			MaxResponseBytes: 128, MaxConcurrentRequests: 1,
			RequestTimeout: time.Second,
		},
	})
	require.NoError(t, err)

...
func TestExportMCPReadToolsAreDefaultAndWritesRequireOwnOptIn(t *testing.T) {
	...
	require.Contains(t, readOnly, "get_export_plan")
	require.Contains(t, readOnly, "list_export_output_problems")
...
func TestExportCancellationAdvertisesSafeRepeatAfterLostResponse(t *testing.T) {
	tool := catalogMap(toolCatalog(false, false, true))["cancel_export_job"]
	require.NotNil(t, tool)
	require.NotNil(t, tool.Annotations)
	require.True(t, tool.Annotations.IdempotentHint)
	require.Equal(t, new(true), tool.Annotations.DestructiveHint)
}
...
func TestExportMCPToolsDriveExactSyntheticPDFJob(t *testing.T) {
	...
	plan := bundle.Plan{ID: planID, Source: source, Fingerprint: fingerprint, Total: 1,
		Roles: []bundle.RolePolicy{{Role: "original"}}, ExpiresAt: "2099-01-01T00:00:00Z"}
	...
	daemon := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/exports/plans/"+planID:
			result = plan
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/exports/plans/"+planID+"/problems" && r.URL.Query().Get("after") == "0":
			result = bundle.OutputProblems{PlanID: planID, Fingerprint: fingerprint, After: 0, Next: 1,
				Total: 2, Items: []bundle.OutputProblem{{NodeID: 7, VersionID: versionID, Role: "text", Reason: "unavailable"}}}
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/exports/plans/"+planID+"/problems" && r.URL.Query().Get("after") == "1":
			result = bundle.OutputProblems{PlanID: planID, Fingerprint: fingerprint, After: 1, Next: 0,
				Total: 2, Items: []bundle.OutputProblem{{NodeID: 7, VersionID: versionID, Role: "pages", Reason: "unavailable"}}}
	...
	retainedPlan := call("get_export_plan", map[string]any{"plan_id": planID})
	require.Equal(t, sourceID, retainedPlan["source_id"])
	require.Equal(t, memberHash, retainedPlan["member_hash"])
	problems := call("list_export_output_problems", map[string]any{"plan_id": planID, "after": 0})
	require.Equal(t, fingerprint, problems["fingerprint"])
	require.EqualValues(t, 2, problems["total"])
	require.EqualValues(t, 1, problems["next"])
	require.Equal(t, "text", firstExportProblemRole(t, problems))
	finalPage := call("list_export_output_problems", map[string]any{"plan_id": planID, "after": 1})
	require.EqualValues(t, 0, finalPage["next"])
	require.Equal(t, "pages", firstExportProblemRole(t, finalPage))
...
func firstExportProblemRole(t *testing.T, page map[string]any) string {
	t.Helper()
	items, ok := page["items"].([]any)
	require.True(t, ok)
	require.Len(t, items, 1)
	problem, ok := items[0].(map[string]any)
	require.True(t, ok)
	role, ok := problem["role"].(string)
	require.True(t, ok)
	return role
}

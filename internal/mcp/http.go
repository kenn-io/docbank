...
// ValidHTTPBearerToken accepts a bounded bearer value without whitespace or controls.
func ValidHTTPBearerToken(token string) bool {
...
func (guard *httpTransportGuard) authenticated(header http.Header) bool {
	...
	if !ok || !ValidHTTPBearerToken(candidate) {
		return false
	}
...
func (guard *httpTransportGuard) acquire() bool {
...
func (guard *httpTransportGuard) release() {
	guard.activityMu.Lock()
	defer guard.activityMu.Unlock()
	guard.activeTotal--
}

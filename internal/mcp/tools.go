...
func catalogInstructions(allowProcessing, allowPackageWrites, allowReportWrites bool) string {
	if !allowProcessing && !allowPackageWrites && !allowReportWrites {
...
func toolCatalog(allowProcessing, allowPackageWrites, allowReportWrites bool) []*sdkmcp.Tool {
...
func registerToolCatalog(
	server *sdkmcp.Server, allowProcessing, allowPackageWrites, allowReportWrites bool, lease *daemonLease, plans *processingPlanRegistry, logger *slog.Logger,
) *reportHandleSigner {
	tools := toolCatalog(allowProcessing, allowPackageWrites, allowReportWrites)
	...
	for _, tool := range tools {
		...
		case "get_report_summary", "get_report_dates", "create_report", "revise_report":

...
func toolCatalog(allowProcessing, allowPackageWrites bool, allowExportWrites ...bool) []*sdkmcp.Tool {
	...
	for _, definition := range definitions {
		...
		annotation := &sdkmcp.ToolAnnotations{
			...
			IdempotentHint: (!definition.write && !definition.nonIdempotent) || definition.idempotent, DestructiveHint: &definition.destructive, OpenWorldHint: &openWorld,
		}
...
func registerToolCatalog(
	server *sdkmcp.Server, allowProcessing, allowPackageWrites, allowExportWrites bool, lease *daemonLease, plans *processingPlanRegistry, archives *exportArchiveRegistry, logger *slog.Logger,
) {
	...
	for _, tool := range tools {
		...
		case "preview_export_plan", "get_export_job", "create_export_source", "create_export_plan", "start_export_job", "cancel_export_job", "open_export_archive", "download_export_archive":
...
func domainErrorMessage(code string) string {
	...
	case "visibility_changed":
		return "The export source is no longer visible."
	case "access_denied":
		return "The current owner cannot access this Docbank result."

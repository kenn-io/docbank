...
func executeProcessingToolWithPolicy(
	...
	// The daemon connection authenticates as local admin, so forwarding a
	// scoped caller would enqueue work without its durable source grant.
	if !policy.local() {
		return nil, api.ErrOperationDenied
	}

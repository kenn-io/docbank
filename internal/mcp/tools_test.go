...
func TestDefaultToolCatalogIsFixedBoundedAndReadOnly(t *testing.T) {
	...
	wantNames := []string{
		"get_agent_capabilities", "get_vault_info", "list_documents", "list_tags", "search_documents", "get_document",
		"list_document_versions", "read_rendition_text", "get_processing_plan",
		"get_processing_status", "get_processing_coverage", "list_processing_profiles", "get_format_coverage", "get_package_import",
		...
		"lookup_bates_label", "preview_export_plan", "get_export_plan", "list_export_output_problems",
		"get_export_job", "open_export_archive", "download_export_archive",
		"open_report_artifact", "download_report_artifact",
		"get_report_summary", "list_report_history", "get_report_dates",
	}
	...
	for index, tool := range tools {
		...
		assert.Equal(t, tool.Name != "open_export_archive" && tool.Name != "download_export_archive" &&
			tool.Name != "open_report_artifact" && tool.Name != "download_report_artifact",
...
func TestScopedCatalogOmitsUnfencedPackageTools(t *testing.T) {
	...
	assert.Contains(t, names, "list_processing_profiles")
	assert.Contains(t, names, "list_tags")
...
func TestSessionFileCatalogAdvertisesQualifiedSourceReads(t *testing.T) {
	server := newServerWithOptions(testImplementation(), ServerOptions{ScopedAgentSession: true,
		Principal:       api.Principal{SubjectID: "agent:synthetic", CredentialKind: "agent_session"},
		AllowProcessing: true, AllowPackageWrites: true, AllowExportWrites: true, AllowReportWrites: true})
	listed := decodeResult(t, exchangeRaw(t, server, requestFor("tools/list", map[string]any{})))
	assert.Equal(t, []string{"get_agent_capabilities", "get_document", "get_format_coverage", "get_vault_info", "list_documents",
		"list_processing_profiles", "list_tags", "read_rendition_text"},
		listedToolNames(t, listed))
}

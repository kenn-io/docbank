...
func TestSupervisorTracksDeterministicTerminalStates(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		synctest.Wait()
		jobs := s.Snapshot()
		require.True(t, len(jobs) == 3 && jobs[0].Status == StatusCompleted && jobs[1].Status == StatusFailed)
...
func TestSupervisorCapturesPanicsAndBoundsErrors(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		synctest.Wait()
		jobs := s.Snapshot()
		require.True(t, len(jobs) == 2 && jobs[0].Status == StatusFailed && jobs[1].Status == StatusFailed)

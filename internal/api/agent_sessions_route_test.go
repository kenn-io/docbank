...
func TestAgentSessionRoutesMintScopeAndRevokeWithoutMasterFallback(t *testing.T) {
	...
	grantRequest, err := http.NewRequestWithContext(t.Context(), http.MethodGet,
		ts.URL+"/api/v1/agent/capabilities", nil)
	require.NoError(t, err)
	grantRequest.Header.Set("X-Api-Key", "")
	grantRequest.Header.Set(api.AgentSessionHeader, issued.Token)
	grantResponse, err := ts.Client().Do(grantRequest)
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, grantResponse.StatusCode)
	...
	require.NoError(t, json.UnmarshalRead(grantResponse.Body, &grantBody))
	require.NoError(t, grantResponse.Body.Close())
	require.Equal(t, "agent:"+issued.ID, grantBody.Session.SubjectID)
	require.Equal(t, []string{"read"}, grantBody.Session.Operations)
	require.Equal(t, []string{allowed.CurrentVersionID}, grantBody.Session.SourceIDs)
	require.Equal(t, uint64(1), grantBody.Session.GrantRevision)
	...
	require.Equal(t, http.StatusUnauthorized, read(issued.Token, "/api/v1/agent/capabilities"))

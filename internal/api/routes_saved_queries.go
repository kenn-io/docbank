...
func registerSavedQueryRunRoute(
	...
	huma.Register(api, huma.Operation{
		...
		run, page, err := snapshots.RunSaved(ctx, owner, in.SavedQueryID, revision, store.SnapshotRequest{
		...
		if err != nil {
			return nil, NewError(http.StatusInternalServerError, "internal", "could not encode saved query run")
		}

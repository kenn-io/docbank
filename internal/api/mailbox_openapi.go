...
const (
	mailboxHeaderLocation  = "header"
	mailboxIntegerType     = "integer"
	mailboxNDJSONMediaType = "application/x-ndjson"
)

// MailboxContainerInput omits the daemon-owned identity.
type MailboxContainerInput struct {
	ID     string `json:"id"`
...
type MailboxSettingsInput struct {
	...
	LabelTags     map[string]string `json:"label_tags,omitempty"`
}

type MailboxJobInput struct {
	ID              string               `json:"id"`
	ContainerID     string               `json:"container_id"`
	ContainerSHA256 string               `json:"container_sha256"`
	Settings        MailboxSettingsInput `json:"settings"`
}

func (r MailboxJobInput) storeRequest() store.MailboxJobRequest {
	return store.MailboxJobRequest{ID: r.ID, ContainerID: r.ContainerID, ContainerSHA256: r.ContainerSHA256, Settings: store.MailboxSettings(r.Settings)}
}

type MailboxResumeInput struct {
	Request      MailboxJobInput `json:"request"`
	Continuation bool            `json:"continuation"`
}

type MailboxArchiveInput struct {
	ID          string `json:"id"`
...
func registerMailboxOpenAPI(api huma.API) {
	...
	for _, r := range []struct {
		...
		{http.MethodPost, "/containers", "beginMailboxContainer", "Declare an immutable mailbox container", "201", reflect.TypeFor[MailboxContainerInput](), reflect.TypeFor[store.MailboxContainer]()},
		...
		{http.MethodPost, "/containers/{id}/preview", "previewMailboxContainer", "Preview a bounded sample with an explicit dialect", "200", reflect.TypeFor[previewRequest](), reflect.TypeFor[mailbox.Preview]()},
		{http.MethodPost, "/jobs", "beginMailboxJob", "Start an immutable resumable mailbox import", "201", reflect.TypeFor[MailboxJobInput](), reflect.TypeFor[store.MailboxJob]()},
		...
		{http.MethodPost, "/jobs/{id}/resume", "resumeMailboxJob", "Resume or explicitly continue the exact source and settings", "200", reflect.TypeFor[MailboxResumeInput](), reflect.TypeFor[store.MailboxJob]()},
		{http.MethodPost, "/archives", "registerMailboxArchive", "Register an application-independent EML archive", "201", reflect.TypeFor[MailboxArchiveInput](), reflect.TypeFor[store.MailboxArchive]()},
		...
		op := &huma.Operation{OperationID: r.id, Method: r.method, Path: "/api/v1/mailbox" + r.path, Summary: r.summary, Responses: map[string]*huma.Response{r.status: {Description: r.summary}, "default": {Description: "Mailbox request failed", Content: map[string]*huma.MediaType{emailDocumentJSONMediaType: {Schema: registry.Schema(reflect.TypeFor[Error](), true, "")}}}}}
		if r.output != nil {
			media := emailDocumentJSONMediaType
			if r.id == "mailboxEvents" {
				media = mailboxNDJSONMediaType
			}
			op.Responses[r.status].Content = map[string]*huma.MediaType{media: {Schema: registry.Schema(r.output, true, "")}}
		}
		if r.input != nil {
			op.RequestBody = &huma.RequestBody{Required: true, Description: "JSON body, at most 1 MiB. Owner is derived from authenticated identity.", Content: map[string]*huma.MediaType{emailDocumentJSONMediaType: {Schema: registry.Schema(r.input, true, "")}}}
		}
		...
		if r.id == "listMailboxJobs" || r.id == "mailboxOccurrences" {
			kind, format := openAPIStringType, ""
			if r.id == "mailboxOccurrences" {
				kind, format = mailboxIntegerType, "int64"
			}
			op.Parameters = append(op.Parameters, &huma.Param{Name: "after", In: openAPIQueryLocation, Schema: &huma.Schema{Type: kind, Format: format}}, &huma.Param{Name: "limit", In: openAPIQueryLocation, Description: "Page size, 1 through 100", Schema: &huma.Schema{Type: mailboxIntegerType}})
		}

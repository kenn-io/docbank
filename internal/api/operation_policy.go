...
type logOperationAudit struct{ logger *slog.Logger }

// NewLogOperationAudit writes bounded authorization records to the daemon's
// configured logger without credential, source identity, or content fields.
func NewLogOperationAudit(logger *slog.Logger) OperationAudit {
	if logger == nil {
		logger = slog.Default()
	}
	return logOperationAudit{logger: logger}
}

func (a logOperationAudit) RecordOperation(ctx context.Context, event OperationAuditEvent) {
	a.logger.LogAttrs(ctx, slog.LevelInfo, "operation_authorization",
		slog.String("actor", event.Actor),
		slog.String("operation", string(event.Operation)),
		slog.String("outcome", string(event.Outcome)),
		slog.Uint64("grant_revision", event.GrantRevision),
		slog.Int("source_count", event.SourceCount),
	)
}
...
// WithAuditIfAbsent preserves an explicitly configured audit sink while
// giving transport defaults a safe sink of their own.
func (p *OperationPolicy) WithAuditIfAbsent(audit OperationAudit) *OperationPolicy {
	if p == nil {
		return NewOperationPolicy(OperationPolicyOptions{Audit: audit})
	}
	if p.audit != nil {
		return p
	}
...
// AuthorizeSourceGrant rechecks a queued job against current independent grant
// authority. It deliberately uses only the persisted, credential-free binding.
func (p *OperationPolicy) AuthorizeSourceGrant(ctx context.Context, binding store.SourceGrantBinding) error {
	if binding.SourceID == "" {
		return ErrOperationDenied
	}
	_, err := p.Authorize(ctx, OperationAuthorizationRequest{
		Principal: Principal{
			SubjectID: binding.SubjectID, CredentialKind: binding.CredentialKind,
			Audience: binding.Audience, GrantRevision: binding.GrantRevision,
			ExpiresAt: binding.ExpiresAt, Operations: []Operation{OperationProcessing},
			SourceIDs: []string{binding.SourceID},
		},
		Operation: OperationProcessing, SourceIDs: []string{binding.SourceID},
		RequireAll: true, Protected: true,
	})
	return err
}

...
type AgentCapabilities struct {
	...
	Session  *AgentSessionProjection     `json:"session,omitempty"`
}

// AgentSessionProjection contains only the authenticated caller's live grant.
// It never includes the bearer token or daemon host paths.
...
func registerAgentCapabilitiesRoute(api huma.API, d Deps, registry *agentapi.Registry) {
	...
	huma.Register(api, huma.Operation{
		...
		body := AgentCapabilities{Contract: capabilities.Schema, Server: capabilities.Server}
		if !principal.Local {
			body.Session = &AgentSessionProjection{
				SubjectID: principal.SubjectID, CredentialKind: principal.CredentialKind,
				Audience: principal.Audience, Operations: append([]Operation(nil), principal.Operations...),
				SourceIDs: append([]string(nil), decision.SourceIDs...), GrantRevision: decision.GrantRevision,
		...
		return &output{CacheControl: "no-store", Body: body}, nil
	})
}
...
func availableAgentCapabilities(capabilities agentops.Capabilities, principal Principal) agentops.Capabilities {
	...
	for _, operation := range capabilities.Server.Operations {
		if !principal.Local && operation.ID == "list_documents" {
			// The operator and scoped catalog routes are alternatives. Publish
			// only the one this credential can enter.
			operation.RouteIDs = []string{"POST /api/v1/documents/scoped"}
		}

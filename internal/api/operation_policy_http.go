...
func scopedRequestAllowed(method, path string) bool {
	...
	case http.MethodGet:
		switch {
		case len(parts) == 2 && parts[0] == "formats" && parts[1] == "capabilities":
			return true
		case len(parts) == 2 && parts[0] == "agent" && parts[1] == "capabilities":
			return true
		case len(parts) == 2 && parts[0] == "processing" && parts[1] == "profiles":
			return true

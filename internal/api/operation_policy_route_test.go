...
func TestProcessingProfileDiscoveryRechecksScopedReadGrant(t *testing.T) {
	now := time.Date(2026, 9, 22, 12, 0, 0, 0, time.UTC)
	authority := &routeGrantAuthority{}
	var authenticated api.Principal
	...
	ts, _ := newTestServer(t, func(deps *api.Deps) {
		configure(deps)
		authenticated = routePrincipal(now)
		authority.set(authenticated)
		deps.OperationPolicy = api.NewOperationPolicy(api.OperationPolicyOptions{
			Authority: authority, Now: func() time.Time { return now },
		})
	...
	headers := policyRouteHeaders()
	response, body := get(t, ts, "/api/v1/processing/profiles", headers)
	require.Equal(t, http.StatusOK, response.StatusCode, body)

	authority.mutate(func(grant *api.Principal) { grant.Operations = nil })
	response, body = get(t, ts, "/api/v1/processing/profiles", headers)
	assert.Equal(t, http.StatusForbidden, response.StatusCode, body)
}

func TestScopedCredentialCannotBeMixedWithMasterCredential(t *testing.T) {
	now := time.Date(2026, 9, 22, 12, 0, 0, 0, time.UTC)
	var principal api.Principal
	ts, _ := newTestServer(t, func(deps *api.Deps) {
		principal = routePrincipal(now)
		deps.AuthenticatePrincipal = policyRouteAuthenticator(&principal)
	})
	for _, headers := range []map[string]string{
		{"X-Api-Key": testAPIKey, "Authorization": "Bearer synthetic-remote"},
		{"X-Api-Key": testAPIKey, "Authorization": "Bearer " + testAPIKey},
	} {
		resp, body := get(t, ts, "/api/v1/info", headers)
		assert.Equal(t, http.StatusUnauthorized, resp.StatusCode, body)
	}
}

...
func readMailboxJSON(w http.ResponseWriter, r *http.Request, out any) bool {
	...
	if err == nil {
		...
		if err != nil {
			err = errors.Join(store.ErrMailboxInvalid, err)
		}
	}
	if err != nil {
...
func mailboxError(err error) *Error {
	...
	case errors.Is(err, store.ErrMailboxLimit):
		code = "mailbox_limit"
	case errors.Is(err, store.ErrMailboxInvalid), errors.Is(err, mailbox.ErrRejected):
	default:
		problem, _ := errors.AsType[*Error](FromStoreError(err))
		return problem
	}
...
func registerMailboxRoutes(mux *http.ServeMux, api huma.API, d Deps, g *gate) {
	...
	mux.HandleFunc("POST /api/v1/mailbox/containers", func(w http.ResponseWriter, r *http.Request) {
		var input MailboxContainerInput
		if !readMailboxJSON(w, r, &input) {
			return
		}
		request := store.MailboxContainerRequest{ID: input.ID, Owner: mailboxOwner(d), SHA256: input.SHA256, Size: input.Size, Format: input.Format}
	...
	mux.HandleFunc("POST /api/v1/mailbox/jobs", func(w http.ResponseWriter, r *http.Request) {
		...
		err := g.mutate(func() error {
			var err error
			j, err = d.Store.BeginMailboxJob(r.Context(), mailboxOwner(d), request.storeRequest())
			return err
		})
		if err != nil {
			writeMailboxError(w, err)
	...
	mux.HandleFunc("POST /api/v1/mailbox/jobs/{id}/resume", func(w http.ResponseWriter, r *http.Request) {
		...
		err := g.mutate(func() error {
			var err error
			j, err = d.Store.ResumeMailboxJob(r.Context(), mailboxOwner(d), input.Request.storeRequest(), input.Continuation)
			return err
		})
		if err != nil {
			writeMailboxError(w, err)
...
func registerMailboxTransferRoutes(mux *http.ServeMux, d Deps, g *gate, service *mailbox.Service) {
	mux.HandleFunc("POST /api/v1/mailbox/archives", func(w http.ResponseWriter, r *http.Request) {
		var input MailboxArchiveInput
		if !readMailboxJSON(w, r, &input) {
			return
		}
		a := store.MailboxArchive{ID: input.ID, Owner: mailboxOwner(d), Description: input.Description}
	...
	mux.HandleFunc("POST /api/v1/mailbox/transfers", func(w http.ResponseWriter, r *http.Request) {
		...
		r.Body = http.MaxBytesReader(w, r.Body, request.Size)

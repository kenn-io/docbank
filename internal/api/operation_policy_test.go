...
func TestOperationPolicyRequireAllWithWholeGrant(t *testing.T) {
	now := time.Date(2026, 9, 22, 12, 0, 0, 0, time.UTC)
	principal := testPrincipal(now)
	policy := NewOperationPolicy(OperationPolicyOptions{
	...
	decision, err := policy.Authorize(t.Context(), OperationAuthorizationRequest{
		Principal: principal, Operation: OperationRead, RequireAll: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(decision.SourceIDs, principal.SourceIDs) {
		t.Fatalf("whole grant = %v", decision.SourceIDs)
	}
}

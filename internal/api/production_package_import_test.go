...
func syntheticRecipientArchive(t *testing.T) (string, production.RecipientManifest) {
	t.Helper()
	...
	jobID, setID, memberID := uuid.NewString(), uuid.NewString(), uuid.NewString()
	...
	reservation := documentproduction.NumberReservation{Contract: documentproduction.NumberReservationContractV1,
		...
		State: "reserved", Numbers: []documentproduction.AssignedNumber{{MemberID: memberID, MemberOrdinal: 1,
			Page: 1, Text: "OUT000041"}}}
	var err error
	...
	for _, part := range []struct {
		...
		artifact := documentproduction.Artifact{ID: uuid.NewString(), MemberID: memberID, MemberOrdinal: 1,
			Role: part.role, Page: part.page, Path: "private/" + privateCanary + "-" + part.role, SHA256: recipientImportHash(part.data),
			Size: int64(len(part.data)), MediaType: part.media}
	...
	projection, err := production.PlanPackageProjection(job, reservation,
...
func TestRecipientProductionPackageImportsIntoFreshVault(t *testing.T) {
	path, manifest := syntheticRecipientArchive(t)
	...
	require.Equal(t, 1, preflight.Records)
	require.Equal(t, 1, preflight.Pages)
	...
	require.Equal(t, manifest.Documents[0].PDFSHA256, members[0].BlobSHA256)
	...
	for _, representation := range members[0].Representations {
		...
		if representation.Role == "produced_pdf" && representation.Status == "available" &&
			representation.BlobSHA256 == manifest.Documents[0].PDFSHA256 {
			producedPDF = true
		}
	}
	...
	for _, query := range []struct {
		term string
		want int
	}{{"Retained", 1}, {"OMITTED-SYNTHETIC-CANARY", 0}} {

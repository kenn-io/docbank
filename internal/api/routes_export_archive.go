...
func registerExportArchiveRead(mux *http.ServeMux, api huma.API, d Deps) {
	huma.Register(api, huma.Operation{
		OperationID: "getExportArchiveAuthority", Method: http.MethodGet,
		Path:    "/api/v1/exports/jobs/{id}/archive/authority",
		...
		if d.Store == nil || d.Exports == nil {
			return nil, NewError(http.StatusServiceUnavailable, "export_unavailable", "export worker unavailable")
		}
		owner, err := exportOwner(ctx)
		if err != nil {
			return nil, exportProblem(err)
		}
		if !validPageJobPathID(in.ID) {
			return nil, NewError(http.StatusNotFound, "not_found", "not found")
		}
		job, err := checkExportArchiveRead(ctx, d.Store, owner, in.ID)
		if err != nil {
			return nil, archiveReadProblem(err)
		}
		return &struct {
			CacheControl string `header:"Cache-Control"`
			Body         bundle.Receipt
		}{CacheControl: "no-store", Body: *job.Receipt}, nil
	})

	...
	operations := []*huma.Operation{
		...
		{OperationID: "listPackageCustodians", Method: http.MethodGet, Path: "/api/v1/packages/by-id/{package_id}/custodians",
			Summary: "List active package custodian claims", Parameters: []*huma.Param{packageID,
				{Name: "row_id", In: openAPIQueryLocation, Schema: &huma.Schema{Type: openAPIStringType}},
...
func registerPackageCustodianRoutes(mux *http.ServeMux, d Deps, g *gate) {
	mux.HandleFunc("GET /api/v1/people", func(w http.ResponseWriter, r *http.Request) {
		...
		afterName, afterID, err := decodePeopleCursor(r.URL.Query().Get("cursor"))
		if err != nil {
			writeError(w, NewError(http.StatusUnprocessableEntity, "validation", "invalid people cursor"))
			return
		}
		people, err := d.Store.PeopleByDisplayName(r.Context(), r.URL.Query().Get("query"), afterName, afterID, limit+1)
		if err != nil {
		...
		if len(people) > limit {
			last := people[limit-1]
			out.NextCursor = encodePeopleCursor(last.DisplayNameFolded, last.PersonID)
		}
	...
	mux.HandleFunc("GET /api/v1/packages/by-id/{package_id}/custodians", func(w http.ResponseWriter, r *http.Request) {
		...
		offset, err := decodeCustodianCursor(r.URL.Query().Get("cursor"), packageID, scope.PackageRecordID)
		if err != nil {
		...
		if offset+len(assignments) < int(total) {
			out.NextCursor = encodeCustodianCursor(packageID, scope.PackageRecordID, offset+len(assignments))
		}
		writeJSON(w, http.StatusOK, out)
	})
...
func encodePeopleCursor(name, id string) string {
	return base64.RawURLEncoding.EncodeToString([]byte(name + "\x00" + id))
}

func decodePeopleCursor(cursor string) (string, string, error) {
	if cursor == "" {
		return "", "", nil
	}
...
func encodeCustodianCursor(packageID, rowID string, offset int) string {
	return base64.RawURLEncoding.EncodeToString([]byte(packageID + "\x00" + rowID + "\x00" + strconv.Itoa(offset)))
}

func decodeCustodianCursor(cursor, packageID, rowID string) (int, error) {
	if cursor == "" {
		return 0, nil
	}
	raw, err := base64.RawURLEncoding.DecodeString(cursor)
	parts := strings.Split(string(raw), "\x00")
	if err != nil || len(parts) != 3 || parts[0] != packageID || parts[1] != rowID {

...
func TestProcessingPlanRouteIsAuthenticatedAndReturnsReviewedDisclosure(t *testing.T) {
	...
	assert.Equal(t, []any{"byte_length", "content_hash", "detected_media_type", "synthetic_filename"},
...
func TestProcessingCoverageTracksTrashRestoreAndSupersessionEligibility(t *testing.T) {
	...
	assert.Equal(t, "ineligible", trashedCoverage.Renditions.State)
	assert.Equal(t, 1, trashedCoverage.Renditions.Ineligible)
	...
	assert.Equal(t, "ineligible", superseded.Renditions.State)
	assert.Equal(t, 1, superseded.Renditions.Ineligible)
...
func (provider *blockingProcessingProvider) Render(ctx context.Context, upload document.AuthorizedUpload,
	...
	result, err := provider.inner.Render(ctx, upload, authorization)
	if err != nil {

...
func TestPackageCustodianResolveRejectsOtherScopes(t *testing.T) {
	...
	require.NoError(t, err)
	hash, size, err := catalog.Blobs.Write(strings.NewReader("synthetic document"))
	require.NoError(t, err)
	node, err := catalog.IngestFileExact(t.Context(), run, catalog.RootID(), "ordinary.txt",
		hash, size, "text/plain", "ordinary.txt", "")
	require.NoError(t, err)
	original, err := catalog.CreatePerson(t.Context(), "Synthetic Original", "operator")
	require.NoError(t, err)
	replacement, err := catalog.CreatePerson(t.Context(), "Synthetic Replacement", "operator")
	require.NoError(t, err)

	for _, scope := range []store.CustodianScope{
		{Kind: "package", PackageID: pkg.PackageID},
		{Kind: "collection", IngestID: run.ID()},
		{Kind: "document", NodeID: node.ID, ContentVersionID: node.CurrentVersionID},
	} {
		t.Run(scope.Kind, func(t *testing.T) {
			assignment, err := catalog.SetCustodian(t.Context(), store.CustodianRequest{
				Scope: scope, PersonID: original.PersonID, RawLabel: "Synthetic claim",
				Rank: "primary", Basis: "operator_assigned", SourceRef: "synthetic", IfMatchRevision: 1,
			})
			require.NoError(t, err)
			response := srv.call(t, http.MethodPost,
				"/api/v1/packages/custodians/"+assignment.AssignmentID+"/resolve",
				mustPackageJSON(t, api.CustodianResolveRequest{
					PersonID: replacement.PersonID, IfMatchRevision: assignment.Revision,
				}), nil)
			after, _, err := catalog.Custodians(t.Context(), scope, false, 10, 0)
			require.NoError(t, err)
			require.Len(t, after, 1)
			require.NotNil(t, after[0].PersonID)
			if scope.Kind == "package" {
				assert.Equal(t, http.StatusOK, response.Code, response.Body.String())
				assert.Equal(t, replacement.PersonID, *after[0].PersonID)
				assert.Equal(t, assignment.Revision+1, after[0].Revision)
			} else {
				assert.Equal(t, http.StatusNotFound, response.Code, response.Body.String())
				assert.Equal(t, original.PersonID, *after[0].PersonID)
				assert.Equal(t, assignment.Revision, after[0].Revision)
			}
		})
	}
}
...
func TestPeopleCursorRejectsChangedQuery(t *testing.T) {
	...
	for _, query := range []string{"", "Ada"} {
		t.Run("query="+query, func(t *testing.T) {
			path := "/api/v1/people?query=" + url.QueryEscape(query)
			response := srv.get(t, path+"&limit=1")
			require.Equal(t, http.StatusOK, response.Code, response.Body.String())
			var first api.PersonPage
			require.NoError(t, json.Unmarshal(response.Body.Bytes(), &first))
			require.Len(t, first.Items, 1)
			require.Equal(t, "Ada One", first.Items[0].DisplayName)
			require.NotEmpty(t, first.NextCursor)
			response = srv.get(t, path+"&limit=10&cursor="+first.NextCursor)
			require.Equal(t, http.StatusOK, response.Code, response.Body.String())
			var remaining api.PersonPage
			require.NoError(t, json.Unmarshal(response.Body.Bytes(), &remaining))
			require.NotEmpty(t, remaining.Items)
			require.Equal(t, "Ada Two", remaining.Items[0].DisplayName)
			require.Empty(t, remaining.NextCursor)
			for _, changed := range []string{"", "Ada", "Beth"} {
				if changed == query {
					continue
				}
				response := srv.get(t, "/api/v1/people?query="+url.QueryEscape(changed)+"&cursor="+first.NextCursor)
				assert.Equal(t, http.StatusUnprocessableEntity, response.Code, "query=%q: %s", changed, response.Body.String())
			}
		})
	}
}
...
func TestPackageLabelCursorContinuesWithMaximumFilters(t *testing.T) {
	...
	received, err := catalog.PackageLabels(t.Context(), pkg.PackageID, occurrenceID)
	require.NoError(t, err)
	require.Len(t, received, 1)
	label, labelSet := strings.Repeat("\x01", 256), strings.Repeat("\x02", 256)
	assigned := make([]store.PackageLabelRow, 0, 2)
	...
	require.NoError(t, catalog.AssignPackageLabels(t.Context(), pkg.PackageID, occurrenceID,
		received[0].ContentVersionID, assigned))
	path := "/api/v1/packages/label-candidates?label=" + url.QueryEscape(label) +
		"&package_id=" + pkg.PackageID + "&label_set=" + url.QueryEscape(labelSet) + "&provenance=assigned"
	response := srv.get(t, path+"&limit=1")
	require.Equal(t, http.StatusOK, response.Code, response.Body.String())
	var first api.PackageLabelCandidatePage
	require.NoError(t, json.Unmarshal(response.Body.Bytes(), &first))
	require.Len(t, first.Items, 1)
	require.Equal(t, "begin", first.Items[0].Endpoint)
	require.NotEmpty(t, first.NextCursor)
	response = srv.get(t, path+"&limit=10&cursor="+first.NextCursor)
	require.Equal(t, http.StatusOK, response.Code, "cursor bytes=%d: %s", len(first.NextCursor), response.Body.String())
	var last api.PackageLabelCandidatePage
	require.NoError(t, json.Unmarshal(response.Body.Bytes(), &last))
	require.Len(t, last.Items, 1)
	require.Equal(t, "end", last.Items[0].Endpoint)
...
func TestPackageCustodianCursorRejectsChangedFilters(t *testing.T) {
	...
	require.NoError(t, err)
	for _, label := range []string{"Owner A", "Owner B", "Owner C"} {
		personID := ""
		if label == "Owner C" {
		...
		_, err := catalog.SetCustodian(t.Context(), store.CustodianRequest{
			Scope:    store.CustodianScope{Kind: "package", PackageID: pkg.PackageID},
			PersonID: personID, RawLabel: label, Rank: "additional", Basis: "package_column",
			SourceRef: "CUSTODIAN", IfMatchRevision: 1,
		})
		require.NoError(t, err)
	}
	base := "/api/v1/packages/by-id/" + pkg.PackageID + "/custodians"
	for _, unresolved := range []string{"false", "true"} {
		t.Run("unresolved="+unresolved, func(t *testing.T) {
			path := base + "?unresolved_only=" + unresolved
			response := srv.get(t, path+"&limit=1")
			require.Equal(t, http.StatusOK, response.Code, response.Body.String())
			var first api.CustodianPage
			require.NoError(t, json.Unmarshal(response.Body.Bytes(), &first))
			require.Len(t, first.Items, 1)
			require.NotEmpty(t, first.NextCursor)
			response = srv.get(t, path+"&limit=10&cursor="+first.NextCursor)
			require.Equal(t, http.StatusOK, response.Code, response.Body.String())
			var remaining api.CustodianPage
			require.NoError(t, json.Unmarshal(response.Body.Bytes(), &remaining))
			labels := []string{first.Items[0].RawLabel}
			for _, item := range remaining.Items {
				labels = append(labels, item.RawLabel)
			}
			want := []string{"Owner A", "Owner B"}
			if unresolved == "false" {
				want = append(want, "Owner C")
			}
			require.ElementsMatch(t, want, labels)
			require.Empty(t, remaining.NextCursor)
			changed := "false"
			if unresolved == "false" {
				changed = "true"
			}
			response = srv.get(t, base+"?unresolved_only="+changed+"&cursor="+first.NextCursor)
			assert.Equal(t, http.StatusUnprocessableEntity, response.Code, response.Body.String())
			response = srv.get(t, path+"&row_id="+rowID+"&cursor="+first.NextCursor)
			assert.Equal(t, http.StatusUnprocessableEntity, response.Code, response.Body.String())
		})
	}
}

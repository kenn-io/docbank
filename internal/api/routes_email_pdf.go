...
func registerEmailPDFRoutes(mux *http.ServeMux, api huma.API, d Deps, g *gate) {
	...
	mux.HandleFunc("POST /api/v1/email-pdfs", func(w http.ResponseWriter, r *http.Request) {
		if d.RequestEmailPDF == nil {
			reason := d.EmailPDFUnavailableReason
			if reason == "" {
				reason = "Configure [email_pdf] with a pinned Chromium bundle and fonts on a Linux daemon with a systemd user manager, bubblewrap, and delegated cgroup v2 memory/pids controllers."
			}
			writeError(w, NewError(http.StatusServiceUnavailable, "email_pdf_unavailable", reason))
			return
		}
	...
	profileParam := &huma.Param{Name: "profile", In: openAPIPathLocation, Required: true, Schema: &huma.Schema{Type: openAPIStringType}}
	jobParam := &huma.Param{Name: "job_id", In: openAPIPathLocation, Required: true, Schema: &huma.Schema{Type: openAPIStringType}}
	api.OpenAPI().AddOperation(&huma.Operation{OperationID: "renderEmailPDF", Method: "POST", Path: "/api/v1/email-pdfs", Summary: "Render one exact email version as a retained verified PDF", RequestBody: &huma.RequestBody{Required: true, Content: map[string]*huma.MediaType{emailDocumentJSONMediaType: {Schema: registry.Schema(reflect.TypeFor[document.EmailPDFRequest](), true, "")}}}, Responses: map[string]*huma.Response{"200": {Description: "Existing rendition job and exact retained receipt", Content: map[string]*huma.MediaType{emailDocumentJSONMediaType: {Schema: registry.Schema(reflect.TypeFor[document.EmailPDFJob](), true, "")}}}}})
	for _, op := range []struct {
		id, path, summary string
		result            reflect.Type
		params            []*huma.Param
	}{
		{"listEmailPDFs", "/api/v1/email-pdfs/{version_id}", "List verified retained PDFs for one original email version", reflect.TypeFor[[]document.EmailPDFReceiptV1](), []*huma.Param{versionParam}},
		{"getEmailPDF", "/api/v1/email-pdfs/{version_id}/{profile}", "Read an exact retained PDF receipt", reflect.TypeFor[document.EmailPDFReceiptV1](), []*huma.Param{versionParam, profileParam}},
		{"getEmailPDFJob", "/api/v1/email-pdf-jobs/{job_id}", "Read retained PDF job state", reflect.TypeFor[emailPDFJobState](), []*huma.Param{jobParam}},
	} {
		api.OpenAPI().AddOperation(&huma.Operation{OperationID: op.id, Method: "GET", Path: op.path, Summary: op.summary, Parameters: op.params, Responses: map[string]*huma.Response{"200": {Description: op.summary, Content: map[string]*huma.MediaType{emailDocumentJSONMediaType: {Schema: registry.Schema(op.result, true, "")}}}}})
	}
	api.OpenAPI().AddOperation(&huma.Operation{OperationID: "downloadEmailPDF", Parameters: []*huma.Param{versionParam, profileParam}, Method: "GET", Path: "/api/v1/email-pdfs/{version_id}/{profile}/content", Summary: "Download independently verified retained PDF bytes", Responses: map[string]*huma.Response{"200": {Description: "Exact retained PDF", Content: map[string]*huma.MediaType{"application/pdf": {}}}}})
}

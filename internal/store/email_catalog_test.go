...
func TestEmailSupersededBeforeBodyPublication(t *testing.T) {
	...
	next := f.publication
	next.CanonicalJSON, _, err = document.MarshalEmailV1(changed)
	require.NoError(t, err)
	selected, err := s.PublishEmailGeneration(t.Context(), next)
	require.NoError(t, err)
	require.Equal(t, "pending", selected.BodySearch.State)
	historical, err := s.EmailMetadataGeneration(t.Context(), first.Version.ID, first.Generation.ID)
	require.NoError(t, err)
	require.Equal(t, EmailBodySearch{State: "unavailable", Reason: new("superseded")}, historical.BodySearch)
	reselected, err := s.PublishEmailGeneration(t.Context(), f.publication)
	require.NoError(t, err)

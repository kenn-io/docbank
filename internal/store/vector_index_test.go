...
func TestVectorIndexReaderLeasePinsPriorGenerationAndExpiresWithFence(t *testing.T) {
	...
	require.NoError(t, putVectorIndexGenerationForTest(t, s, first, true))
	lease, err := s.AcquireVectorIndexGeneration(t.Context(), space, "reader", now, time.Minute)
	require.NoError(t, err)
	require.NoError(t, putVectorIndexGenerationForTest(t, s, second, true))
...
func TestVectorIndexProjectionStateIsExcludedFromPortableMetadata(t *testing.T) {
	...
	require.NoError(t, putVectorIndexGenerationForTest(t, s, record, true))
...
func putVectorIndexGenerationForTest(t *testing.T, s *Store, record VectorIndexGenerationRecord, active bool) error {
	t.Helper()

...
const (
	PageJobCompleted = "completed"
	PageJobFailed    = "failed"
	PageJobRunning   = "running"
)
...
func validatePageJob(j PageRenderJob) error {
	...
	switch j.State {
	case "queued", PageJobRunning, PageJobCompleted, "canceled", PageJobFailed:
	default:
		return ErrPageConflict
	}
	...
	if (j.State == PageJobFailed) != (j.FailureCode != "") {
		return ErrPageConflict
	}
	...
	if total > document.MaxPageJobBytes || j.State == PageJobCompleted && len(j.Results) != len(j.Request.Pages) {
		return ErrPageConflict
	}
	return nil
}

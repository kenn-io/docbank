...
func (s *Store) EnsureOrdinaryBlobTx(
	...
	if _, err := tx.Exec(`DELETE FROM derivative_pack_purge_pending WHERE blob_hash=?`, hash); err != nil {
		return fmt.Errorf("revoking derivative pack purge for ordinary blob %s: %w", hash, err)
	}
	return nil
}

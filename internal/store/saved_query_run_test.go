...
func TestSavedQueryRunRechecksPreserveResolverErrors(t *testing.T) {
	...
	dependencies := []query.Dependency{{Kind: query.ReferenceSaved, ID: definition.ID, Revision: definition.Revision}}
	...
	cancel()
	_, err = insertSavedQueryRunTx(canceled, tx, definition.ID, definition.Revision, SnapshotProjection{}, SnapshotPage{})
	require.ErrorIs(t, err, context.Canceled)
	require.ErrorIs(t, recheckSavedQueryRunDependencies(canceled, tx, dependencies), context.Canceled)

	_, err = tx.ExecContext(t.Context(), `DELETE FROM saved_queries WHERE id=?`, definition.ID)
	require.NoError(t, err)
	_, err = insertSavedQueryRunTx(t.Context(), tx, definition.ID, definition.Revision, SnapshotProjection{}, SnapshotPage{})
	require.ErrorIs(t, err, ErrStaleRevision)
	require.ErrorIs(t, recheckSavedQueryRunDependencies(t.Context(), tx, dependencies), ErrStaleRevision)

	require.NoError(t, tx.Rollback())
	_, err = insertSavedQueryRunTx(t.Context(), tx, definition.ID, definition.Revision, SnapshotProjection{}, SnapshotPage{})
	require.ErrorIs(t, err, sql.ErrTxDone)
	require.ErrorIs(t, recheckSavedQueryRunDependencies(t.Context(), tx, dependencies), sql.ErrTxDone)
}
...
func TestSavedQueryRunMaterializesBeforeEnteringMutationGate(t *testing.T) {
	...
	service := newQuerySnapshotService(s, querySnapshotServiceOptions{
		ChargeHook: func(_ context.Context, _ string, rows, _ int64) error {
			materializedRows += rows
			return nil
		},
	})
	t.Cleanup(func() { require.NoError(t, service.Close()) })
	_, page, err := service.RunSaved(t.Context(), "owner", definition.ID, definition.Revision, SnapshotRequest{},
		func(write func() error) error {
			assert.Equal(t, int64(1), materializedRows, "materialization must finish before mutation admission")
			runs, err := s.ListSavedQueryRuns(t.Context(), definition.ID, 100)
			require.NoError(t, err)
			assert.Empty(t, runs, "receipt must not be written before mutation admission")
			return write()
		})
	require.NoError(t, err)
	...
	for _, driverCase := range walkTestDrivers() {
		t.Run(driverCase.name, func(t *testing.T) {
			...
			first, firstPage, err := service.RunSaved(t.Context(), "owner", definition.ID, definition.Revision,
				SnapshotRequest{Query: snapshotTestQuery(t, `{"text":"must-not-override"}`)}, nil)
			require.NoError(t, err)
			...
			require.NoError(t, err)
			second, _, err := service.RunSaved(t.Context(), "owner", definition.ID, definition.Revision, SnapshotRequest{}, nil)
			require.NoError(t, err)
			...
			require.NoError(t, err)
			third, _, err := service.RunSaved(t.Context(), "owner", updated.ID, updated.Revision, SnapshotRequest{}, nil)
			require.NoError(t, err)
		...
		t.Run(test.name, func(t *testing.T) {
			...
			go func() {
				_, _, runErr := service.RunSaved(context.Background(), "owner", definition.ID,
					definition.Revision, SnapshotRequest{}, nil)
				result <- runErr
			}()
			<-entered
			mutate(t, s)
...
func TestSavedQueryRunCanceledAdmissionAndDefinitionDeletionLeaveNoAuthority(t *testing.T) {
	...
	cancel()
	_, _, err = service.RunSaved(canceled, "owner", definition.ID, definition.Revision, SnapshotRequest{}, nil)
	require.ErrorIs(t, err, context.Canceled)
	runs, err := s.ListSavedQueryRuns(t.Context(), definition.ID, 10)
	require.NoError(t, err)
	assert.Empty(t, runs)

	run, _, err := service.RunSaved(t.Context(), "owner", definition.ID, definition.Revision, SnapshotRequest{}, nil)
	require.NoError(t, err)

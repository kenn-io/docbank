...
func TestBatesAndProductionAuthoritySurviveMetadataRestoreTogether(t *testing.T) {
	...
	require.NoError(t, err)
	allocation, err := s.ReserveBatesRange(t.Context(), batesRequest(t, namespace, snapshot, inputs))
	require.NoError(t, err)
	...
	restoredPolicy, err := restored.ProductionPolicy(t.Context(), policy.ID, policy.Version)
	require.NoError(t, err)
	require.Equal(t, policy.ID, restoredPolicy.ID)
	require.Equal(t, policy.Version, restoredPolicy.Version)
	require.Equal(t, policy.SHA256, restoredPolicy.SHA256)
	restoredAllocation, err := restored.BatesAllocation(t.Context(), allocation.AllocationID)
	require.NoError(t, err)

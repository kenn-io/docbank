...
-- commits it. Ordinary unreachable blobs remain backup authority so the
-- pruned-content regret window survives backup and restore.
...
-- Packed erasure targets outlive logical blob membership. The source hash is
-- retained without a foreign key so a crash after blob-row deletion cannot
-- erase the exact store/pack work receipt.
CREATE TABLE IF NOT EXISTS derivative_pack_purge_pending (
    blob_hash TEXT NOT NULL,
    store_id  TEXT NOT NULL REFERENCES blob_stores(store_id),
    pack_id   TEXT NOT NULL,
    PRIMARY KEY (blob_hash, store_id, pack_id),
    FOREIGN KEY (store_id, pack_id)
        REFERENCES blob_packs(store_id, pack_id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS derivative_pack_purge_target
    ON derivative_pack_purge_pending(store_id, pack_id);
...
AFTER INSERT ON content_versions BEGIN
    ...
    DELETE FROM derivative_pack_purge_pending WHERE blob_hash=NEW.blob_hash;
END;

...
func TestMailboxLabelTagsRemainAvailableWhileJobIsActive(t *testing.T) {
	for _, mode := range []string{"ordinary", "audited"} {
		t.Run(mode, func(t *testing.T) {
			...
			c := MailboxContainerRequest{ID: "source", Owner: "one", SHA256: fakeHash("aa"), Size: 3, Format: "mbox"}
			_, err := s.BeginMailboxContainer(ctx, c)
			require.NoError(t, err)
			require.NoError(t, s.RecordRenditionBlob(ctx, c.SHA256, c.Size, BlobPhysical{Encoding: "raw", StoredBytes: c.Size, PackEligible: true, Created: true}))
			require.NoError(t, s.PutMailboxChunk(ctx, c.Owner, c.ID, MailboxChunk{Index: 0, SHA256: c.SHA256, Size: c.Size}))
			_, err = s.SealMailboxContainer(ctx, c.Owner, c.ID, c.SHA256, c.Size)
			require.NoError(t, err)
			request := MailboxJobRequest{ID: "job", ContainerID: c.ID, ContainerSHA256: c.SHA256, Settings: MailboxSettings{
				DestinationID: s.RootID(), LabelTags: map[string]string{"Project": "00000000-0000-4000-8000-000000000001"},
			}}
			_, err = s.BeginMailboxJob(ctx, c.Owner, request)
			require.ErrorIs(t, err, ErrNotFound)
			_, err = s.MailboxJob(ctx, c.Owner, request.ID)
			require.ErrorIs(t, err, ErrNotFound)

			tag, err := s.CreateTag(ctx, "Project")
			require.NoError(t, err)
			request.Settings.LabelTags["Project"] = tag.ID
			_, err = s.BeginMailboxJob(ctx, c.Owner, request)
			require.NoError(t, err)
			...
			_, err = s.DeleteTag(ctx, tag.ID, tag.Revision)
			require.ErrorIs(t, err, ErrMailboxConflict)
			unrelated, err := s.CreateTag(ctx, "Unrelated")
			require.NoError(t, err)
			_, err = s.DeleteTag(ctx, unrelated.ID, unrelated.Revision)
			require.NoError(t, err)

			require.NoError(t, s.FinishMailboxJob(ctx, job.ID, job.Claim, "partial", "segment limit", false))
			_, err = s.DeleteTag(ctx, tag.ID, tag.Revision)
			require.NoError(t, err)
			_, err = s.ResumeMailboxJob(ctx, c.Owner, request, true)
			require.ErrorIs(t, err, ErrNotFound)
			job, err = s.MailboxJob(ctx, c.Owner, request.ID)
			require.NoError(t, err)
			require.Equal(t, "partial", job.State)
		})
	}
}

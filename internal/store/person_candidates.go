...
func normalizePersonCandidate(candidate PersonMatchCandidate) (PersonMatchCandidate, []PersonCandidateOccurrence, error) {
	...
	if len(candidate.Evidence) == 0 || len(candidate.Evidence) > document.MaxPersonCandidateEvidence {
	...
	for _, occurrence := range occurrences {
		if validateUUIDv4(occurrence.ContentVersionID) != nil ||
			!slices.Contains(document.PersonRoles(), document.PersonRole(occurrence.Role)) ||
	...
	if len(raw) > document.MaxPersonCandidateEvidence {
...
func (s *Store) OpenPersonCandidate(
	...
	if openCount >= document.MaxOpenPersonCandidates {
...
func (s *Store) DecidePersonCandidate(ctx context.Context, decision CandidateDecision) (PersonMatchCandidate, error) {
	...
	err := s.withLogicalTx(ctx, func(tx *sql.Tx) error {
		...
		affectedVersions := make([]string, 0, len(occurrences))
		for _, occurrence := range occurrences {
			affectedVersions = append(affectedVersions, occurrence.ContentVersionID)
		}
		personID := ""
		...
		if personID != "" {
			_, err = materializeCandidateAssertionsTx(ctx, tx, personID, occurrences)
			if err != nil {
				return err
			}
		}
		...
		if err := markPersonAssertionVersionsDirtyTx(ctx, tx, affectedVersions); err != nil {
			return err
		}
...
func (s *Store) AssertDocumentPerson(ctx context.Context, assertion PersonDocumentAssertion) (PersonDocumentAssertion, error) {
	...
	err := s.withLogicalTx(ctx, func(tx *sql.Tx) error {
		...
		if err := markPersonAssertionVersionsDirtyTx(ctx, tx, []string{assertion.ContentVersionID}); err != nil {
			return err
		}
...
func validatePersonAssertion(assertion PersonDocumentAssertion) error {
	...
	if validateUUIDv4(assertion.ContentVersionID) != nil || validateUUIDv4(assertion.PersonID) != nil || assertion.Revision < 1 ||
		!slices.Contains(document.PersonRoles(), document.PersonRole(assertion.Role)) ||
		!slices.Contains([]string{"assert", "suppress"}, assertion.Action) ||
		len(assertion.Note) > document.MaxPersonAssertionNoteBytes || !utf8.ValidString(assertion.Note) {
		return ErrInvalidPerson
	}
	return nil
}
...
func materializeCandidateAssertionsTx(
	...
	versions := make([]string, 0, len(occurrences))
	...
	for _, occurrence := range occurrences {
		...
		if err == nil {
			if existing.Action != "assert" {
				return nil, ErrPersonIdentityConflict
			}
			continue
		}
		if !errors.Is(err, sql.ErrNoRows) {
		...
		versions = append(versions, occurrence.ContentVersionID)
	}
	return versions, nil
}

func markPersonAssertionVersionsDirtyTx(ctx context.Context, tx *sql.Tx, versions []string) error {
	versions = slices.Compact(slices.Sorted(slices.Values(versions)))
	for _, version := range versions {
		_, err := tx.ExecContext(ctx, `INSERT INTO document_people_dirty(content_version_id,reason,marked_at)
			SELECT ?,'person_assertion_changed',? WHERE EXISTS(
				SELECT 1 FROM content_versions WHERE version_id=?
			) ON CONFLICT(content_version_id) DO UPDATE SET
				revision=revision+1,reason=excluded.reason,marked_at=excluded.marked_at`,
			version, nowRFC3339(), version)
		if err != nil {
			return err
		}
	}

...
func TestRefreshContentMapPreservesHistoryAndRejectsStaleBase(t *testing.T) {
	s := newTestStore(t)
	ctx := t.Context()
	access := MapAccess{Owner: "local", AllSources: true}
	node, err := s.CreateFile(ctx, s.RootID(), "source.txt", fakeHash("refresh-v1"), 12, "text/plain")
	require.NoError(t, err)
	identity, err := s.EnsureDocumentIdentity(ctx, node.ID)
	require.NoError(t, err)
	definition := mapTestDefinition(document.ContentMapPin{DocumentUID: identity.DocumentUID, Mode: document.MapPinFollowCurrent})
	...
	created, err := s.CreateContentMap(ctx, access, definition, plan.DefinitionDigest)
	require.NoError(t, err)
	first, err := s.CreateContentMapSnapshot(ctx, access, created.ID, created.Revision)
	require.NoError(t, err)
	_, replacement, err := s.ReplaceContent(ctx, node.ID, node.Revision, fakeHash("refresh-v2"), 14, "text/plain")
	require.NoError(t, err)
	refreshed, err := s.RefreshContentMap(ctx, access, created.ID, created.Revision, first.ID)
	require.NoError(t, err)
	assert.NotEqual(t, first.ID, refreshed.Snapshot.ID)
	assert.Equal(t, first.ID, refreshed.Delta.BeforeSnapshotID)
	assert.Equal(t, refreshed.Snapshot.ID, refreshed.Delta.AfterSnapshotID)
	assert.Equal(t, 1, refreshed.Delta.Counts.VersionChanged)
	assert.Equal(t, replacement.ID, refreshed.Snapshot.Sections[0].Entries[0].Member.ContentVersionID)
	storedFirst, err := s.ContentMapSnapshotByID(ctx, access, first.ID)
	require.NoError(t, err)
	assert.Equal(t, first, storedFirst)
	_, err = s.RefreshContentMap(ctx, access, created.ID, created.Revision, first.ID)
	require.ErrorIs(t, err, ErrStaleRevision)
	_, err = s.RefreshContentMap(ctx, access, created.ID, created.Revision+1, refreshed.Snapshot.ID)
	require.ErrorIs(t, err, ErrStaleRevision)
}

func TestProposeContentMapFromExistingTagIsDeterministicAndReadOnly(t *testing.T) {
	s := newTestStore(t)
	ctx := t.Context()
	access := MapAccess{Owner: "local", AllSources: true}
	tag, err := s.CreateTag(ctx, "Research")
	require.NoError(t, err)
	first, err := s.ProposeContentMap(ctx, access, ContentMapProposalRequest{TagID: tag.ID})
	require.NoError(t, err)
	second, err := s.ProposeContentMap(ctx, access, ContentMapProposalRequest{TagID: tag.ID})
	require.NoError(t, err)
	assert.Equal(t, first, second)
	assert.Equal(t, "Research", first.Definition.Title)
	require.Len(t, first.Definition.Sections, 1)
	assert.Equal(t, []string{tag.ID}, first.Definition.Sections[0].Selector.Filters.TagIDs)
	_, err = s.ContentMapByID(ctx, access, tag.ID)
	require.ErrorIs(t, err, ErrNotFound, "a proposal must not save a map")
	_, err = s.ProposeContentMap(ctx, access, ContentMapProposalRequest{TagID: "missing"})
	require.ErrorIs(t, err, ErrNotFound)
	_, err = s.ProposeContentMap(ctx, access, ContentMapProposalRequest{Title: "Broad", Query: &query.Query{
		V: 1, Syntax: "simple", Mode: "lexical", Sort: query.Sort{Field: "name", Direction: "asc"}}})
	require.ErrorIs(t, err, ErrInvalidContentMap, "empty queries must not silently propose a vault-wide map")
	queryPlan, err := s.ProposeContentMap(ctx, access, ContentMapProposalRequest{Title: "Tagged research", Query: &query.Query{
		V: 1, Syntax: "simple", Mode: "lexical", Filters: query.Filters{TagIDs: []string{tag.ID}},
		Sort: query.Sort{Field: "name", Direction: "asc"}}})
	require.NoError(t, err)
	assert.Equal(t, "Tagged research", queryPlan.Definition.Title)
	assert.Equal(t, []string{tag.ID}, queryPlan.Definition.Sections[0].Selector.Filters.TagIDs)
}

func TestTagProposedMapRefreshTracksAssignmentsWithoutConceptAuthority(t *testing.T) {
	s := newTestStore(t)
	ctx := t.Context()
	access := MapAccess{Owner: "local", AllSources: true}
	tag, err := s.CreateTag(ctx, "Research")
	require.NoError(t, err)
	node, err := s.CreateFile(ctx, s.RootID(), "source.txt", fakeHash("tag-proposal"), 12, "text/plain")
	require.NoError(t, err)
	plan, err := s.ProposeContentMap(ctx, access, ContentMapProposalRequest{TagID: tag.ID})
	require.NoError(t, err)
	created, err := s.CreateContentMap(ctx, access, plan.Definition, plan.DefinitionDigest)
	require.NoError(t, err)
	first, err := s.CreateContentMapSnapshot(ctx, access, created.ID, created.Revision)
	require.NoError(t, err)
	require.Empty(t, first.Sections[0].Entries)
	_, err = s.AssignTag(ctx, tag.ID, node.ID, node.Revision)
	require.NoError(t, err)
	_, err = s.ContentMapSnapshotByID(ctx, access, first.ID)
	require.NoError(t, err, "the prior snapshot must remain authorized after a tag assignment")
	projection, err := s.materializeQuerySnapshot(ctx, SnapshotRequest{Query: *plan.Definition.Sections[0].Selector},
		snapshotMaterializeOptions{MaxRows: MaxSearchSourceFenceIDs})
	require.NoError(t, err)
	require.Len(t, projection.Rows, 1)
	_, err = s.EnsureDocumentIdentity(ctx, projection.Rows[0].NodeID)
	require.NoError(t, err)
	added, err := s.RefreshContentMap(ctx, access, created.ID, created.Revision, first.ID)
	require.NoError(t, err)
	require.Equal(t, 1, added.Delta.Counts.Added)
	require.Len(t, added.Snapshot.Sections[0].Entries, 1)
	assigned, err := s.NodeByID(ctx, node.ID)
	require.NoError(t, err)
	_, err = s.UnassignTag(ctx, tag.ID, node.ID, assigned.Revision)
	require.NoError(t, err)
	removed, err := s.RefreshContentMap(ctx, access, created.ID, created.Revision, added.Snapshot.ID)
	require.NoError(t, err)
	assert.Equal(t, 1, removed.Delta.Counts.Removed)
	assert.Empty(t, removed.Snapshot.Sections[0].Entries)
	storedAdded, err := s.ContentMapSnapshotByID(ctx, access, added.Snapshot.ID)
	require.NoError(t, err)
	assert.Equal(t, added.Snapshot, storedAdded)
}

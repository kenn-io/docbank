...
// Adding document identity authority after the existing version-24 layout
// must not reuse that layout's version number. Otherwise a version-24 database
// can be mistaken for the new current schema.
...
func TestOpenRejectsUnreleasedSchemaWithoutCutover(t *testing.T) {
	for _, test := range v090UpgradeDrivers() {
		t.Run(test.name, func(t *testing.T) {
			...
			require.ErrorContains(t, err, "has no supported JSONL cutover")
		})
	}
}

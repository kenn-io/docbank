...
// ClaimEmbeddingWork claims one exact eligible embedding job without
// consuming unrelated queued work.
...
func (s *Store) claimEmbeddingWork(ctx context.Context, jobID, owner string, at time.Time,
	...
	err := s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		args = append(args, at.UTC().Format(timestampLayout), at.UTC().Format(timestampLayout))
		targetSQL := ""
		if jobID != "" {
			targetSQL = " AND j.job_id=?"
		...
		err := tx.QueryRowContext(ctx, `SELECT j.job_id FROM embedding_jobs j
			...
			AND j.state IN ('queued','retry_wait','running') AND j.available_at<=?
			  ...
			  AND NOT EXISTS(SELECT 1 FROM current_rendition_roots r WHERE r.root_id=j.job_id
			    AND r.active=1 AND r.expires_at>?)`+targetSQL+`
			ORDER BY j.available_at,j.job_id LIMIT 1`, args...).Scan(&selectedJobID)
		...
		if err := tx.QueryRowContext(ctx, `SELECT 1+max(j.claim_epoch,
			...
			FROM embedding_jobs j WHERE job_id=?`, selectedJobID).Scan(&epoch); err != nil {
			return err
		}
		...
		if _, err := tx.ExecContext(ctx, `UPDATE embedding_jobs SET state='running',claim_count=claim_count+1,claim_owner=?,claim_epoch=?,lease_expires_at=?,updated_at=? WHERE job_id=?`,
			owner, epoch, expires.Format(timestampLayout), at.UTC().Format(timestampLayout), selectedJobID); err != nil {
			return err
		}
		work, err = loadEmbeddingJobWorkTx(ctx, tx, s.vaultID, selectedJobID)
		if err != nil {
			return err
		}
		root := CurrentRenditionRoot{ID: selectedJobID, Kind: RenditionRootWorkerLease,
		...
		claim = EmbeddingJobClaim{AttemptID: selectedJobID, Owner: owner, Epoch: epoch, LeaseExpiresAt: expires}
		found = true
		return nil
	})

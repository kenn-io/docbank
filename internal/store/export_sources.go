...
const exportFailedState = "failed"
...
func (s *Store) PutExportChunk(ctx context.Context, owner, id string, index int, members []bundle.Member) error {
	...
	return s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		if source.Kind != "upload" || index < 0 || index >= (source.Total+bundle.ChunkMembers-1)/bundle.ChunkMembers {
			return bundle.ErrConflict
		}
		var prior []byte
...
func (s *Store) SealExportSource(ctx context.Context, owner, id string) (bundle.Source, error) {
	...
	err := s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		for i := range (source.Total + bundle.ChunkMembers - 1) / bundle.ChunkMembers {
			var raw []byte
			...
			want := min(bundle.ChunkMembers, source.Total-i*bundle.ChunkMembers)
			if len(chunk) != want {
				return bundle.ErrConflict
			}

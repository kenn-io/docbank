...
func TestHydrateEmbeddingInputGenerationRequiresExactCanonicalArtifact(t *testing.T) {
	...
	hydrated, err := HydrateEmbeddingInputGeneration(projection, artifact)
	require.NoError(t, err)
	assert.Equal(t, record.InputGeneration, hydrated)

	corrupt := append([]byte(nil), artifact...)
	corrupt[len(corrupt)-1] ^= 1
	_, err = HydrateEmbeddingInputGeneration(projection, corrupt)
...
func TestEmbeddingCatalogLeasedStageAndPublishAreAtomicallyFenced(t *testing.T) {
	...
	_, err = s.ActiveEmbeddingHead(t.Context(), head.Key)
	require.ErrorIs(t, err, ErrNotFound)

	_, err = s.PublishEmbeddingHeadWithLease(t.Context(), head, consent, prior,
		root.ID, root.FencingToken, at)
	require.NoError(t, err)
	active, err := s.ActiveEmbeddingHead(t.Context(), head.Key)
	require.NoError(t, err)
	assert.Equal(t, record.ID, active.Record.SetID)
...
func TestEmbeddingJobCatalogClaimsRetriesAndResumesDurably(t *testing.T) {
	...
	at := time.Date(2026, 8, 26, 12, 0, 0, 0, time.UTC)
	claim, work, found, err := s.ClaimNextEmbeddingWork(t.Context(), "worker-a", at, 5*time.Minute)
	require.NoError(t, err)
	require.True(t, found)
	assert.Equal(t, job.ID, claim.AttemptID)
	assert.Equal(t, record.InputGeneration.ID, work.InputGeneration.ID)
	_, _, found, err = s.ClaimNextEmbeddingWork(t.Context(), "worker-b", at, 5*time.Minute)
	require.NoError(t, err)
	...
	_, _, found, err = s.ClaimNextEmbeddingWork(t.Context(), "worker-b", at.Add(time.Minute), 5*time.Minute)
	require.NoError(t, err)
	assert.False(t, found, "durable retry delay must prevent a hot loop")
	resumed, _, found, err := s.ClaimNextEmbeddingWork(t.Context(), "worker-b", at.Add(2*time.Minute), 5*time.Minute)
	require.NoError(t, err)
	require.True(t, found)
	...
	_, _, found, err = restored.ClaimNextEmbeddingWork(t.Context(), "worker-c", at.Add(6*time.Minute), 5*time.Minute)
	require.NoError(t, err)
...
func TestEmbeddingJobsRebuildFromPortableAuthorityAfterMetadataRestore(t *testing.T) {
	...
	require.NoError(t, err)
	at := time.Date(2026, 8, 26, 12, 0, 0, 0, time.UTC)
	_, _, found, err := source.ClaimNextEmbeddingWork(t.Context(), "worker-before-backup", at, 5*time.Minute)
	require.NoError(t, err)
	require.True(t, found)
	...
	resumed, _, found, err := target.ClaimNextEmbeddingWork(t.Context(), "worker-after-restore", at.Add(6*time.Minute), 5*time.Minute)
	require.NoError(t, err)
	require.True(t, found)
...
func TestEmbeddingJobLeaseCannotRenewAfterExpiry(t *testing.T) {
	...
	require.NoError(t, err)
	at := time.Date(2026, 8, 26, 12, 0, 0, 0, time.UTC)
	claim, work, found, err := s.ClaimNextEmbeddingWork(t.Context(), "worker-expiry", at, time.Minute)
	require.NoError(t, err)
	require.True(t, found)
	...
	require.ErrorIs(t, err, ErrEmbeddingJobFenced)
	resumed, _, found, err := s.ClaimNextEmbeddingWork(t.Context(), "worker-resume", at.Add(time.Minute), time.Minute)
	require.NoError(t, err)
	require.True(t, found)
	assert.Greater(t, resumed.Epoch, claim.Epoch)
}
...
func TestEmbeddingJobProviderUnavailableRetryBudgetIsDurable(t *testing.T) {
	...
	for attempt := 1; attempt <= embeddingJobMaxClaims; attempt++ {
		claim, work, found, err := s.ClaimNextEmbeddingWork(t.Context(), "worker-budget", at, time.Minute)
		require.NoError(t, err)
		require.True(t, found)
	...
	_, _, found, err := s.ClaimNextEmbeddingWork(t.Context(), "worker-budget", at, time.Minute)
	require.NoError(t, err)
...
func TestEmbeddingJobExistingStaleHeadDoesNotSuppressReplacement(t *testing.T) {
	...
	_, work, found, err := s.ClaimNextEmbeddingWork(t.Context(), "worker-replacement",
		time.Date(2026, 8, 26, 12, 0, 0, 0, time.UTC), time.Minute)
	require.NoError(t, err)
	require.True(t, found)
...
func TestVersionPruneRemovesQueuedEmbeddingJobGenerationAndRoot(t *testing.T) {
	...
	_, _, found, err := s.ClaimNextEmbeddingWork(t.Context(), "worker-prune",
		time.Date(2026, 8, 26, 12, 0, 0, 0, time.UTC), time.Hour)
	require.NoError(t, err)
	require.True(t, found)

...
func TestCollectionCoverageFiveStatesAndExactAuthority(t *testing.T) {
	...
	require.Equal(t, int64(5), got.Collection.FileCount)
	require.Equal(t, CoverageCounts{Complete: 1, Partial: 1, Failed: 1, Unprocessed: 1, None: 1}, *got.Collection.Coverage.Counts)
	require.NotEmpty(t, got.Collection.Coverage.GenerationID)

	other := catalogProcessingProfile(t, true)
	isolated, err := s.CollectionQuality(t.Context(), run.ID(), CoverageSelection{"configured", other.Fingerprint}, nil)
	require.NoError(t, err)
	require.Equal(t, CoverageCounts{Unprocessed: 5}, *isolated.Collection.Coverage.Counts)
	...
	updated, err := s.CollectionQuality(t.Context(), run.ID(), selection, nil)
	require.NoError(t, err)
	require.Equal(t, CoverageCounts{Partial: 1, Failed: 1, Unprocessed: 2, None: 1}, *updated.Collection.Coverage.Counts)
}
...
func TestCollectionCoverageRetainsActivatedOutputAfterFailure(t *testing.T) {
	...
	for _, state := range []string{"complete", "none", "empty_segments", "blank_segments", "degraded", "truncated", "partial_success"} {
		t.Run(state, func(t *testing.T) {
			...
			got, err := s.CollectionQuality(t.Context(), run.ID(), CoverageSelection{"configured", profile.Fingerprint}, nil)
			require.NoError(t, err)
			...
			require.Equal(t, want, *got.Collection.Coverage.Counts)
		})
	}
}
...
func TestCollectionCoverageNativeTextFollowsServingSearchPath(t *testing.T) {
	...
	before, err := s.CollectionQuality(t.Context(), run.ID(), selection, nil)
	require.NoError(t, err)
	require.Equal(t, CoverageCounts{Complete: 1, Unprocessed: 1}, *before.Collection.Coverage.Counts)
	hits, _, err := s.SearchPage(t.Context(), "nativeuniqueterm", 10)
	require.NoError(t, err)
	require.Len(t, hits, 1)
	collectionCoveragePublish(t, s, nodes[0], profile, "complete")
	after, err := s.CollectionQuality(t.Context(), run.ID(), selection, nil)
	require.NoError(t, err)
	require.Equal(t, CoverageCounts{Complete: 1, Unprocessed: 1}, *after.Collection.Coverage.Counts)
	hits, _, err = s.SearchPage(t.Context(), "nativeuniqueterm", 10)
	require.NoError(t, err)
...
func TestCollectionCoverageSelectionAndUnavailableStates(t *testing.T) {
	...
	for _, selection := range []CoverageSelection{{}, {Configuration: "unconfigured"}, {Configuration: "profile_required"}} {
		got, err := s.CollectionQuality(t.Context(), run.ID(), selection, nil)
		require.NoError(t, err)
		require.Nil(t, got.Collection.Coverage.Counts)
		require.NotEmpty(t, got.Collection.Coverage.Configuration)
	}
	got, err := s.CollectionQuality(t.Context(), run.ID(), CoverageSelection{"configured", profile.Fingerprint}, nil)
	require.NoError(t, err)
	require.Equal(t, CoverageCounts{Unprocessed: 1}, *got.Collection.Coverage.Counts, "valid policy need not have catalog rows yet")
	_, err = s.CollectionQuality(t.Context(), run.ID(), CoverageSelection{Configuration: "configured"}, nil)
...
func TestCollectionCoverageStagedAndMismatchedGenerationAreUnprocessed(t *testing.T) {
	...
	require.NoError(t, err)
	staged, err := s.CollectionQuality(t.Context(), run.ID(), selection, nil)
	require.NoError(t, err)
	require.Equal(t, CoverageCounts{Unprocessed: 1}, *staged.Collection.Coverage.Counts)
	require.Empty(t, staged.Collection.Coverage.GenerationID)
	...
	require.NoError(t, err)
	mismatch, err := s.CollectionQuality(t.Context(), run.ID(), selection, nil)
	require.NoError(t, err)
	require.Equal(t, CoverageCounts{Unprocessed: 1}, *mismatch.Collection.Coverage.Counts)
	require.Equal(t, active.ID, mismatch.Collection.Coverage.GenerationID)
...
func TestCollectionCoverageBrokenGenerationIsNotMissingCollection(t *testing.T) {
	...
	require.NoError(t, err)
	t.Run("browsing ignores processing catalog", func(t *testing.T) {
		_, _, err := s.Collections(t.Context(), 10, 0)
		require.NoError(t, err)
		_, err = s.CollectionByID(t.Context(), run.ID())
		require.NoError(t, err)
		_, err = s.CollectionMembers(t.Context(), run.ID(), 10, 0)
		require.NoError(t, err)
	})
	t.Run("unconfigured quality ignores processing catalog", func(t *testing.T) {
		got, err := s.CollectionQuality(t.Context(), run.ID(), CoverageSelection{}, nil)
		require.NoError(t, err)
		require.Nil(t, got.Collection.Coverage.Counts)
	})
	_, err = s.CollectionQuality(t.Context(), run.ID(), CoverageSelection{"configured", profile.Fingerprint}, nil)
	require.Error(t, err)
...
func TestCollectionCoverageSupersessionMultiImportAndEmptyRetainedCollection(t *testing.T) {
	...
	for _, id := range []string{first.ID(), second.ID()} {
		got, err := s.CollectionQuality(t.Context(), id, selection, nil)
		require.NoError(t, err)
		require.Equal(t, CoverageCounts{Complete: 1}, *got.Collection.Coverage.Counts)
	}
	...
	got, err := s.CollectionQuality(t.Context(), first.ID(), selection, nil)
	require.NoError(t, err)
	require.Equal(t, int64(0), got.Collection.FileCount)
	require.Equal(t, CoverageCounts{}, *got.Collection.Coverage.Counts)
	quality, err := s.CollectionQuality(t.Context(), first.ID(), selection, nil)
	require.NoError(t, err)
	require.Equal(t, got, quality)
...
func TestCollectionCoverageOperatorRequired(t *testing.T) {
	...
	got, err := s.CollectionQuality(t.Context(), run.ID(), selection, nil)
	require.NoError(t, err)
	require.Equal(t, CoverageCounts{Failed: 1}, *got.Collection.Coverage.Counts)
}
...
func TestCollectionCoverageLatestExactAttemptUsesWaiterAndJobTimes(t *testing.T) {
	...
	got, err := s.CollectionQuality(t.Context(), run.ID(), selection, nil)
	require.NoError(t, err)
	require.Equal(t, CoverageCounts{Unprocessed: 1}, *got.Collection.Coverage.Counts)

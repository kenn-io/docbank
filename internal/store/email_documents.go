...
func (s *Store) publishEmailDocumentsTx(
	...
	err = func() error {
		...
		for i, p := range parts {
			...
			rel := document.EmailDocumentRelation{
				OperationID: request.OperationID, Order: i + 1, Parent: request.Parent,
				GenerationID: request.GenerationID, AttachmentID: request.AttachmentID,
				PartPath: p.Path, SiblingOrder: p.SiblingOrder, Filename: p.Filename.SafeName,
				Outcome: document.EmailDocumentPartOutcome(p),
			}
			...
			if hasReuse {
				if err := validateEmailDocumentReuse(ctx, tx, chosen, p); err != nil {
					return err
				}
				...
				if errors.Is(err, ErrExists) {
					return fmt.Errorf("%w: operation %s cannot create %q because the name already exists; "+
						"use a new operation ID or explicitly reuse the existing child", ErrEmailDocumentConflict, request.OperationID, name)
				}
				if err != nil {
					return err
				}
...
func validateEmailDocumentReuse(ctx context.Context, tx *sql.Tx, chosen document.EmailDocumentReuse, part document.EmailPartV1) error {
	v, err := emailVersion(ctx, tx, chosen.Child.VersionID)
	if err != nil {
		return err
	}
	if documentIdentity(v) != chosen.Child || v.BlobHash != part.Payload.SHA256 || v.Size != part.Payload.Size {
		return ErrEmailDocumentConflict
	}

...
func TestDocumentCatalogReturnsOnlySummaryProcessingAndActiveRenditionIdentity(t *testing.T) {
	...
	require.NoError(t, publishRenditionForTest(t, s, attachment,
		"2026-08-28T10:01:00.000000000Z", fakeHash("a8")))
...
func TestDocumentCatalogRejectsUnboundedTraversal(t *testing.T) {
	for _, fixture := range []struct {
		...
		{"depth", MaxWalkDepth, "dir"},
		{"path bytes", 128, strings.Repeat("界", 85)},
		{"name", 1, strings.Repeat("x", 256)},
	} {
		t.Run(fixture.name, func(t *testing.T) {
			s := newTestStore(t)
			parent := s.RootID()
			for range fixture.depth {
				dir, err := s.Mkdir(t.Context(), parent, fixture.segment)
				require.NoError(t, err)
				parent = dir.ID
			}
			fileName := "file.txt"
			...
			_, err := s.CreateFile(t.Context(), parent, fileName, fakeHash("bounded-catalog"), 1, "text/plain")
			require.NoError(t, err)
			_, err = s.ListDocuments(t.Context(), DocumentCatalogQuery{}, nil, DocumentCatalogTraversalNext)
			require.ErrorIs(t, err, ErrInvalidDocumentQuery)
			scoped, err := s.Mkdir(t.Context(), s.RootID(), "scoped")
			require.NoError(t, err)
			_, err = s.CreateFile(t.Context(), scoped.ID, "valid.txt", fakeHash("bounded-scoped"), 1, "text/plain")
			require.NoError(t, err)
			page, err := s.ListDocuments(t.Context(), DocumentCatalogQuery{PathPrefix: "/scoped"}, nil, DocumentCatalogTraversalNext)
			require.NoError(t, err)
			require.Equal(t, []string{"/scoped/valid.txt"}, documentCatalogPaths(page.Items))
		})
	}
}

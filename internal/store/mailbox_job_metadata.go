...
func exportMailboxJobs(ctx context.Context, q metadataQuerier, write metadataWrite) error {
	...
	for {
		...
		if c.State != mailboxContainerSealed || c.SHA256 != j.ContainerSHA256 {
			return ErrMailboxInvalid
		}
		if err = write(metadataMailboxJob{Type: "mailbox_job", Job: j}); err != nil {
			return err
		}
		var ordinal, imported, rejected, pending, canceled int64
		for {
			...
			for _, o := range page {
				...
				switch o.Outcome {
				case mailboxOccurrencePending, "canceled":
					if o.Ordinal != j.Checkpoint+1 || o.ReceiptID != "" {
						return ErrMailboxInvalid
					}
					if o.Outcome == mailboxOccurrencePending {
						pending++
					} else {
						canceled++
					}
		...
		if ordinal != j.Checkpoint+pending+canceled || imported != j.Imported || rejected != j.Rejected || pending != j.Pending || canceled != j.Canceled {
			return ErrMailboxInvalid
		}
		after = id
	}

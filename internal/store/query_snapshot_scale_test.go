...
func TestQuerySnapshotScale(t *testing.T) { //nolint:paralleltest // reports timings and memory that other tests would skew
	...
	for _, target := range []int{25000, 118000} {
		...
		assert.Equal(t, 7, available, "all facets except unconfigured text coverage must finish within the budget")
		assert.Equal(t, map[string]int{"coverage_unconfigured": 1}, reasons)

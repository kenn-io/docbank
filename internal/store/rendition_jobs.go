...
var (
	...
	ErrRenditionJobWaiterReselected = errors.New("rendition job waiter authority was reselected")
)
...
func (s *Store) EnqueueRenditionJob(
	...
	err = s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		if err := tx.QueryRowContext(ctx, `SELECT execution_identity_fingerprint
			FROM rendition_jobs WHERE job_id=?`, jobID).Scan(&storedExecution); err != nil {
			return fmt.Errorf("reading shared rendition job identity: %w", err)
		}
		if storedExecution != executionFingerprint {
			return errors.New("shared rendition build has an incompatible execution identity")
		}
	...
	err := s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		err := tx.QueryRowContext(ctx, `
			SELECT state,phase,claim_owner,claim_epoch,lease_expires_at,available_at,
			       provider_started,provider_attempts,provider_resume_handle,lexical_generation_id
			FROM rendition_jobs WHERE job_id=?`, jobID).Scan(
			&state, &phase, &currentOwner, &epoch, &leaseRaw, &availableRaw,
			&providerStarted, &providerAttempts, &resume, &generationID)
		...
		if state == RenditionJobRunning {
			...
			if renditionJobProviderBoundaryAmbiguous(phase, providerStarted, resume.Valid) {
				if _, err := tx.ExecContext(ctx, `
	...
	err := s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		if !waiterID.Valid {
			candidateID, candidateAuthorization, selectErr := selectRenditionJobWaiterTx(
				ctx, tx, s.vaultID, job, at.UTC())
			if selectErr != nil {
				noAuthorizedWaiter = selectErr
			} else {
...
func selectRenditionJobWaiterTx(
	ctx context.Context, tx *sql.Tx, vaultID string, job RenditionJob, at time.Time,
) (string, ProviderOperationAuthorization, error) {
	waiterIDs, err := renditionWaitingIDsTx(ctx, tx, job.ID)
	if err != nil {
	...
	for _, candidateID := range waiterIDs {
		...
		if err == nil {
			authorization, err = authorizeProviderOperationTx(ctx, tx, vaultID, request, at)
		}
		if err == nil {
	...
	if sawStale && len(waiterIDs) != 0 {
		return "", ProviderOperationAuthorization{}, ErrRenditionJobStaleAuthority
	}
...
func (s *Store) BeginRenditionProvider(
	...
	err = s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		request, requestErr := renditionWaiterAuthorizationTx(ctx, tx, job, waiterID)
		if requestErr != nil && !errors.Is(requestErr, ErrRenditionJobStaleAuthority) {
			return requestErr
		}
		if requestErr != nil {
			err = requestErr
		...
		if readErr := tx.QueryRowContext(ctx, `
			...
			&storedSnapshot, &resumeHandle); readErr != nil {
			return fmt.Errorf("reading rendition provider authority: %w", readErr)
		}
		if len(snapshotJSON) != 0 {
		...
		if errors.Is(err, ErrProcessingConsentRequired) ||
			...
			if _, rejectErr := tx.ExecContext(ctx, `UPDATE rendition_job_waiters
				SET state='rejected',failure_code=?,updated_at=?
				WHERE waiter_id=? AND job_id=? AND state='waiting'`,
			...
			if selectErr != nil {
				...
				authorityErr = errors.Join(err, selectErr)
				return nil
			}
			result, requeueErr := tx.ExecContext(ctx, `UPDATE rendition_jobs SET
				state='queued',claim_owner=NULL,lease_expires_at=NULL,
				claim_epoch=claim_epoch+1,available_at=?,selected_waiter_id=?,
				authorization_grant_id=?,authorization_incarnation_id=?,
				authorization_revocation_fence=?,failure_code=NULL,updated_at=?
				WHERE job_id=? AND claim_epoch=?`, at.UTC().Format(timestampLayout),
				nextWaiterID, nextAuthorization.GrantID,
				nextAuthorization.ProcessingIncarnationID, nextAuthorization.RevocationFence,
		...
		_, err = tx.ExecContext(ctx, `
			UPDATE rendition_jobs SET phase='provider',provider_started=1,
				...
				selected_waiter_id=?,authorization_grant_id=?,
				authorization_incarnation_id=?,authorization_revocation_fence=?,updated_at=?
			WHERE job_id=?`, attemptIncrement, nullableBytes(snapshotJSON), waiterID, authorization.GrantID,
	...
	if waiterReselected {
		return ProviderOperationAuthorization{}, ErrRenditionJobWaiterReselected
	}
	if authorityErr != nil {
		return ProviderOperationAuthorization{}, authorityErr
	}
...
func (s *Store) MarkRenditionJobRetry(
	...
	return s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		result, err := tx.ExecContext(ctx, `UPDATE rendition_jobs SET
			...
			phase=CASE
				WHEN phase='provider' AND provider_resume_handle IS NULL THEN 'queued'
				ELSE phase
			END,
			provider_started=CASE
				WHEN phase IN ('build_staged','generation_staged') OR
					(phase='provider' AND provider_resume_handle IS NOT NULL)
					THEN provider_started
				ELSE 0
			END,
			...
			END,
			selected_waiter_id=CASE
				WHEN phase='provider' AND provider_resume_handle IS NULL THEN NULL
				ELSE selected_waiter_id
			END,
			authorization_grant_id=CASE
				WHEN phase='provider' AND provider_resume_handle IS NULL THEN NULL
				ELSE authorization_grant_id
			END,
			authorization_incarnation_id=CASE
				WHEN phase='provider' AND provider_resume_handle IS NULL THEN NULL
				ELSE authorization_incarnation_id
			END,
			authorization_revocation_fence=CASE
				WHEN phase='provider' AND provider_resume_handle IS NULL THEN NULL
				ELSE authorization_revocation_fence
			END,
...
func validateCapturedArtifactPolicyForProfile(
	policy normalizedCapturedArtifactPolicyV1, profile document.ProcessingProfileV1,
) error {
	sanitizedMarkdownCount := 0
	if profile.RetentionDisclosure.RetainSanitizedMarkdown {
		sanitizedMarkdownCount = 1
	}
	deterministicCounts := []struct {
		role  string
		count int
	}{
		{role: catalogArtifactNormalizedEvidence, count: 1},
		{role: catalogArtifactSanitizedMarkdown, count: sanitizedMarkdownCount},
	}
	for _, deterministic := range deterministicCounts {
		cardinality := policy.cardinalities[deterministic.role]
		if deterministic.count < cardinality.MinCount || deterministic.count > cardinality.MaxCount {
			return fmt.Errorf(
				"role %q deterministically produces %d artifacts outside [%d,%d]",
				deterministic.role, deterministic.count, cardinality.MinCount, cardinality.MaxCount)
		}
	}
	requested := make(map[string]bool, len(profile.Rendition.RequestedArtifacts))
	...
	for role, cardinality := range policy.cardinalities {
		if role == catalogArtifactNormalizedEvidence ||
			role == catalogArtifactSanitizedMarkdown {
			continue
		}
		if cardinality.MaxCount > 0 && !renditionProfileRetainsArtifact(profile, role, requested) {
			return fmt.Errorf("role %q is captured but not retained by the processing profile", role)
		}
	}
	for role := range requested {
		if renditionProfileRetainsArtifact(profile, role, requested) &&
			policy.cardinalities[role].MaxCount == 0 {
			return fmt.Errorf("retained role %q is absent from the captured artifact policy", role)
		}
	}
	return nil
}

func renditionProfileRetainsArtifact(
	profile document.ProcessingProfileV1, role string, requested map[string]bool,
) bool {
	if !requested[role] {
		return false
	}
	if role == string(document.EvidenceArtifactMarkdown) {
		return profile.RetentionDisclosure.RetainProviderMarkdown
	}
	return profile.RetentionDisclosure.RetainTypedArtifacts
}
...
func (s *Store) finishRenditionJobClaim(
	...
	return s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		)`, claim.JobID).Scan(&hasWaiting); err != nil {
			return fmt.Errorf("checking rendition job consumers: %w", err)
		}
		reopenableStaged := state == RenditionJobFailed &&
			(code == RenditionFailureConsent || code == RenditionFailureStaleAuthority) &&
			(job.Phase == RenditionPhaseBuildStaged || job.Phase == RenditionPhaseGenerationStaged)
		...
		if reopenableStaged {
			if _, err := tx.ExecContext(ctx, `UPDATE rendition_jobs SET
				phase='queued',provider_started=0,provider_resume_handle=NULL,
				execution_snapshot_json=NULL,selected_waiter_id=NULL,
				authorization_grant_id=NULL,authorization_incarnation_id=NULL,
				authorization_revocation_fence=NULL,lexical_generation_id=NULL,
				updated_at=? WHERE job_id=? AND claim_epoch=?`,
				at.UTC().Format(timestampLayout), claim.JobID, claim.Epoch); err != nil {
				return fmt.Errorf("resetting orphaned reopenable rendition job: %w", err)
			}
		}
...
func (s *Store) PublishRenditionJob(
	...
	err := s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		for _, waiterID := range waiterIDs {
			...
			if err != nil {
				if errors.Is(err, ErrRenditionJobStaleAuthority) {
					rejected = append(rejected, rejectedWaiter{
						id: waiterID, code: RenditionFailureStaleAuthority,
					})
					continue
				}
				return err
			...
			if _, err := authorizeProviderOperationTx(
				...
				if errors.Is(err, ErrProcessingConsentRequired) ||
					...
					errors.Is(err, ErrProcessingConsentRevoked) {
					rejected = append(rejected, rejectedWaiter{
						id: waiterID, code: RenditionFailureConsent,
					})
					continue
				}
				return err
		...
		for _, authority := range authorized {
			attachment := RenditionAttachmentRecord{
				...
				BuildID:          claim.JobID, Profile: authority.profile, AttachedAt: publishedAt,
			}
...
func refreshRenditionJobRootsTx(
	ctx context.Context, tx *sql.Tx, jobID string, phase RenditionJobPhase,
	generationID sql.NullString, epoch int64, at string,
) error {
	...
	if err := putCurrentRenditionRootTx(ctx, tx, CurrentRenditionRoot{
		ID: renditionJobRootID("build", jobID), Kind: RenditionRootJob,
		TargetKind: RenditionRootBuild, TargetID: jobID,
		FencingToken: epoch, RecordedAt: at,
	}); err != nil {
		return fmt.Errorf("refreshing rendition build root: %w", err)
	}
	if phase != RenditionPhaseGenerationStaged {
		return nil
	}
	if !generationID.Valid {
		return errors.New("refreshing rendition generation root: generation identity is missing")
	}
	if err := putCurrentRenditionRootTx(ctx, tx, CurrentRenditionRoot{
		...
		return fmt.Errorf("refreshing rendition generation root: %w", err)
	}
	return nil
}
...
func loadRenditionJobTx(ctx context.Context, query rowQuerier, id string) (RenditionJob, error) {
	...
	err := query.QueryRowContext(ctx, `
		SELECT j.job_id,j.source_sha256,j.rendition_request_fingerprint,
		       ...
		       j.execution_identity_fingerprint,j.state,j.phase,j.claim_epoch,
		       j.provider_attempts,j.failure_code,
		...
		&job.ExecutionIdentityFingerprint, &job.State, &job.Phase, &job.ClaimEpoch,
		&job.ProviderAttempts, &failure,
	...
	if !validRenditionJobState(job.State) || !validRenditionJobPhase(job.Phase) ||
		job.FailureCode != "" && !validRenditionFailureCode(job.FailureCode) {
		return RenditionJob{}, errors.New("rendition job has invalid durable status")
	}
	return job, nil
}
...
func loadRenditionJobWaiterTx(
	...
	var failureCode *RenditionFailureCode
	if failure.Valid {
		code := RenditionFailureCode(failure.String)
		failureCode = &code
		waiter.FailureCode = code
	}
	if !validRenditionWaiterFailure(waiter.State, failureCode) {
		return RenditionJobWaiter{}, errors.New("rendition job waiter has invalid durable failure")
	}
	return waiter, nil
}
...
func renditionWaiterAuthorizationTx(
	...
	if err := validateRenditionWaiterInputClasses(request.InputClasses); err != nil {
		return ProviderOperationAuthorizationRequest{}, fmt.Errorf("%w: %w", err, ErrRenditionJobStaleAuthority)
	}
	...
	if err != nil {
		return ProviderOperationAuthorizationRequest{}, fmt.Errorf(
			"reading rendition job captured artifact policy: %w", err)
	}
	if !slices.Equal(request.RetainedArtifactClasses, policy.retainedRoles()) {
		return ProviderOperationAuthorizationRequest{}, fmt.Errorf(
			"rendition waiter retained artifact classes do not match captured policy: %w",
			ErrRenditionJobStaleAuthority)
	}
	return request, nil
}
...
func validRenditionJobState(value RenditionJobState) bool {
	switch value {
	case RenditionJobQueued, RenditionJobRunning, RenditionJobRetryWait,
		RenditionJobOperatorRequired, RenditionJobFailed, RenditionJobCompleted:
		return true
	default:
		return false
	}
...
func validRenditionJobStatePhase(state RenditionJobState, phase RenditionJobPhase) bool {
	switch phase {
	case RenditionPhaseQueued:
		return state == RenditionJobQueued || state == RenditionJobRunning ||
			state == RenditionJobRetryWait || state == RenditionJobFailed
	case RenditionPhaseProvider:
		return state == RenditionJobQueued || state == RenditionJobRunning ||
			state == RenditionJobRetryWait || state == RenditionJobOperatorRequired ||
			state == RenditionJobFailed
	case RenditionPhaseBuildStaged, RenditionPhaseGenerationStaged:
		return state == RenditionJobQueued || state == RenditionJobRunning ||
			state == RenditionJobRetryWait || state == RenditionJobFailed
	case RenditionPhasePublished:
		return state == RenditionJobCompleted
	default:
		return false
	}
}

func renditionJobProviderBoundaryAmbiguous(
	phase RenditionJobPhase, providerStarted, hasResumeHandle bool,
) bool {
	return providerStarted && !hasResumeHandle &&
		phase != RenditionPhaseBuildStaged &&
		phase != RenditionPhaseGenerationStaged
}

func validateRenditionWaiterInputClasses(inputs []string) error {
	if !slices.Equal(inputs, []string{string(document.RenditionInputOriginalFile)}) {
		return errors.New("rendition waiter input classes do not match original source egress")
	}
	return nil
}

func validRenditionJobPhase(value RenditionJobPhase) bool {
	switch value {
	case RenditionPhaseQueued, RenditionPhaseProvider, RenditionPhaseBuildStaged,
		RenditionPhaseGenerationStaged, RenditionPhasePublished:
		return true
	default:
		return false
	}
...
func validRenditionFailureCode(value RenditionFailureCode) bool {
	switch value {
	case RenditionFailureTransient, RenditionFailureTerminal, RenditionFailureAmbiguous,
...
func validRenditionWaiterState(value string) bool {
	return value == "waiting" || value == "published" || value == "rejected"
}

func validRenditionWaiterFailure(state string, code *RenditionFailureCode) bool {
	if code == nil {
		return true
	}

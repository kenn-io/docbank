...
func TestCompileQuerySupportsEveryBoundExpressionField(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "extension", text: `extension:tar_gz`, want: `%.tar\_gz`},
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if text, ok := testCase.want.(string); ok && text != `%.tar\_gz` {
...
func TestCompileQuerySupportsCanonicalCoverageAndDuplicatePredicates(t *testing.T) {
	...
	for _, text := range []string{`text_coverage:pending`, `has_duplicates:yes`, `has_duplicates:TRUE`} {
...
func TestCompiledQueryRootFacetOmissionDropsOnlyOneDimension(t *testing.T) {
	nested := compilerQuery(t, "")
	nested.Filters.MIMETypes = []string{"application/x-nested"}
	root := compilerQuery(t, `saved:inside`)
	root.Filters = query.Filters{
		Paths: []string{"/root-path"}, CollectionIDs: []string{compilerCollectionID},
		TagIDs: []string{compilerTagID}, MediaFamilies: []string{"document"},
		MIMETypes: []string{"application/x-root"}, Extensions: []string{"root_ext"},
		ModifiedAfter: "2026-01-02T03:04:05Z", SizeMin: 17,
	}
	compiled, err := CompileQuery(t.Context(), root, compilerResolver(&nested))
	require.NoError(t, err)
	...
	tests := []struct {
		...
		{dimension: "paths", value: "/root-path"},
		{dimension: "collections", value: compilerCollectionID},
		{dimension: "tags", value: compilerTagID},
		{dimension: "media_family", value: "document"},
		{dimension: "extension", value: `%.root\_ext`},
		{dimension: "modified", value: "2026-01-02T03:04:05.000000000Z"},
		{dimension: "size", value: int64(17)},
	}
	for _, testCase := range tests {
		t.Run(testCase.dimension, func(t *testing.T) {
			_, allArgs, err := compiled.Bind("", "")
			require.NoError(t, err)
			_, omittedArgs, err := compiled.Bind("", testCase.dimension)
			require.NoError(t, err)
			assert.Contains(t, allArgs, testCase.value)
			assert.NotContains(t, omittedArgs, testCase.value)
		})
	}
	_, omittedMIME, err := compiled.Bind("", "mime")
	require.NoError(t, err)
	assert.NotContains(t, omittedMIME, "application/x-root")
	assert.Contains(t, omittedMIME, "application/x-nested")
}
...
func TestCompiledQueryBindsFacetUnionsAndExclusions(t *testing.T) {
	...
	for _, expected := range []any{
		...
		`%.pdf`, `%.tar\_gz`, "2026-01-02T03:04:05.000000000Z",
...
func TestCompiledQueryNeverInterpolatesBoundValuesAndEnforcesLimits(t *testing.T) {
	...
	_, _, err = compiled.Bind("", "unknown")
	assertWholeExpressionError(t, compiled.Query.Text, err)
	tooLarge := CompiledQuery{Query: compilerQuery(t, "bounded"), expression: compiledQueryFragment{sql: strings.Repeat("x", maxCompiledQuerySQL)}}
	_, _, err = tooLarge.Bind("", "")
	assertWholeExpressionError(t, tooLarge.Query.Text, err)
	tooMany := CompiledQuery{Query: compilerQuery(t, "bounded"), expression: compiledQueryFragment{sql: "1", args: make([]any, maxCompiledQueryArgs+1)}}

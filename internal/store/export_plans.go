...
func (s *Store) CreateExportPlan(ctx context.Context, owner string, r bundle.PlanRequest) (bundle.Plan, error) {
	...
	err = s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		if source.State != exportSealedState || source.MemberHash != r.MemberHash {
			return bundle.ErrConflict
		}
		var count int
...
func resolveExportDocument(ctx context.Context, tx *sql.Tx, m bundle.Member, policies []bundle.RolePolicy) (bundle.Document, error) {
	...
	for _, policy := range policies {
		...
		switch policy.Role {
		case "original":
			roles = append(roles, bundle.Role{Role: "original", Status: exportRoleAvailable, Path: base + "original", SHA256: m.SHA256, Size: m.Size, MediaType: d.MediaType})
		case "text":
		...
		if len(roles) == 0 || err != nil {
			...
			roles = []bundle.Role{{Role: policy.Role, Status: exportRoleUnavailable}}
...
func resolveExportText(ctx context.Context, q metadataQuerier, m bundle.Member, p bundle.RolePolicy, base string) (bundle.Role, error) {
	r := bundle.Role{Role: "text", Status: exportRoleAvailable, Path: base + "text.md", MediaType: "text/markdown; charset=utf-8"}
...
func resolveExportPages(ctx context.Context, q metadataQuerier, m bundle.Member, p bundle.RolePolicy, base string) ([]bundle.Role, []document.PageFrameV1, error) {
	...
	for page := 1; page <= d.PageCount; page++ {
		...
		roles = append(roles, bundle.Role{Role: "pages", Status: exportRoleAvailable, Path: fmt.Sprintf("%spages/%06d.png", base, page), SHA256: view.Image.SHA256, Size: view.Image.Size, MediaType: "image/png", Recipe: raw, Page: &view.Image})
	}

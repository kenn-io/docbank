...
func TestHistoricalSourceMetadataOmitsCurrentAttachmentFacts(t *testing.T) {
	...
	require.NoError(t, err)
	originalVersionID := node.CurrentVersionID
	...
	node, _, err = s.Move(ctx, node.ID, s.RootID(), "renamed.pdf", node.Revision)
	require.NoError(t, err)
	_, _, err = s.ReplaceContent(ctx, node.ID, node.Revision, fakeHash("b2"), 13, "application/pdf")
	require.NoError(t, err)

	view, err := s.ContentVersionSourceMetadata(ctx, originalVersionID)
	require.NoError(t, err)
	assert.Equal(t, node.ID, view.Attachment.NodeID)
	assert.Equal(t, originalVersionID, view.Attachment.ContentVersionID)

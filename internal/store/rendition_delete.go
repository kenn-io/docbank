...
func deleteEmbeddingAuthorityForVersionsTx(ctx context.Context, tx *sql.Tx, versionIDs []string) error {
	for _, versionID := range versionIDs {
		generationIDs, err := stringColumnTx(ctx, tx, "embedding generations", `
			SELECT input_generation_id FROM embedding_sets WHERE content_version_id=?
			UNION SELECT generation_id FROM embedding_jobs WHERE content_version_id=?
			ORDER BY 1`, versionID, versionID)
		if err != nil {
			return err
		}
		vectorSpaceIDs, err := stringColumnTx(ctx, tx, "embedding vector spaces", `
			SELECT vector_space_id FROM embedding_sets WHERE content_version_id=?
			UNION SELECT vector_space_id FROM embedding_jobs WHERE content_version_id=?
			ORDER BY 1`, versionID, versionID)
		if err != nil {
			return err
		}
		if _, err := tx.ExecContext(ctx, `DELETE FROM current_rendition_roots WHERE root_kind=? AND root_id IN (
			...
			return fmt.Errorf("releasing embedding job roots for content version %s: %w", versionID, err)
		}
		if _, err := tx.ExecContext(ctx, `DELETE FROM embedding_jobs WHERE content_version_id=?`, versionID); err != nil {
			return fmt.Errorf("deleting embedding jobs for content version %s: %w", versionID, err)
		}
		...
		var vectorSetIDs, payloadIDs []string
		...
		for _, id := range generationIDs {
			result, err := tx.ExecContext(ctx, `DELETE FROM embedding_input_generations WHERE generation_id=? AND NOT EXISTS(
				SELECT 1 FROM embedding_sets WHERE input_generation_id=?) AND NOT EXISTS(
				SELECT 1 FROM embedding_jobs WHERE generation_id=?)`, id, id, id)
			if err != nil {
				return err
			}
		...
		for _, id := range vectorSpaceIDs {
			if _, err := tx.ExecContext(ctx, `DELETE FROM embedding_vector_spaces WHERE vector_space_id=?
				...
				AND NOT EXISTS(SELECT 1 FROM embedding_jobs WHERE vector_space_id=?)`, id, id, id, id); err != nil {
				return err
			}
		}
	}

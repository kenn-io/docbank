...
func resolveExportDocumentRows(ctx context.Context, q metadataQuerier, member bundle.Member, policies []bundle.RolePolicy, selected string, write func(bundle.Document) error) error {
	...
	for _, relation := range publication.Receipt.Relations {
		...
		for _, policy := range attachments {
			role := bundle.Role{Role: policy.Role, Status: mediaCoverageUnavailable, Reason: relation.Outcome}
			if relation.Child != nil {
				...
				if policy.Role == "attachment_original" {
					...
					if e != nil {
						role = bundle.Role{Status: mediaCoverageUnavailable, Reason: "No qualified retained email PDF for this child and recipe."}
					}
					role.Role = policy.Role
				}
			}
			if role.Status == mediaCoverageUnavailable && !policy.AllowUnavailable {

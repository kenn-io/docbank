...
var personMetadataRequiredFields = map[string][]string{
	metadataPersonType:              {metadataTypeField, "person_id", "display_name", "display_name_folded", "origin", "state", "revision", "created_at", "updated_at"},
	...
	metadataPersonMergeType:         {metadataTypeField, "merge_id", "operation_id", "request_sha256", "survivor_person_id", "absorbed_person_id", "absorbed_display_name", "moved_json", "survivor_revision_before", "survivor_revision_after", "created_at"},
	metadataPersonSplitType:         {metadataTypeField, "operation_id", "request_sha256", "receipt_json", "created_at"},
	metadataCustodianAssignmentType: {metadataTypeField, "assignment_id", "scope_kind", metadataIngestIDField, "package_id", "package_record_id", "node_id", metadataContentVersionIDField, "person_id", "raw_label", "raw_label_folded", "rank", "basis", "source_ref", "revision", "recorded_at", "retired_at"},
	metadataPersonAssertionType:     {metadataTypeField, "assertion_id", metadataContentVersionIDField, "person_id", "role", "action", "note", "recorded_at", "revision"},
	metadataPersonCandidateType:     {metadataTypeField, "candidate_id", "actor_key", "display_name", "suggested_person_id", "reason", "evidence_json", "evidence_sha256", "occurrence_count", "revision", "state", "decided_person_id", "created_at", "decided_at"},
}
...
func validateMetadataPersonMerge(r metadataPersonMerge) error {
	...
	for _, ids := range [][]string{moved.IdentityIDs, moved.AssignmentIDs, moved.AssertionIDs, moved.SupersededCandidate} {
...
func validateMetadataPersonCandidate(r metadataPersonMatchCandidate) error {
	if r.Type != metadataPersonCandidateType || validateUUIDv4(r.CandidateID) != nil || r.Revision < 1 ||
		!validCandidateText(r.ActorKey, document.MaxActorKeyBytes, false) || !validPersonName(r.DisplayName) ||
		...
		len(r.EvidenceJSON) == 0 || len(r.EvidenceJSON) > document.MaxPersonCandidateEvidence || r.OccurrenceCount < 1 {
	...
	wantDigest, err := candidateEvidenceDigest(occurrences)
	if err != nil || wantDigest != r.EvidenceSHA256 {
...
func validatePersonMetadataState(ctx context.Context, q metadataQuerier) error {
	...
	checks := []struct {
		...
		{"person alias authority", `SELECT EXISTS(
			...
			WHERE (a.reason='merged' AND (retired.person_id IS NOT NULL OR survivor.person_id IS NULL OR
			   ...
			   OR (a.reason='deleted' AND (retired.person_id IS NULL OR retired.state<>'retired' OR a.surviving_person_id IS NOT NULL))
		)`},
		{"person merge authority", `SELECT EXISTS(
			...
			WHERE survivor.person_id IS NULL OR a.reason<>'merged' OR a.surviving_person_id<>m.survivor_person_id
		)`},
	...
	for _, check := range checks {
		...
		if check.name == "person candidate queue bounds" {
			args = append(args, document.MaxOpenPersonCandidates)
		}

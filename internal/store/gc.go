	...
	err := s.withStorageTx(ctx, func(tx *sql.Tx) error {
		...
		if lexicalSchema {
			...
			for _, generation := range generations {
				...
				if err := releaseCollectedLexicalGenerationTx(ctx, tx, generation.id); err != nil {
					return err
				}
				result, err = tx.ExecContext(ctx,
...
func (s *Store) UnreachableDerivativePurgeBlobs(ctx context.Context) ([]BlobInfo, error) {
	rows, err := s.db.QueryContext(ctx, `
		...
		WHERE NOT EXISTS (SELECT 1 FROM ordinary_blob_authority o WHERE o.blob_hash = b.hash)

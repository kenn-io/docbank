...
func compiledFixtureIDs(t *testing.T, q metadataQuerier, compiled CompiledQuery, generation, omitted string) []int64 {
	t.Helper()
	predicate, args, err := compiled.Bind(generation, omitted)
	require.NoError(t, err)
...
func TestCompiledQuerySQLiteReferenceAndFacetScopes(t *testing.T) {
	...
	require.Equal(t, []int64{alpha.ID, beta.ID}, compiledFixtureIDs(t, s.db, compiled, "", "mime"))
	compiled = compileFixtureQuery(t, s, `saved:Choice`, query.Filters{SizeMin: 11})
	require.Empty(t, compiledFixtureIDs(t, s.db, compiled, "", ""))
	require.Equal(t, []int64{alpha.ID}, compiledFixtureIDs(t, s.db, compiled, "", "size"))
...
func TestCompiledQuerySQLiteCollectionUnionDeduplicatesMembership(t *testing.T) {
	...
	require.Empty(t, compiledFixtureIDs(t, s.db, compiled, "", ""))
	require.Equal(t, []int64{node.ID}, compiledFixtureIDs(t, s.db, compiled, "", "collections"))
}
...
func TestCompiledQuerySQLitePathMediaAndTime(t *testing.T) {
	...
	compiled := compileFixtureQuery(t, s, `path:/case_1 AND media_family:document AND extension:pdf`, query.Filters{
		ModifiedAfter: "2026-01-02T03:04:05Z", ModifiedBefore: "2026-01-02T03:04:05.000000002Z",
	})

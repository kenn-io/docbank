...
func TestQuerySnapshotCacheReservationsEvictionAndExpiry(t *testing.T) {
	t.Run("capacity pressure expires handles before evicting live rows", func(t *testing.T) {
		...
		service := newQuerySnapshotService(s, querySnapshotServiceOptions{
			Now:    func() time.Time { return now },
			Limits: snapshotCacheLimits{MaxRows: 2, IdleTTL: time.Hour},
			ChargeHook: func(context.Context, string, int64, int64) error {
				if advanceDuringBuild {
		...
		t.Cleanup(func() { require.NoError(t, service.Close()) })
		request := SnapshotRequest{Query: snapshotTestQuery(t, `{}`)}
		first, err := service.Create(t.Context(), "owner", request)
		require.NoError(t, err)
		now = now.Add(20 * time.Minute)
		second, err := service.Create(t.Context(), "owner", request)
		require.NoError(t, err)
		now = now.Add(9 * time.Minute)
		_, err = service.CopyMembers(t.Context(), "owner", first.SnapshotID, first.MemberHash)
		require.NoError(t, err)
		advanceDuringBuild = true
		_, err = service.Create(t.Context(), "owner", request)
		require.NoError(t, err)
		_, err = service.CopyMembers(t.Context(), "owner", second.SnapshotID, second.MemberHash)
		require.NoError(t, err, "the older live handle must survive when a newer-used handle expires during a build")
	})

	t.Run("publication releases unused batch reservations", func(t *testing.T) {
		s := newTestStore(t)
		_, err := s.CreateFile(t.Context(), s.RootID(), "a.txt", fakeHash("batch-reservation"), 1, "text/plain")
		require.NoError(t, err)
		service := newQuerySnapshotService(s, querySnapshotServiceOptions{
			Limits: snapshotCacheLimits{MaxRows: 2},
		})
		t.Cleanup(func() { require.NoError(t, service.Close()) })
		request := SnapshotRequest{Query: snapshotTestQuery(t, `{}`)}
		first, err := service.Create(t.Context(), "owner", request)
		...
		require.NoError(t, err)
		_, err = service.CopyMembers(t.Context(), "owner", first.SnapshotID, first.MemberHash)
		require.NoError(t, err, "unused reservations must not evict an existing snapshot")
		stats := service.stats()
		assert.Equal(t, int64(2), stats.CachedRows)
		assert.Zero(t, stats.ReservedRows)
		assert.Zero(t, stats.ReservedBytes)
		assert.Less(t, stats.CachedBytes, int64(256<<10))
	})
...
func TestQuerySnapshotCacheRejectsBusyAndCanceledAdmission(t *testing.T) {
	...
	for i := range querySnapshotMaxBuilders {
		...
		go func(owner string) {
			defer wg.Done()
			_, createErr := service.Create(context.Background(), owner, SnapshotRequest{Query: value})

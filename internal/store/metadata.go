...
// ExportBackup writes deterministic backup authority. It preserves ordinary
// unreachable blobs for the regret window and omits only explicitly marked,
// uncommitted provider staging.
...
// BackupBlobAuthorityCTE includes positive ordinary-blob authority, including
// pruned content retained for the regret window, plus every current durable
// reference. Uncommitted provider staging and pending exact erasure have no
// implicit authority.
const BackupBlobAuthorityCTE = `
WITH backup_authorized_blobs(hash) AS (
	SELECT hash FROM blobs b
	WHERE EXISTS (SELECT 1 FROM ordinary_blob_authority o WHERE o.blob_hash=b.hash)
	OR EXISTS (SELECT 1 FROM content_versions v WHERE v.blob_hash=b.hash)
	OR EXISTS (SELECT 1 FROM rendition_artifacts a WHERE a.blob_hash=b.hash)
	OR EXISTS (SELECT 1 FROM rendition_builds r WHERE r.source_sha256=b.hash)
	OR EXISTS (SELECT 1 FROM rendition_jobs j WHERE j.source_sha256=b.hash)
	OR EXISTS (SELECT 1 FROM embedding_input_generations g WHERE g.generation_blob_hash=b.hash)
	OR EXISTS (SELECT 1 FROM embedding_vector_sets v WHERE v.payload_blob_hash=b.hash)
	OR EXISTS (SELECT 1 FROM watch_sources w WHERE w.blob_hash=b.hash)
)
`

...
func TestPeopleRebuildWaitsForFailureRetryAndLateDirtyVersion(t *testing.T) {
	s := newTestStore(t)
	...
	require.NoError(t, err)
	_, err = s.db.Exec(`INSERT INTO document_people_dirty(content_version_id,reason,marked_at,revision) VALUES(?, 'late authority', ?, 1)`, version.ID, nowRFC3339())
	require.NoError(t, err)
	require.NoError(t, s.RefreshDocumentPeopleBuilds(t.Context()))
	build, err = s.DocumentPeopleBuild(t.Context(), operationID)
	require.NoError(t, err)
	require.Equal(t, "running", build.State, "a late dirty key before the worker cursor keeps the build open")
...
func TestPeopleRebuildCountsFailedAttemptThatRemainsDirty(t *testing.T) {
	s := newTestStore(t)
	version := seedDocumentPeopleEvent(t, s, "dirty-failure.txt", "a2", nil)
	operationID, err := newUUIDv4()
	require.NoError(t, err)
	_, err = s.RebuildDocumentPeople(t.Context(), operationID)
	require.NoError(t, err)
	_, err = s.db.Exec(`INSERT INTO document_people_dirty(content_version_id,reason,marked_at,revision) VALUES(?, 'retryable failure', ?, 1)`, version.ID, nowRFC3339())
	require.NoError(t, err)
...
func TestPeopleRebuildConsumesDirtyTerminalFailure(t *testing.T) {
	s := newTestStore(t)
	version := seedDocumentPeopleEvent(t, s, "dirty-unavailable.txt", "a3", nil)
	operationID, err := newUUIDv4()
	require.NoError(t, err)
	_, err = s.RebuildDocumentPeople(t.Context(), operationID)
	require.NoError(t, err)
	_, err = s.db.Exec(`INSERT INTO document_people_dirty(content_version_id,reason,marked_at,revision) VALUES(?, 'terminal failure', ?, 1)`, version.ID, nowRFC3339())
	require.NoError(t, err)

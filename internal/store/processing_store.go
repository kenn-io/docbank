...
func (s *Store) resolveExplicitProcessingSourceFence(
	...
	if err := tx.Commit(); err != nil {
		return ProcessingSourceFenceResolution{}, fmt.Errorf("closing source-fence snapshot: %w", err)
	}
...
func (s *Store) resolveFilteredProcessingSourceFenceSnapshot(
	...
	if err := snapshot.QueryRowContext(ctx, `SELECT COUNT(*) FROM `+nodeFrom+`
		...
		return ProcessingSourceFenceResolution{}, fmt.Errorf("counting source-fence scope: %w", err)
	}
	...
	if err != nil {
		return ProcessingSourceFenceResolution{}, fmt.Errorf("resolving source-fence scope: %w", err)
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id); err != nil {
			return ProcessingSourceFenceResolution{}, fmt.Errorf("reading source-fence scope: %w", err)
		}
		ids = append(ids, id)
	}
	if err := rows.Err(); err != nil {

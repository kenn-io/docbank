...
func exportBundleMetadata(ctx context.Context, q metadataQuerier, write metadataWrite) error {
	...
	for _, id := range ids {
		...
		if source.ID != id || source.State != "sealed" || !canonical.IsSHA256Hex(source.RequestSHA256) || source.Total < 1 || source.Total > bundle.MaxMembers {
	...
	for _, id := range ids {
		...
		err = walkExportDocumentBytes(ctx, q, id, func(raw []byte) error {
			...
			for _, r := range d.Roles {
				...
				if r.Status == mediaCoverageUnavailable {
...
func importBundleMetadata(ctx context.Context, tx *sql.Tx, raw jsontext.Value) error {
	...
	case "source":
		...
		if s.ID != r.ID || s.State != "sealed" || s.Total < 1 || s.Total > bundle.MaxMembers {
			return bundle.ErrConflict
		}

...
func computeDerivativeAuthorityStats(ctx context.Context, q rowQuerier) (*DerivativeAuthorityStats, bool, error) {
	...
	if err := func() error {
		var present bool
		if err := q.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM sqlite_master WHERE type='table' AND name='page_images')`).Scan(&present); err != nil {
			return err
		}
		if !present {
			return nil
		}
		rows, err := q.QueryContext(ctx, `SELECT i.version_id,i.page,i.recipe_sha256,i.checksum,i.blob_hash,b.size FROM page_images i JOIN blobs b ON b.hash=i.blob_hash ORDER BY i.version_id,i.recipe_sha256,i.page`)
		if err != nil {
			return err
		}
		defer func() { _ = rows.Close() }()
		for rows.Next() {
			var version, recipe, checksum, hash string
			var page, size int64
			if err := rows.Scan(&version, &page, &recipe, &checksum, &hash, &size); err != nil {
				return err
			}
			item := get("included", "page_image")
			if err := addDerivativeClassBytes(&item.LogicalBytes, size); err != nil {
				return err
			}
			item.Count++
			item.blobs[hash] = struct{}{}
			for _, field := range []string{version, strconv.FormatInt(page, 10), recipe, checksum, hash, strconv.FormatInt(size, 10)} {
				writeDerivativeClassField(item.checksum, field)
			}
		}
		return rows.Err()

...
func TestAgentCapabilitiesWithholdCurrentIncompleteOperationsWithoutLosingCensus(t *testing.T) {
	...
	if named != 20 {
		t.Fatalf("expected the current 20 named operations, got %d", named)
	}
	...
	if len(capabilities.Server.Operations) != 0 {
		t.Fatalf("named operations with missing behavioral metadata were advertised: %+v",

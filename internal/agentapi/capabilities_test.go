...
func TestAgentCapabilitiesPublishReviewedNativeOperationsAndRetainCensus(t *testing.T) {
	...
	if named != 29 {
		t.Fatalf("expected the current 29 named operations, got %d", named)
	}
	...
	if len(capabilities.Server.Operations) != 24 {
		t.Fatalf("expected 24 reviewed native document, report and export operations, got %d", len(capabilities.Server.Operations))
	}

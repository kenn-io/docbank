...
func TestAgentCapabilitiesPublishReviewedNativeOperationsAndRetainCensus(t *testing.T) {
	...
	if named != 31 {
		t.Fatalf("expected the current 31 named operations, got %d", named)
	}
	...
	if len(capabilities.Server.Operations) != 26 {
		t.Fatalf("expected 26 reviewed native document, report and export operations, got %d", len(capabilities.Server.Operations))
	}

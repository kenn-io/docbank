...
func TestAgentCapabilitiesPublishReviewedNativeOperationsAndRetainCensus(t *testing.T) {
	...
	if named != 28 {
		t.Fatalf("expected the current 28 named operations, got %d", named)
	}
	...
	if len(capabilities.Server.Operations) != 13 {
		t.Fatalf("expected 13 reviewed native report/export operations, got %d", len(capabilities.Server.Operations))
	}

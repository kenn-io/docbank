...
func TestAgentCapabilitiesPublishReviewedNativeOperationsAndRetainCensus(t *testing.T) {
	...
	if named != 32 {
		t.Fatalf("expected the current 32 named operations, got %d", named)
	}
	...
	if len(capabilities.Server.Operations) != 27 {
		t.Fatalf("expected 27 reviewed native document, report and export operations, got %d", len(capabilities.Server.Operations))
	}

...
func (c *Client) doEmailDocuments(ctx context.Context, method, path string, input, output any) error {
	var body io.Reader
	if input != nil {
		data, err := marshalJSONRequest(input)
		if err != nil {
			return err
		}
	...
	_, err := c.doRequest(ctx, method, path, nil, body, func(reader io.Reader) error {
		data, err := io.ReadAll(io.LimitReader(reader, document.EmailDocumentMaxJSONBytes+1))
		if err != nil {
			return err
		}
		if len(data) > document.EmailDocumentMaxJSONBytes {

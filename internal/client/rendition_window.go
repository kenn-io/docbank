...
func (c *Client) RenditionTextWindow(
	...
	_, err = c.doRequest(ctx, http.MethodPost, "/api/v1/renditions/windows", nil, bytes.NewReader(body),
		func(reader io.Reader) error {
			encoded, err := io.ReadAll(io.LimitReader(reader, maxRenditionWindowResponseBytes+1))
			if err != nil {
				return errors.New("reading rendition window response")
			}
			if len(encoded) > maxRenditionWindowResponseBytes {
				return errors.New("rendition window response is too large")
			}
			if err := json.Unmarshal(encoded, &result, json.RejectUnknownMembers(true)); err != nil {
				return errors.New("rendition window response is invalid")
			}
			return validateRenditionTextWindow(request, result)
		})
	if err != nil {

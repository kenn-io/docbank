...
func (searcher *Searcher) expand(ctx context.Context, query Query) ([]string, *ProviderReceipt, Degradation, error) {
	...
	operation := ProviderOperation{Stage: ProviderStageExpansion, ProfileID: config.Profile.ID,
		Scope: cloneSearchScope(query.Scope), InputClass: ProviderInputQueryText, VariantLimit: config.Profile.MaxVariants,
...
func (searcher *Searcher) rerank(ctx context.Context, query Query, report Report) (Report, *ProviderReceipt, Degradation, error) {
	return searcher.rerankWithRevalidation(ctx, query, report, nil)
}

type rerankingRevalidator func(context.Context, Query, Report) (Report, error)

func (searcher *Searcher) rerankWithRevalidation(ctx context.Context, query Query, report Report,
	revalidate rerankingRevalidator,
	...
	operation := ProviderOperation{Stage: ProviderStageReranking, ProfileID: config.Profile.ID,
		Scope: cloneSearchScope(query.Scope), InputClass: ProviderInputQueryAndExcerpt, CandidateCount: len(candidates),
		QueryBytes: len(query.Text), QueryByteLimit: maxProviderQueryBytes,
	...
	if revalidate != nil {
		revalidated, err := revalidate(stageCtx, query, report)
		if err != nil {
			if ctx.Err() != nil {
				return Report{}, nil, DegradationNone, ctx.Err()
			}
			if stageCtx.Err() != nil && isDeadlineError(err) {
				return searcher.rerankFailure(config, report, stageOutcome(stageCtx), len(candidates))
			}
			return Report{}, nil, DegradationNone, ErrRerankingFailed
		}
		if !reflect.DeepEqual(candidates,
			rerankingCandidates(revalidated.Results, config.Profile.MaxCandidates)) {
			return Report{}, nil, DegradationNone, ErrRerankingFailed
		}
		report = revalidated
	}
...
func rerankingEvidenceBytes(reference EvidenceReference) int {
	return len(reference.Kind) + len(reference.VaultID) + len(reference.ContentVersionID) +
		len(strconv.FormatInt(reference.NodeID, 10)) + len(strconv.FormatInt(reference.NodeRevision, 10)) +
...
func isDeadlineError(err error) bool {
	...
	if wrapped, ok := err.(interface{ Unwrap() error }); ok {
		return isDeadlineError(wrapped.Unwrap())
	}
	//nolint:errorlint // Exact sentinel identity prevents custom equivalence from weakening the authority fence.

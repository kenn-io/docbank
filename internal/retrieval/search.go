...
// ErrExpandedSearchFailed reports an expanded-query retrieval failure without
// exposing original or provider-generated query text.
...
func (searcher *Searcher) Search(ctx context.Context, query Query) (Report, error) {
	...
	base, err := searcher.searchBase(ctx, query)
	if err != nil {
		return Report{}, err
	}
	reports := make([]Report, 1, len(variants)+1)
	reports[0] = base
	for _, text := range variants {
		variant := query
		variant.Text = text
		report, searchErr := searcher.searchBase(ctx, variant)
		if searchErr != nil {
			if ctx.Err() != nil {
	...
	if searcher.reranking.Enabled && len(report.Results) > 0 {
		report, err = searcher.revalidateReport(ctx, query, report)
		if err != nil {
			if ctx.Err() != nil {
				return Report{}, ctx.Err()
			}
	...
	report, rerankingReceipt, rerankingDegradation, err := searcher.rerankWithRevalidation(
		ctx, query, report, searcher.revalidateReport)
	if err != nil {
		return Report{}, err
	}
	...
	return searcher.revalidateReport(ctx, query, report)
}
...
func (searcher *Searcher) revalidateReport(ctx context.Context, query Query, report Report) (Report, error) {
	...
	if !ok {
		return Report{}, errors.New("search candidate revalidation is unavailable")
	}
	...
	for i, result := range report.Results {
		...
		requested[i] = store.SearchCandidateIdentity{NodeID: result.Document.NodeID,
			NodeRevision: nodeRevision, ContentVersionID: result.Document.ContentVersionID,
			PathChecksum: store.SearchPathChecksum(result.Path), Evidence: evidence}
	}
	semanticProfile, semanticBinding := "", ""
	if report.Coverage.State != CoverageUnknown {
		semanticProfile, semanticBinding = query.ProcessingProfileFingerprint, query.BindingID
	}
	revalidation, err := backend.RevalidateSearchCandidates(ctx, requested, cloneSearchScope(query.Scope),
		semanticProfile, semanticBinding)
	if err != nil {
		return Report{}, err
	}
	if revalidation.Coverage != nil {
		...
		if report.Coverage.ScopedDocuments != report.Coverage.CompleteDocuments {
			...
			if report.RequestedMode == ModeAuto && report.Coverage.BindingRequired && report.ActualMode != ModeLexical {
				return Report{}, errors.New("required semantic coverage changed during candidate revalidation")
			}
		}
	}
	...
	allowed := make(map[documentKey]struct{}, len(revalidation.Candidates))
	for _, candidate := range revalidation.Candidates {
		allowed[documentKey{nodeID: candidate.NodeID, version: candidate.ContentVersionID}] = struct{}{}
	}
	results := report.Results[:0]
	for _, result := range report.Results {
		if _, ok := allowed[documentKey{nodeID: result.Document.NodeID,
...
func (searcher *Searcher) searchBase(ctx context.Context, query Query) (Report, error) {
	...
	case ModeAuto:
		...
		if semanticErr != nil {
			...
			if !errors.As(semanticErr, &degradable) {
...
func (searcher *Searcher) semantic(ctx context.Context, query Query,
	...
	authority, err := backend.AcquireSemanticSearchAuthority(ctx,
		...
		searcher.clock().UTC(), searcher.leaseDuration, cloneSearchScope(query.Scope))
	if err != nil {
	...
	if err != nil {
		if errors.Is(err, document.ErrEmbeddingInvalidResult) || ctx.Err() != nil {
			return nil, coverage, false, err
		}
		return nil, coverage, false, degradableSemanticFailure(DegradationProviderUnavailable, err)
	}
	stored := authority.Lease.Generation
	if stored.ID != store.VectorIndexGenerationID(stored.SourceManifestChecksum, stored.Bytes) {
		return nil, coverage, false, errors.New("leased vector generation ID does not match source-bound bytes")
	}
	...
	resolution, err := backend.ResolveSemanticCandidates(ctx, query.ProcessingProfileFingerprint,
		...
		neighbors, query.Limit, cloneSearchScope(query.Scope))
	if err != nil {
		return nil, coverage, false, err
	}
...
func normalizeQuery(query Query) (Query, error) {
	...
	if len(query.Scope.ContentVersionIDs) > store.MaxSearchSourceFenceIDs {
		return Query{}, errors.New("search source fence exceeds 4096 content versions")
	}
	query.Scope = cloneSearchScope(query.Scope)
	return query, nil
}

func cloneSearchScope(scope store.SearchOptions) store.SearchOptions {
...
func (searcher *Searcher) collectLexical(ctx context.Context, query Query) ([]Candidate, bool, error) {
	hits, truncated, err := searcher.backend.SearchExplainedLexicalCandidates(ctx, query.Text, query.Limit,
		cloneSearchScope(query.Scope))
	if err != nil {

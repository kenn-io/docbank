...
func TestRerankRejectsDuplicateStableIDsAndLocalBoundsBeforeSecrets(t *testing.T) {
	profile := testProfile(LatencySlow)
	profile.MaxTotalExcerptBytes = int64(profile.MaxExcerptBytes)
	...
	for name, mutate := range map[string]func(*retrieval.RerankingRequest){
		"query": func(value *retrieval.RerankingRequest) {
			value.Query = strings.Repeat("q", profile.MaxQueryBytes+1)
		},
		"excerpt": func(value *retrieval.RerankingRequest) {
			value.Candidates[0].Excerpt = strings.Repeat("d", profile.MaxExcerptBytes+1)
		},
		"total excerpts": func(value *retrieval.RerankingRequest) {
			length := profile.MaxExcerptBytes/2 + 1
			value.Candidates[0].Excerpt = strings.Repeat("a", length)
			value.Candidates[1].Excerpt = strings.Repeat("b", length)
		},
		"candidate count": func(value *retrieval.RerankingRequest) {
			value.Candidates = make([]retrieval.RerankingCandidate, profile.MaxCandidates+1)
		},
	} {
		t.Run(name, func(t *testing.T) {
			request := rerankingRequest()
			mutate(&request)
			_, err := client.Rerank(t.Context(), request)
...
func TestRerankPreservesCancellationDuringResponseRead(t *testing.T) {
	...
	case <-time.After(synchronizationWatchdog):
		require.FailNow(t, "timed out waiting for response read")
	}
	cancel()
	select {
	case err := <-done:
	...
	case <-time.After(synchronizationWatchdog):
		require.FailNow(t, "timed out waiting for canceled response read")
	}
}

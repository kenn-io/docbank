...
That MCP process advertises eight bounded reads: agent capabilities, a vault
summary, scoped documents, exact document metadata, visible tags, rendition
text, processing profiles, and format coverage. It reads the exact source grant
from the daemon, and a revoked session cannot keep reading through the MCP
process. Use CLI or the
authenticated daemon API for report and export reads until those MCP handlers
are qualified for private sessions.

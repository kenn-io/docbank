...
That MCP process advertises seven bounded reads: agent capabilities, a vault
summary, scoped documents, visible tags, rendition text, processing profiles,
and format coverage. It reads the exact source grant from the daemon, and a
revoked session cannot keep reading through the MCP process. Use CLI or the

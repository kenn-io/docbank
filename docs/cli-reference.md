...
```text
docbank documents list [--path-prefix /] [--sort path|name|modified_at|size|media_type] [--direction asc|desc] [--page-size 50] [--cursor TOKEN] [--source-version UUID ...] [--json]
```
...
Repeat `--source-version` to list only those exact content versions. An agent
session requires this bounded source fence; the daemon checks every requested
version against the current grant before returning a page.
...
| `history [--offset 0] [--limit 20]` | List retained report requests and summaries as JSON, up to 50 per page. Source visibility is checked on each page; withdrawn counts are withheld. |
| `show <export-id>` | Read the frozen summary as JSON after the daemon checks current source visibility. |
...
docbank export show-plan <plan-id>
docbank export problems <plan-id> [--after <offset>]
...
`show-plan` reads the immutable plan header from a saved ID. `problems` reads
up to 50 unavailable output details at a time; pass the returned `next` offset
to read another page. Both commands use the same daemon owner checks as the
plan preview.
...
## docbank agent capabilities

```text
docbank agent capabilities [--json]
```

Reads the operations currently available to the selected daemon credential.
The default output lists operation IDs; `--json` includes reviewed bindings,
bounds, runtime feature states, and the registry digest. A read-only agent
session sees only operations whose routes it can use. This is separate from
the remote-target `/api/v1/capabilities` negotiation response.
...
The catalog contains 34 read tools by default. `--allow-processing` adds

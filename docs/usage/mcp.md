...
Both transports have the fixed 34-tool read catalog described below.
With `DOCBANK_AGENT_SESSION_FILE` set, the MCP catalog contains eight bounded
reads: agent capabilities, vault summary, scoped documents, exact document
metadata, visible tags, rendition text, processing profiles, and format
coverage. Docbank reads the source grant from the daemon and checks it again
for each call.
...
| --- | --- |
| `get_agent_capabilities` | Reads the current credential's reviewed operation IDs, CLI and MCP bindings, page bounds, response caps, and runtime feature states. It does not expose host paths or credentials. |
...
| `list_tags` | Pages through up to 250 visible tag definitions and assignment counts. Scoped sessions see only tags on sources in their current grant. |
...
| `get_export_plan` | Reads the immutable plan header, selected roles, source hash, fingerprint, and expiry for one native export. |
| `list_export_output_problems` | Reads up to 50 unavailable output details for one frozen plan. Pass `next` as `after` for another page. |

...
512 MiB of private temporary storage per MCP server. Bundle verification is
serialized across the process, so another bundle open may wait. A handle is
published only after the complete artifact matches the authenticated daemon's
size and SHA-256 response. Bundles also pass independent packet verification.

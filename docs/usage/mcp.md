...
With `DOCBANK_AGENT_SESSION_FILE` set, the MCP catalog contains only
`get_agent_capabilities`, `list_tags`, `list_processing_profiles`, and
`get_format_coverage`. The daemon checks the private session on every call.

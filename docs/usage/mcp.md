...
With `DOCBANK_AGENT_SESSION_FILE` set, the MCP catalog contains seven bounded
reads: agent capabilities, vault summary, scoped documents, visible tags,
rendition text, processing profiles, and format coverage. Docbank reads the
source grant from the daemon and checks it again for each call.

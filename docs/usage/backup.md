...
!!! info "Planned — audited-history fidelity"
    ...
    chain heads, baseline ancestor-spine witnesses, replayable path-topology
    transitions and path-effect commitments, a stable vault ID, both allocator
    high-water marks, a
    ...
    reconcile attached metadata, derive each path-affecting descendant set from
    the prior topology projection, verify later mutations and allocation lineage

...
- Modify: `cmd/docbank/cmd/vault.go` (add `openVaultExclusive`)
- Test: append to `cmd/docbank/cmd/cli_test.go`

**Interfaces:**
- Consumes: `store.UnreachableBlobs/DeleteBlobRows/AllBlobs` (Task 13), `blob.Store.Remove/Open/Path` (Task 10), `openVaultExclusive` (vault.go), `runCLI` helper.
- Produces:
  - `docbank gc` — dry-run by default: lists candidate count and reclaimable bytes. `docbank gc --run` deletes blob files first, then their metadata rows; a missing file during deletion is not an error (crash reconciliation). gc opens the vault via `openVaultExclusive()` — the exclusive inter-process lock keeps a concurrent ingest from deduplicating against a blob file gc is about to delete.
...
Add to `cmd/docbank/cmd/vault.go` (after `openVault`):

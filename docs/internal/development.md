...
| `document/mistral` | Mistral OCR, capability evidence, unit limits, and the optional DOCX render-to-PDF route | generic format detection, DOCX conversion, or storage authority |
...
PPTX contract and also identifies the configured DOCX conversion route. For
DOCX, `document/renderpdf` owns conversion and page counting.

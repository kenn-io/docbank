...
| `document/isolate` | fixed local provider protocols, executable integrity, native isolation and bounded output | provider response parsing, vault ownership or database I/O |
...
`pkg` methods that submit accepted background work must hold the vault read lease until the processing service returns. The submission method transfers the lease to the goroutine that calls `StartWithProgress`, and that goroutine releases it after the service finishes. The embedded service uses a vault-owned lifecycle context after enqueue. Rendition-chunk embedding admission waits for a published rendition head. The embedding reconciler discovers that dependency through paged published-head scans with current-version, profile, exact attachment and consent checks. Rendition publication leaves dependent admission to the scan. Normal inline processing runs embeddings after successful publication. Bounded resume reports pending work when rendition publication consumes its job budget. Embedded callers drive recovery through `ResumeProcessing` because embedded scheduling belongs to the application, while the daemon's worker loops drive it automatically. `Close` cancels that context before waiting for the accepted work and then releases vault resources. A caller context controls admission through acknowledgement; cancelling it after acknowledgement does not cancel the accepted job.

Source metadata collection trims and validates each list entry before applying the 256-entry limit. Scalars and list entries accept at most 65,536 UTF-8 bytes. Oversized entries produce the same omission warning, and valid siblings survive. Extractor v4 creates a distinct immutable generation through Store publication; canonical serialization retains its strict bounds and publication requires verified EOF.

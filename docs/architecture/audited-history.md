...
```text
F("docbank-audit") || U(1) || F(record_kind) || U(field_count) ||
...
Every content version requires a positive `node_revision`, canonical
`introduced_operation_id`, and a `transition_kind` of `content_create`,
`content_replace`, or `content_revert`. Revert requires `source_version_id`;
the other two kinds require it absent. Import rejects invalid presence and
duplicate `(node_id, node_revision)` or `(node_id,
introduced_operation_id)` pairs before hashing or installing authority.

Version revision chronology is portable authority. For each file node, every
retained `node_revision` is positive and unique, never exceeds the node's
current revision, and sorts retained heads in their original order. A retained
`content_create` is unique for its node, has revision one and `recorded_at`
...
When at least one audit scope exists, version 1 additionally includes:

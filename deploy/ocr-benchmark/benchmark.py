...
def main() -> None:
    ...
    report = {"endpoint": args.endpoint, "cases": {}}
    for name, spec in manifest.items():
        ...
        try:
            ...
            if not isinstance(pages, list) or not pages:
                raise ValueError("endpoint returned no pages")
            report["cases"][name] = {
            ...
            report["cases"][name] = {"seconds": round(time.monotonic() - started, 3), "ok": False, "error_type": type(error).__name__}
    successful = [case for case in report["cases"].values() if case["ok"]]
    report["summary"] = {

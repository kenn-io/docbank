...
def artifact_digest(path: Path, algorithm: str) -> str:
    ...
    if algorithm == "git-sha1":
        ...
        return digest.hexdigest()
    raise RuntimeError("engine manifest uses an unsupported digest")


def validate_snapshot(root: Path, artifacts: object) -> None:
    if not isinstance(artifacts, list) or not artifacts:
        raise RuntimeError("engine model manifest is empty")
    expected_paths = set()
    for artifact in artifacts:
        if not isinstance(artifact, dict) or set(artifact) != {"path", "algorithm", "digest"}:
            raise RuntimeError("engine model manifest entry is invalid")
        relative = artifact["path"]
        if not isinstance(relative, str) or not relative or Path(relative).is_absolute() or ".." in Path(relative).parts:
            raise RuntimeError("engine model manifest path is invalid")
        if relative in expected_paths:
            raise RuntimeError("engine model manifest path is duplicated")
        expected_paths.add(relative)
        candidate = root / relative
        if not candidate.is_file() or artifact_digest(candidate, artifact["algorithm"]) != artifact["digest"]:
            raise RuntimeError("engine model snapshot does not match the pinned deployment")
    actual_paths = {
    ...
    if actual_paths != expected_paths:
        raise RuntimeError("engine model snapshot does not match the complete manifest")
...
def validated_deployment() -> str:
    ...
    validate_snapshot(MODEL_ROOT, manifest["model_files"])
    return fingerprint

...
func ParseProfile(raw []byte) (ProfileV1, error) {
	...
	if !bytes.Equal(raw, canonical) {
		return ProfileV1{}, errors.New("unstructured: profile is not canonical")
	}
...
func validateProfile(profile ProfileV1) error {
	...
	if profile.CredentialBinding != "" {
		if err := validateIdentity(profile.CredentialBinding, "credential binding", false, 128); err != nil {
			return err
		}
	}

...
func TestClientResumesOnlyTheFixedJobRoute(t *testing.T) {
	...
	resumeValue := testResumeValue(t, testJobID, started, checkpointed, fixture.metadata.ByteLength)
	var paths []string
...
func TestClientResumesHistoricalAuthorizationWithRecordedReceiptInterval(t *testing.T) {
	...
	result, err := document.ResumeRendition(t.Context(), client, snapshot, document.RenditionResumeHandle{
		Value: testResumeValue(t, testJobID, startedAt, checkpointedAt, fixture.metadata.ByteLength),
	}, nil)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	assert.True(t, completedAt.After(expiresAt))
...
func TestClientRejectsPartialOutputAndSchemaDrift(t *testing.T) {
	tests := []struct {
		...
		{name: "failed", body: `{"status":"Failed","progress":0.5,"reason":"private source detail","result":null}`, want: document.RenditionErrorUnsupportedInput},
		{name: "unknown status", body: `{"status":"Paused","progress":0.5,"reason":null,"result":null}`, want: document.RenditionErrorPolicyRejected},
		{name: "unknown model field", body: `{"status":"Completed","progress":1,"reason":null,"model":"next-model","result":null}`, want: document.RenditionErrorPolicyRejected},
...
func TestClientClassifiesHTTPAndSubmissionFailures(t *testing.T) {
	tests := []struct {
		...
		{name: "redirect", status: http.StatusTemporaryRedirect, want: document.RenditionErrorPolicyRejected},
		{name: "transient", status: http.StatusBadGateway, want: document.RenditionErrorTransient},
	}
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
	...
	for _, status := range []int{http.StatusNotFound, http.StatusGone} {
		...
		_, err = fixture.client(t, transport).RenderResumable(t.Context(), nil, fixture.authorization,
			&document.RenditionResumeHandle{Value: testResumeValue(t, testJobID,
...
func TestSubmissionFailureRemainsAmbiguousWhenWallTimeoutOccursAfterEgress(t *testing.T) {
	operationCtx, cancel := context.WithDeadline(t.Context(), time.Now().Add(-time.Second))
	defer cancel()
	client := &Client{}
	err := client.submissionFailure(t.Context(), operationCtx, time.Time{},
		"Reducto submission outcome is ambiguous", context.DeadlineExceeded)
	assertCode(t, err, document.RenditionErrorAmbiguousSubmission)
}

func TestSubmissionFailureRemainsAmbiguousWhenAuthorizationExpiresAfterEgress(t *testing.T) {
	client := &Client{}
	err := client.submissionFailure(t.Context(), t.Context(), time.Now().UTC().Add(-time.Second),
		"Reducto submission outcome is ambiguous", errors.New("synthetic transport failure"))
	assertCode(t, err, document.RenditionErrorAmbiguousSubmission)
}
...
func newFixture(t *testing.T, family, mediaType, filename string, source []byte) *fixture {
	...
	return &fixture{
		...
		authorization: document.RenditionAuthorization{
			...
			MaxProviderMarkdownBytes: 16 << 10, MaxArtifactBytes: 32 << 10,
			MaxArtifacts: 1, MaxTotalResultBytes: 128 << 10,
...
func testResumeValue(t *testing.T, jobID string, startedAt, submittedAt time.Time, inputBytes int64) string {
	t.Helper()
	raw, err := json.Marshal(testResumePayload{
		Version: "reducto-resume/v2", JobID: jobID,
	...
	require.NoError(t, err)
	return "r2." + base64.RawURLEncoding.EncodeToString(raw)
}

func decodeTestResumeHandle(t *testing.T, value string) testResumePayload {
	t.Helper()
	require.True(t, strings.HasPrefix(value, "r2."))
	raw, err := base64.RawURLEncoding.DecodeString(strings.TrimPrefix(value, "r2."))
	require.NoError(t, err)
...
func executionSnapshot(
	...
	require.NoError(t, err)
	rendition, err := document.NewRenditionPolicy(normalization, 1_000)
	require.NoError(t, err)

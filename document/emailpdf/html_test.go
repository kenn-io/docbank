...
func TestHTMLSanitizerShowsUnsafeResourcesAndExpandedHiddenQuotes(t *testing.T) {
	source := "Subject: unsafe\r\nDate: nonsense date\r\nContent-Type: text/html; charset=utf-8\r\n\r\n" +
		"<p onclick='alert(1)'>body</p><blockquote hidden style='display:none'>full quote</blockquote>" +
		"<img src='https://example.test/track'><img src='cid:absent'>" +
		"<img src='CID:absent' alt='uppercase CID'><img src='DATA:image/png;base64,AA==' alt='embedded'>" +
		"<script>danger()</script><iframe src='file:///etc/passwd'></iframe><a href='https://example.test/long'>link</a>"
	got, err := decodedHTML(t, source, "Letter")
	require.NoError(t, err)
	s := string(got.Bytes)
	for _, want := range []string{
		"full quote", "nonsense date", "date_invalid", "remote resource", "missing inline image",
		"[missing inline image: uppercase CID]", "[unsupported embedded image: embedded]",
		"active content", "size:Letter portrait", "https://example.test/long",
	} {
		require.Contains(t, s, want)
	}
	for _, forbidden := range []string{"onclick", "display:none", "<script", "<iframe", "src=\"https:", "src=\"data:", "src=\"DATA:", "file:///etc/passwd"} {

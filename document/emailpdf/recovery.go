...
func stopUnit(ctx context.Context, unit string) error {
	...
	defer cancel()
	stop := exec.CommandContext(ctx, "systemctl", "--user", "stop", unit) //nolint:gosec // Generated or recovery-validated exact UUID unit, never a shell command.
	stop.Env = userManagerEnv()
	stop.Stdout, stop.Stderr = io.Discard, io.Discard
	stopErr := stop.Run()
	check := exec.CommandContext(ctx, "systemctl", "--user", "show", unit, "--property=LoadState", "--value") //nolint:gosec // The same validated exact unit is queried without a shell.
	check.Env = stop.Env
...
// RecoverStale is called only by the exclusive vault owner on the PDF spool.
// It stops and proves absence of every specifically marked unit

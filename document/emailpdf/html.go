...
func inlineImage(ctx context.Context, out *htmlOutput, n *html.Node, message document.EmailMessageV1, parts map[string]document.EmailPartV1, open OpenArtifact) error {
	...
	scheme, cid, hasScheme := strings.Cut(src, ":")
	if hasScheme && strings.EqualFold(scheme, "data") {
		placeholder("unsupported embedded image")
		return nil
	}
	if !hasScheme || !strings.EqualFold(scheme, "cid") {

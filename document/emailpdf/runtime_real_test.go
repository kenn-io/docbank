			...
			for {
				...
				case <-ticker.C:
					...
					for _, path := range paths {
						if readyRendererUnit(filepath.Dir(path)) == "" {
							continue
						}
						cancel()
						ready <- nil
...
func readyRendererUnit(dir string) string {
	// The worker writes readiness only after verifying Browser.getVersion.
	// Its PID belongs to the private namespace and cannot index the host /proc.
	ready, err := os.ReadFile(filepath.Join(dir, "renderer-ready"))
	if err != nil {
		return ""
	}
	...
	marker, err := os.ReadFile(filepath.Join(dir, unitMarker))
	if err != nil {
		return ""
	}
	unit := string(marker)

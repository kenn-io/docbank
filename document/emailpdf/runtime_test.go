...
func TestReadOutputRefusesLinksOutsideStaging(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "message.pdf")
	content := []byte("synthetic staged output")
	...
	if runtime.GOOS == "linux" {
		// A compromised renderer can create links in /work. The host must not
		// follow those links when collecting output after the service stops.
		link := filepath.Join(t.TempDir(), "message.pdf")
		require.NoError(t, os.Symlink(path, link))
		_, err = readOutput(link)
		require.ErrorContains(t, err, "regular file")
	}
}

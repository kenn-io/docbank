...
func (client *Client) Embed(ctx context.Context, inputs []document.EmbeddingInput, authorization document.EmbeddingAuthorization) (document.EmbeddingResult, error) {
	execution, err := client.embed(ctx, inputs, authorization)
	return execution.Result, err
}

func (client *Client) embed(ctx context.Context, inputs []document.EmbeddingInput, authorization document.EmbeddingAuthorization) (Execution, error) {
	if client == nil || ctx == nil {
		return Execution{}, errors.New("openaihosted: client and context are required")
	}
	...
	if err != nil {
		return Execution{}, errors.New("openaihosted: request encoding failed")
	}
	...
	if err != nil || !validSecret(secret) {
		if contextErr := requestCtx.Err(); contextErr != nil {
			return Execution{}, fmt.Errorf("openaihosted: credential resolution canceled: %w", contextErr)
		}
		return Execution{}, errors.New("openaihosted: API-key resolution failed")
	}
	request, err := http.NewRequestWithContext(requestCtx, http.MethodPost, origin+embeddingsPath, bytes.NewReader(payload))
	if err != nil {
		return Execution{}, errors.New("openaihosted: request construction failed")
	}
	...
	if err != nil {
		if contextErr := requestCtx.Err(); contextErr != nil {
			return Execution{}, fmt.Errorf("openaihosted: request canceled: %w", contextErr)
		}
	...
	result, promptTokens, totalTokens, err := client.validateAndOrder(decoded, inputs)
	if err != nil {
		return Execution{}, err
	}
	...
	if contextErr := requestCtx.Err(); contextErr != nil {
		return Execution{}, contextErr
	}
	return Execution{Result: result, Receipt: executionReceipt(client.descriptor, promptTokens, totalTokens)}, nil
}

func (client *Client) validateAndOrder(response wireResponse, inputs []document.EmbeddingInput) (document.EmbeddingResult, int64, int64, error) {
	if response.Object != "list" || response.Model != Model || response.Usage == nil ||
		...
		*response.Usage.PromptTokens < 0 || *response.Usage.TotalTokens < *response.Usage.PromptTokens || len(response.Data) != len(inputs) {
		return document.EmbeddingResult{}, 0, 0, &ProviderError{Kind: ErrPermanentResponse}
	}
	...
	for _, item := range response.Data {
		...
		if err := client.validateVector(item.Embedding); err != nil {
			return document.EmbeddingResult{}, 0, 0, err
		}
	...
	return document.EmbeddingResult{Vectors: vectors}, *response.Usage.PromptTokens, *response.Usage.TotalTokens, nil
}

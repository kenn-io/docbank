...
func TestEmbedRejectsStrictHostedResponseDrift(t *testing.T) {
	...
	client := hostedClientReturning(t, "text/plain", valid)
	_, err := client.Embed(context.Background(), twoHostedInputs(), hostedAuthorization(client.descriptor, 2))
	require.ErrorIs(t, err, ErrPermanentResponse)
}

func TestEmbedRejectsNullEmbeddingCoordinate(t *testing.T) {
	client := hostedClientReturning(t, "application/json", `{
		"object":"list",
		"data":[{"object":"embedding","embedding":[1,null,0],"index":0}],
		"model":"text-embedding-3-large",
		"usage":{"prompt_tokens":1,"total_tokens":1}
	}`)

	result, err := client.Embed(t.Context(), oneHostedInput(), hostedAuthorization(client.descriptor, 1))
	require.ErrorIs(t, err, ErrPermanentResponse)
...
func TestEmbedRejectsLateSuccessAfterCancellation(t *testing.T) {
	...
	go func() {
		result, err := client.Embed(ctx, oneHostedInput(), hostedAuthorization(client.descriptor, 1))
		done <- struct {
			result document.EmbeddingResult
			err    error
...
func TestEmbedRejectsLateSuccessAfterAdapterDeadline(t *testing.T) {
	profile := hostedTestProfile(t)
...
func hostedClientReturning(t *testing.T, mediaType, body string) *Client {
	t.Helper()
	return newHostedTestClient(t, hostedTestProfile(t), hostedSecrets{"secret:openai": "sk-synthetic"}, roundTripFunc(func(request *http.Request) (*http.Response, error) {
		response := hostedJSONResponse(request, http.StatusOK, body)

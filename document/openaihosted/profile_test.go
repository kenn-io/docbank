...
func TestHostedDimensionAccepts3072AndRejects3073BeforeRequestAuthority(t *testing.T) {
	accepted := hostedTestProfile(t)
	accepted.Descriptor.Dimension = 3072
	accepted.Descriptor = hostedDescriptorFor(t, accepted)
	...
	rejected := accepted
	rejected.Descriptor.Dimension = 3073
	_, err = PolicyFingerprint(rejected)
	require.Error(t, err)
}

...
func New(profile Profile, secrets SecretResolver, httpClient *http.Client) (*Client, error) {
	...
	if profile.MaxSourceBytes < 0 || profile.MaxSourceBytes > maxBridgeSourceBytes ||
		profile.MaxProviderMarkdownBytes < 0 || profile.MaxProviderMarkdownBytes > maxBridgeMarkdownBytes ||
		...
		profile.MaxEvidenceUnits < 0 || profile.MaxEvidenceUnits > maxBridgeEvidenceUnits {
	...
	return &Client{
		...
		maxTotalResultBytes: profile.MaxTotalResultBytes, maxEvidenceUnits: profile.MaxEvidenceUnits,
	}, nil
}
...
func (client *Client) Render(
	...
	if err := client.validateProfileAuthorization(upload.Metadata(), authorization); err != nil {
		return document.RenditionResult{}, err
	}
...
func (client *Client) validateProfileAuthorization(
	...
	if client.maxSourceBytes > 0 && (metadata.ByteLength > client.maxSourceBytes ||
		authorization.SourceBytes > client.maxSourceBytes) ||
		...
		return classifiedError(document.RenditionErrorPolicyRejected,
			"bridge authorization exceeds provider profile ceilings", 0, nil)
	}
	return nil
}
...
func (client *Client) decodeCompleted(
	...
	if client.maxEvidenceUnits > 0 && len(evidence.Units) > client.maxEvidenceUnits {
		return document.RenditionResult{}, malformedError(
			"bridge evidence exceeds provider profile unit ceiling", nil)
	}
	var markdown []byte
	if wire.ProviderMarkdown != nil {
		...
		if err != nil {
			return document.RenditionResult{}, fmt.Errorf("bridge provider Markdown: %w", err)
		}
	...
	if client.maxArtifacts > 0 && len(wire.Artifacts) > client.maxArtifacts {
		return document.RenditionResult{}, malformedError(
			"bridge artifacts exceed provider profile count ceiling", nil)
	}
	encodedEvidence, err := json.Marshal(evidence, json.Deterministic(true))
	if err != nil {
		return document.RenditionResult{}, malformedError("bridge evidence cannot be measured", err)
	}
	totalBytes := int64(len(encodedEvidence) + len(markdown))
	artifacts := make([]document.RenditionArtifact, 0, len(wire.Artifacts))
	seenRoles := make(map[document.EvidenceArtifactRole]bool, len(wire.Artifacts))
	for _, artifact := range wire.Artifacts {
		if seenRoles[artifact.Role] {
			return document.RenditionResult{}, malformedError("bridge returned a duplicate artifact role", nil)
		}
		seenRoles[artifact.Role] = true
		payload, err := client.resolveArtifact(ctx, envelope.JobID, artifact, authorization.MaxArtifactBytes)
		if err != nil {
			return document.RenditionResult{}, err
		}
		totalBytes += int64(len(payload))
		if client.maxTotalResultBytes > 0 && totalBytes > int64(client.maxTotalResultBytes) {
			return document.RenditionResult{}, malformedError(
				"bridge result exceeds provider profile total byte ceiling", nil)
		}
	...
	if err != nil {
		return document.RenditionResult{}, fmt.Errorf("bridge: fingerprint authorization: %w", err)
	}
	...
	return document.RenditionResult{
		Evidence: evidence, ProviderMarkdown: markdown, Artifacts: artifacts, Receipt: receipt,
	}, nil
}
...
func classifiedError(
	...
	providerError, err := document.NewRenditionProviderError(code, message, retry, cause)
	if err != nil {
		fallback, fallbackErr := document.NewRenditionProviderError(
			document.RenditionErrorMalformedEvidence, "bridge returned an invalid error", 0, err)
		if fallbackErr == nil {
			return fallback
		}

...
func TestDescriptorReturnsDefensiveSlices(t *testing.T) {
	fixture := newBridgeFixture(t, http.HandlerFunc(func(http.ResponseWriter, *http.Request) {}))
	want := fixture.client.Descriptor()
	first := fixture.client.Descriptor()
	first.InputKinds[0] = document.EmbeddingInputKind("changed")
	first.SupportedRequestModes[0] = document.ModelInputMode("changed")

	assert.Equal(t, want, fixture.client.Descriptor())
}
...
func TestResponseContractFailsClosed(t *testing.T) {
	tests := []struct {
		...
		{name: "duplicate response member", contentType: responseMediaType(), body: func(manifest requestManifest) string {
			return strings.TrimSuffix(validResponseBody(manifest), "}") + `,"vectors":[]}`
		}, category: embeddingbridge.ErrorMalformedResponse},

...
func TestNormativeEmbeddingContractIsFixedStrictAndVersioned(t *testing.T) {
	...
	assert.NotEmpty(t, requestContract.Canonicalization.Booleans)
	...
	assert.Equal(t, []string{
		"provider_id", "descriptor_fingerprint", "policy_fingerprint", "max_batch_items", "max_input_bytes", "max_response_bytes", "disclose_filename",
	}, requestContract.Canonicalization.ObjectMemberOrder["$.authorization"])
		...
		payload := fmt.Sprintf(`{
			...
			"inputs":[{
				...
				"upload":{
					...
					"byte_length":1,
					"sha256":"%s",
					"capability_record_checksum":"%s",
					"provider_metadata_checksum":"%s",
			...
			"request_checksum":"%s"
		}`, strings.Repeat("a", 64), strings.Repeat("b", 64), strings.Repeat("a", 64), strings.Repeat("b", 64),
			strings.Repeat("c", 64), filename, strings.Repeat("c", 64), strings.Repeat("d", 64), strings.Repeat("e", 64), strings.Repeat("f", 64))
		var value any
		require.NoError(t, stdjson.Unmarshal([]byte(payload), &value))
		return value
	}

	require.NoError(t, validator.Validate(decode("")))
	require.Error(t, validator.Validate(decode("synthetic-source.bin")))
}
...
func serveIndependentContractRequest(
	...
	authorization, authorizationOK := manifest["authorization"].(map[string]any)
	discloseFilename, disclosureOK := authorization["disclose_filename"].(bool)
	authorizedDescriptor, authorizedDescriptorOK := authorization["descriptor_fingerprint"].(string)
	authorizedPolicy, authorizedPolicyOK := authorization["policy_fingerprint"].(string)
	maxBatchItems, maxBatchErr := independentInteger(authorization["max_batch_items"])
	maxInputBytes, maxInputErr := independentInteger(authorization["max_input_bytes"])
	maxResponseBytes, maxResponseErr := independentInteger(authorization["max_response_bytes"])
	inputs, inputsOK := manifest["inputs"].([]any)
	if !descriptorOK || !policyOK || !authorizationOK || !disclosureOK || !authorizedDescriptorOK || !authorizedPolicyOK ||
		maxBatchErr != nil || maxInputErr != nil || maxResponseErr != nil || !inputsOK || len(inputs) == 0 ||
		authorizedDescriptor != descriptorFingerprint || authorizedPolicy != policyFingerprint || int64(len(inputs)) > maxBatchItems {
		return errors.New("independent server: manifest identity")
	}
	keys := make([]string, len(inputs))
	usedFiles := make([]bool, len(files))
	var inputBytes int64
	var authorizedResponseBytes int64
	for index, rawInput := range inputs {
		...
		inputLength, inputLengthErr := independentInteger(input["byte_length"])
		if !inputOK || !keyOK || !kindOK || inputLengthErr != nil || inputLength > maxInputBytes-inputBytes {
			return errors.New("independent server: input identity")
		}
		inputBytes += inputLength
		authorizedResponseBytes += int64(len(key)) + 2*4
		keys[index] = key
	...
	if authorizedResponseBytes > maxResponseBytes {
		return errors.New("independent server: response authorization")
	}

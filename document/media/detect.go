...
func sniff(data []byte) (Metadata, error) {
	...
	case bytes.HasPrefix(data, []byte("\xff\xd8\xff")):
		return sniffImage(data, FormatJPEG, "image/jpeg")
	case bytes.HasPrefix(data, []byte("\x89PNG\r\n\x1a\n")):
		return sniffImage(data, FormatPNG, "image/png")
	case len(data) >= 12 && string(data[:4]) == "RIFF" && string(data[8:12]) == "WEBP":
		width, height, ok := webPDimensions(data)
		if !ok {
			return Metadata{}, ErrMalformedMedia
		}
		return Metadata{Format: FormatWebP, Kind: KindImage, MediaType: "image/webp", Width: width, Height: height}, nil

...
func mp4Metadata(data []byte) (mp4Info, bool) {
	...
	info := mp4Info{mediaDataRanges: mediaDataRanges, source: data, localDataReferences: true}
	...
	info.sampleAuthority = info.sampleAuthority && info.localDataReferences && info.mdatCount > 0 &&
...
type mp4TrackInfo struct {
	dinfCount, drefCount                             int
	dataReferenceCount                               uint32
	dataReferencesLocal                              bool
...
func scanMP4Boxes(data []byte, info *mp4Info, track *mp4TrackInfo, parent string, depth int) bool {
	...
	for offset := 0; offset < len(data); {
		...
		case kind == "dinf" && parent == "minf":
			if track == nil {
				return false
			}
			track.dinfCount++
			track.dataReferencesLocal = parseDINF(payload, track)
		case kind == "dinf" || kind == "dref":
			info.localDataReferences = false
...
// Data references establish which file owns chunk offsets. Metadata-only
// detection may still read dimensions without granting local sample authority.
func parseDINF(payload []byte, track *mp4TrackInfo) bool {
	for offset := 0; offset < len(payload); {
		header, size, ok := mp4BoxHeader(payload, offset)
		if !ok {
			return false
		}
		if string(payload[offset+4:offset+8]) == "dref" {
			track.drefCount++
			count, valid := localDataReferenceCount(payload[offset+header : offset+size])
			if !valid {
				return false
			}
			track.dataReferenceCount = count
		}
		offset += size
	}
	return true
...
func localDataReferenceCount(payload []byte) (uint32, bool) {
	if len(payload) < 8 || binary.BigEndian.Uint32(payload[:4]) != 0 {
		return 0, false
	}
	...
	for offset := 8; offset < len(payload); {
		...
		entry := payload[offset+header : offset+size]
		// Version 0, self-contained flag only, and no external location or alias.
		if string(payload[offset+4:offset+8]) != "url " || len(entry) != 4 || binary.BigEndian.Uint32(entry) != 1 {
			return 0, false
		}
		count++
		offset += size
	}
	return count, count > 0 && count == want
}

func (track *mp4TrackInfo) hasLocalDataReferences() bool {
	if track.dinfCount != 1 || track.drefCount != 1 || !track.dataReferencesLocal ||
		track.stsdCount != 1 || len(track.sampleDescriptions) == 0 {
		return false
	}
	for _, description := range track.sampleDescriptions {
		if description.dataReferenceIndex == 0 || uint32(description.dataReferenceIndex) > track.dataReferenceCount {
			return false
		}
	}
	return true
...
func parseSTSD(payload []byte, track *mp4TrackInfo) bool {
	...
	for offset := 0; offset < len(entries); {
		...
		if len(entry) >= 8 {
			referenceIndex = binary.BigEndian.Uint16(entry[6:8])
		}
		if isVisualSampleEntry(kind) {
			...
			configuration.dataReferenceIndex = referenceIndex
			track.sampleDescriptions = append(track.sampleDescriptions, configuration)
		} else {
			track.unknownSampleEntries++
			track.sampleDescriptions = append(track.sampleDescriptions, mp4CodecConfiguration{dataReferenceIndex: referenceIndex})
		}
		offset += size
	}
...
func (track *mp4TrackInfo) finish(info *mp4Info) bool {
	// Every track belongs to the uploaded original, including audio tracks.
	// Keep this separate from metadata extraction and visual sample accounting.
	info.localDataReferences = info.localDataReferences && track.hasLocalDataReferences()
...
func mp4SampleRangesDoNotOverlap(samples []mp4SampleRange) bool {
	...
	slices.SortFunc(samples, func(left, right mp4SampleRange) int {

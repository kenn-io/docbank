...
func (scanner *generationJSONPreflight) object(path generationJSONPath, depth int) error {
	...
	for fields := 1; ; fields++ {
		...
		if bytes.ContainsAny(scanner.data[keyStart:keyEnd], "ABCDEFGHIJKLMNOPQRSTUVWXYZ") {
			return errors.New("embedding generation JSON field names must use canonical lowercase spelling")
		}
		scanner.skipSpace()
		if !scanner.consume(':') {
...
func BuildEmbeddingInputs(evidence NormalizedEvidenceV1, policy InputPolicy) (EmbeddingInputGeneration, error) {
	...
	for start := 0; start < len(tokens); {
		...
		if desiredRuneStart <= tokens[start].start {
...
func candidateFitsByteLimits(evidence NormalizedEvidenceV1, tokens []embeddingToken, policy InputPolicy, attachment AttachmentContextSnapshot, limits inputConstructionLimits) (bool, error) {
	...
	contentBytes, err := contentByteLength(evidence, spans, int64(^uint64(0)>>1))
	if err != nil {
		return false, err
	}
	contentRunes := int64(tokens[len(tokens)-1].end - tokens[0].start)
	if err := limits.work.consume([]int64{contentBytes}, []int64{contentRunes}); err != nil {
		return false, err
	}

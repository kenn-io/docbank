...
func TestEmbedDocumentsEnforcesCapabilityAuthorization(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "text needs interleaved", inputs: []voyage.Input{{Parts: []voyage.Part{{Text: "context"}, {Media: png}}}}, authorizations: pngOnly, want: voyage.ErrCapabilityContract, wantText: voyage.CapabilityInterleavedPNG},
		{name: "interleaved video passes when probed", inputs: []voyage.Input{{Parts: []voyage.Part{{Text: "context"}, {Media: mp4}}}}, authorizations: fullAuthorizations(t, policy)},
		{name: "interleaved video needs its capability", inputs: []voyage.Input{{Parts: []voyage.Part{{Text: "context"}, {Media: mp4}}}}, authorizations: fullAuthorizations(t, policy, voyage.CapabilityVideoMP4, voyage.CapabilityBatchLimits), want: voyage.ErrCapabilityContract, wantText: voyage.CapabilityInterleavedMP4},
		...
		{name: "mixed-format batch passes when probed", inputs: []voyage.Input{{Parts: []voyage.Part{{Media: mp4}}}, {Parts: []voyage.Part{{Media: png}}}}, authorizations: fullAuthorizations(t, policy)},
		{name: "interleaved member in a batch passes when probed", inputs: []voyage.Input{{Parts: []voyage.Part{{Text: "context"}, {Media: png}}}, {Parts: []voyage.Part{{Media: png}}}}, authorizations: fullAuthorizations(t, policy)},

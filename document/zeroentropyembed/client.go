...
func (client *Client) embed(ctx context.Context, inputs []document.EmbeddingInput, authorization document.EmbeddingAuthorization, includeReceipt bool) (Execution, error) {
	...
	validatedInputs := cloneEmbeddingInputs(inputs)
	...
	prepared, err := client.prepareRequests(validatedInputs)
	if err != nil {
		return Execution{}, err
	}
	defer func() {
	...
	for _, request := range prepared {
		...
		for local, global := range request.positions {
			result.Vectors[global] = document.EmbeddingVector{Key: validatedInputs[global].Key, Values: vectors[local]}
		}
	...
	if err := document.ValidateEmbeddingProviderResult(client.descriptor, validatedInputs, authorization, result); err != nil {
		return Execution{}, &ProviderError{Kind: ErrPermanentResponse}
	}
...
func cloneEmbeddingInputs(inputs []document.EmbeddingInput) []document.EmbeddingInput {
	cloned := slices.Clone(inputs)
	for index := range cloned {
		cloned[index].HeadingPath = slices.Clone(inputs[index].HeadingPath)
		cloned[index].SourceSpans = slices.Clone(inputs[index].SourceSpans)
	}
	return cloned
}

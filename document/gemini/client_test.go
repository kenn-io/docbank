...
// Network causes must remain inspectable without appearing in diagnostic text.
func TestEmbedClassifiesTransportFailures(t *testing.T) {
	networkError := errors.New("synthetic sensitive transport detail")
	for _, tc := range []struct {
		name        string
		cause, want error
	}{
		{"network", networkError, ErrTransientResponse},
		{"destination policy", providerhttp.ErrDestinationDenied, ErrPermanentResponse},
		{"address policy", providerhttp.ErrAddressDenied, ErrPermanentResponse},
		...
		t.Run(tc.name, func(t *testing.T) {
			profile := geminiTestProfile(t, 128)
			client := newGeminiTestClient(t, profile, syntheticSecrets{"secret:gemini": "synthetic-key"}, roundTripFunc(func(*http.Request) (*http.Response, error) { return nil, tc.cause }))
			_, err := client.Embed(t.Context(), []document.EmbeddingInput{{Key: "document-1", Role: document.EmbeddingRoleDocument, Kind: document.EmbeddingInputRenditionChunk, Text: "passage"}}, geminiSingleAuthorization(profile.Descriptor))
			require.ErrorIs(t, err, tc.want)
			require.ErrorIs(t, err, tc.cause)
			assert.NotContains(t, err.Error(), networkError.Error())
			_, retrySet := RetryAfter(err)
			assert.False(t, retrySet)
			// Files API requests use the same sanitized classification.
			failure := requestFailure(t.Context(), "upload finalize", tc.cause)
			require.ErrorIs(t, failure, tc.want)
			require.ErrorIs(t, failure, tc.cause)
			assert.NotContains(t, failure.Error(), networkError.Error())
		})
	}
}

// The provider samples at most 32 frames; it does not restrict source frames.
func TestGeminiVideoCapabilityUsesDurationNotSamplingCount(t *testing.T) {
	for _, mime := range []string{"video/mp4", "video/quicktime"} {
		facts := document.UploadCapabilityFacts{MediaFamily: "video", MediaType: mime, DurationMS: 10_000, Frames: 300, MaxDurationMS: 120_000, MaxFrames: 3600}
		assert.True(t, geminiCapabilitySupported(facts), mime)
		facts.DurationMS = 120_000
		assert.True(t, geminiCapabilitySupported(facts), mime)
		facts.DurationMS = 120_001
		assert.False(t, geminiCapabilitySupported(facts), mime)
	}
}

// A failed body read is a transport failure even after successful HTTP headers.
func TestEmbedClassifiesInterruptedResponseBody(t *testing.T) {
	profile := geminiTestProfile(t, 128)
	client := newGeminiTestClient(t, profile, syntheticSecrets{"secret:gemini": "synthetic-key"}, roundTripFunc(func(request *http.Request) (*http.Response, error) {

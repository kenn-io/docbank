...
// A finalize failure is terminal even if a later poll would claim ACTIVE.
func TestEmbedFilesAPIRejectsFailedFinalizeAndDeletesFile(t *testing.T) {
	for _, withError := range []bool{false, true} {
		t.Run("error_object_"+strconv.FormatBool(withError), func(t *testing.T) {
			data := geminiTinyPNG(t)
			profile := geminiDirectTestProfile(t, TransportFilesAPI)
			record := geminiCapability(t, profile, data, "synthetic.png", "image/png")
			source := newGeminiLifecycleUpload(data, record)
			...
			fileURI := origin + "/v1beta/" + fileName
			timeline := newGeminiFileTimeline()
			var polls, embeds, deletes atomic.Int32
			client := newGeminiTestClient(t, profile, syntheticSecrets{"secret:gemini": "synthetic-key"}, roundTripFunc(func(request *http.Request) (*http.Response, error) {
				switch {
				case request.Header.Get("X-Goog-Upload-Command") == "start":
					response := geminiJSONResponse(request, `{}`)
					response.Header.Set("X-Goog-Upload-Url", origin+"/upload/v1beta/files?upload_id=synthetic-session-123&upload_protocol=resumable")
					return response, nil
				case request.Header.Get("X-Goog-Upload-Command") == "upload, finalize":
					fileJSON := geminiFileJSON(record, fileName, fileURI, "FAILED", timeline)
					if withError {
						fileJSON = strings.TrimSuffix(fileJSON, "}") + `,"error":{"code":3,"message":"synthetic processing failure","details":[]}}`
					}
					response := geminiJSONResponse(request, `{"file":`+fileJSON+`}`)
					response.Header.Set("X-Goog-Upload-Status", "final")
					return response, nil
				case request.Method == http.MethodGet:
					polls.Add(1)
					return geminiJSONResponse(request, geminiFileJSON(record, fileName, fileURI, "ACTIVE", timeline)), nil
				case request.Method == http.MethodPost && request.URL.Path == embedPath:
					embeds.Add(1)
					return geminiJSONResponse(request, `{"embedding":{"values":[`+testVectorJSON(128)+`]}}`), nil
				case request.Method == http.MethodDelete:
					...
					return geminiJSONResponse(request, `{}`), nil
				default:
					return nil, errors.New("unexpected failed-file lifecycle request")
				}
			}))
...
// TestEmbedFilesAPIBoundsEveryLifecycleBody checks that oversized start,
// finalize, poll, embed, and delete responses retain their capacity classification.
func TestEmbedFilesAPIBoundsEveryLifecycleBody(t *testing.T) {
	for _, stage := range []string{"start", "finalize", "poll", "embed", "delete"} {
		t.Run(stage, func(t *testing.T) {
			...
			wantRequests := map[string]int32{"start": 1, "finalize": 2, "poll": 4, "embed": 4, "delete": 4}[stage]
		...
		t.Run(fixture.name, func(t *testing.T) {
			...
			for _, transport := range []Transport{TransportInline, TransportFilesAPI} {
				t.Run(string(transport), func(t *testing.T) {
					...
					source, err := upload.Authorize(t.Context(), upload.Source{Reader: io.NopCloser(bytes.NewReader(fixture.data)), Directory: t.TempDir()}, record, upload.UploadMetadata{Filename: fixture.filename})
					require.NoError(t, err)
					...
					_, err = document.ExecuteEmbedding(t.Context(), client, directInputs(source), geminiDirectAuthorization(profile.Descriptor, int64(len(fixture.data))))
					require.NoError(t, err)
					if transport == TransportInline {
						assert.Equal(t, int32(1), requests.Load())
					} else {
...
func geminiCapability(t *testing.T, profile Profile, data []byte, filename, mediaType string) media.CapabilityRecord {
	...
	record, err := media.InspectCapability(bytes.NewReader(data), media.InspectionPolicy{
		...
		MaxPixels: 16_000_000, MaxFrames: 3600, MaxDurationMS: maxDuration,
	})
	require.NoError(t, err)
...
func (upload *geminiLifecycleUpload) CapabilityProof() document.UploadCapability {
	upload.capabilityCalls.Add(1)
	return upload.record.UploadCapability()
}
...
func TestExecuteEmbeddingRejectsMissingOrMismatchedLocalProof(t *testing.T) {
	for _, scenario := range []string{"missing", "portable", "changed", "mismatched"} {
		t.Run(scenario, func(t *testing.T) {
			data := geminiTinyPNG(t)
			profile := geminiDirectTestProfile(t, TransportInline)
			record := geminiCapability(t, profile, data, "synthetic.png", "image/png")
			source := newGeminiLifecycleUpload(data, record)
			var input document.AuthorizedUpload = source
			switch scenario {
			case "missing":
				input = struct{ document.AuthorizedUpload }{source}
			case "portable":
				encoded, err := json.Marshal(record)
				require.NoError(t, err)
				require.NoError(t, json.Unmarshal(encoded, &source.record))
			case "changed":
				source.record.Measurements.Pixels++
			case "mismatched":
				source.metadata[0].CapabilityRecordChecksum = strings.Repeat("f", 64)
			}
			secrets := &countingSecrets{value: "synthetic-key"}
			var requests atomic.Int32
			client := newGeminiTestClient(t, profile, secrets, roundTripFunc(func(*http.Request) (*http.Response, error) {
			...
			_, err := document.ExecuteEmbedding(t.Context(), client, directInputs(input), geminiDirectAuthorization(profile.Descriptor, int64(len(data))))
			require.Error(t, err)
			assert.Zero(t, source.readPasses.Load())
			assert.Zero(t, secrets.calls.Load())
			assert.Zero(t, requests.Load())
...
// Inline encoding must release raw bytes before the first network request.
func TestEmbedClearsInlineSourceBeforeSending(t *testing.T) {
	profile := geminiDirectTestProfile(t, TransportInline)
	data := geminiTinyPNG(t)
	source := newGeminiLifecycleUpload(data, geminiCapability(t, profile, data, "synthetic.png", "image/png"))
	source.captureReadBuffer = true
	client := newGeminiTestClient(t, profile, syntheticSecrets{"secret:gemini": "synthetic-key"}, roundTripFunc(func(request *http.Request) (*http.Response, error) {
		require.Len(t, source.capturedReadBuffer, len(data))
		assert.Equal(t, make([]byte, len(data)), source.capturedReadBuffer)
		payload, err := io.ReadAll(request.Body)
		require.NoError(t, err)
		assert.Contains(t, string(payload), base64.StdEncoding.EncodeToString(data))
		return geminiJSONResponse(request, `{"embedding":{"values":[`+testVectorJSON(128)+`]}}`), nil
	}))
	_, err := client.Embed(t.Context(), directInputs(source), geminiDirectAuthorization(profile.Descriptor, int64(len(data))))
	require.NoError(t, err)
...
// A copied record with recomputed public checksums must not authorize bytes
// that never passed inspection, even through a custom AuthorizedUpload.
func TestExecuteEmbeddingRejectsRechecksummedInspectionForReplacementBytes(t *testing.T) {
	for _, transport := range []Transport{TransportInline, TransportFilesAPI} {
		t.Run(string(transport), func(t *testing.T) {
			profile := geminiDirectTestProfile(t, transport)
			record := geminiCapability(t, profile, geminiTinyPNG(t), "synthetic.png", "image/png")
			replacement := []byte("synthetic uninspected bytes")
			digest := sha256.Sum256(replacement)
			record.SourceSHA256 = hex.EncodeToString(digest[:])
			record.SourceBytes = int64(len(replacement))
			record.Policy.ExpectedSHA256 = record.SourceSHA256
			record.Policy.ExpectedBytes = record.SourceBytes
			encoded, err := json.Marshal(record.Policy, json.Deterministic(true))
			require.NoError(t, err)
			digest = sha256.Sum256(encoded)
			record.PolicyFingerprint = hex.EncodeToString(digest[:])
			record.Checksum = ""
			encoded, err = json.Marshal(record, json.Deterministic(true))
			require.NoError(t, err)
			digest = sha256.Sum256(encoded)
			record.Checksum = hex.EncodeToString(digest[:])
			require.NoError(t, media.ValidateCapabilityRecord(record), "portable checksums are consistent")

			source := newGeminiLifecycleUpload(replacement, record)
			secrets := &countingSecrets{value: "synthetic-key"}
			var requests atomic.Int32
			client := newGeminiTestClient(t, profile, secrets, roundTripFunc(func(request *http.Request) (*http.Response, error) {
				...
				if transport == TransportInline {
					...
					assert.Contains(t, string(body), base64.StdEncoding.EncodeToString(replacement))
					return geminiJSONResponse(request, `{"embedding":{"values":[`+testVectorJSON(128)+`]}}`), nil
				}
				return nil, errors.New("unexpected upload request")
			}))
			_, err = document.ExecuteEmbedding(t.Context(), client, directInputs(source), geminiDirectAuthorization(profile.Descriptor, int64(len(replacement))))
			require.Error(t, err)
			assert.Zero(t, source.readPasses.Load())
			assert.Zero(t, secrets.calls.Load())
			assert.Zero(t, requests.Load())
...
// Both public APIs must report failed deletion, including when embedding also
// fails; the receipt supplements the error rather than hiding cleanup failure.
func TestFilesAPICleanupFailureIsObservable(t *testing.T) {
	for _, withReceipt := range []bool{false, true} {
		for _, embedFails := range []bool{false, true} {
			t.Run("receipt_"+strconv.FormatBool(withReceipt)+"/embed_failure_"+strconv.FormatBool(embedFails), func(t *testing.T) {
				data := geminiTinyPNG(t)
				profile := geminiDirectTestProfile(t, TransportFilesAPI)
				record := geminiCapability(t, profile, data, "synthetic.png", "image/png")
				source := newGeminiLifecycleUpload(data, record)
				fileName := "files/file-123"
				active := geminiFileJSON(record, fileName, origin+"/v1beta/"+fileName, "ACTIVE", newGeminiFileTimeline())
				...
				var err error
				if withReceipt {
					execution, executeErr := client.EmbedWithReceipt(t.Context(), directInputs(source), geminiDirectAuthorization(profile.Descriptor, int64(len(data))))
					result, err = execution.Result, executeErr
					assert.Equal(t, []string{"provider file deletion attempt failed"}, execution.Receipt.Warnings)
				} else {
					result, err = client.Embed(t.Context(), directInputs(source), geminiDirectAuthorization(profile.Descriptor, int64(len(data))))
				}
				require.ErrorIs(t, err, ErrTransientResponse)

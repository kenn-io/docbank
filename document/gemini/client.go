...
// EmbedWithReceipt returns request provenance even when execution or cleanup fails.
func (client *Client) EmbedWithReceipt(ctx context.Context, inputs []document.EmbeddingInput, authorization document.EmbeddingAuthorization) (Execution, error) {
	...
	return Execution{Result: result, Receipt: receipt}, err
}
...
func (client *Client) embed(ctx context.Context, inputs []document.EmbeddingInput, authorization document.EmbeddingAuthorization, receipt *Receipt) (document.EmbeddingResult, error) {
	...
	prepared, err := client.prepareRequests(requestCtx, frozenInputs, authorization, sourceGate)
	if err != nil {
		return document.EmbeddingResult{}, err
	}
...
func (client *Client) prepareRequests(ctx context.Context, inputs []document.EmbeddingInput, authorization document.EmbeddingAuthorization, gate *activeSourceGate) (prepared []preparedInput, retErr error) {
	...
	for index, input := range inputs {
		...
		case input.Role == document.EmbeddingRoleDocument && input.Kind == document.EmbeddingInputOriginalFile:
			prepared[index], retErr = client.prepareOriginal(ctx, input, authorization, gate)
			if retErr != nil {
				return prepared, retErr
			}
			continue
		default:
			return prepared, errors.New("gemini embed: unsupported input role or kind")
		}
		if int64(len(text)) > client.profile.MaxInputBytes-inputBytes {
			return prepared, errors.New("gemini embed: embedding input exceeds profile byte capacity")
		}
		inputBytes += int64(len(text))
		prepared[index].payload, retErr = client.marshalRequest(wirePart{Text: text})
		if retErr != nil {
			return prepared, retErr
		}
	}
...
func (client *Client) prepareOriginal(ctx context.Context, input document.EmbeddingInput, authorization document.EmbeddingAuthorization, gate *activeSourceGate) (preparedInput, error) {
	frozen, ok := input.Source.(*frozenUpload)
	if !ok {
		return preparedInput{}, errors.New("gemini embed: original upload was not frozen")
	}
	...
	verified, err := client.readDirectFile(ctx, frozen.enrolled, gate)
	if err != nil {
		return preparedInput{}, err
	}
	if client.profile.Transport == TransportFilesAPI {
		return preparedInput{file: &verified}, nil
	}
	defer clear(verified.data)
	encoded := base64.StdEncoding.EncodeToString(verified.data)
	payload, err := client.marshalRequest(wirePart{InlineData: &wireInlineData{MIMEType: verified.metadata.MediaType, Data: encoded}})
	if err != nil {
		return preparedInput{}, err
	}
	limit := int64(100 << 20)
	...
	if int64(len(payload)) > limit {
		clear(payload)
		return preparedInput{}, errors.New("gemini embed: inline request exceeds provider byte capacity")
	}
	return preparedInput{payload: payload}, nil
}
...
func (upload *enrolledUpload) liveCapability() (document.UploadCapability, bool) {
	carrier, ok := upload.source.(capabilityCarrier)
	if !ok {
		return document.UploadCapability{}, false
	}
	return carrier.CapabilityProof(), true
}
...
func (upload *frozenUpload) CapabilityProof() document.UploadCapability {
...
func enrollOriginalUploads(inputs []document.EmbeddingInput) ([]document.EmbeddingInput, []*enrolledUpload, error) {
	...
	for _, upload := range enrolled {
		...
		if carrier, ok := upload.source.(capabilityCarrier); ok {
			upload.capability = carrier.CapabilityProof()
...
func (client *Client) validateDirectCapability(upload *enrolledUpload, authorization document.EmbeddingAuthorization) error {
	...
	record, local := upload.capability.Facts()
	if !local {
		return errors.New("gemini embed: direct-file capability record is not locally eligible")
	}
	metadata := upload.metadata
	if metadata.MediaFamily != record.MediaFamily || metadata.MediaType != record.MediaType ||
		metadata.ByteLength != record.SourceBytes || metadata.SHA256 != record.SourceSHA256 ||
		metadata.CapabilityRecordChecksum != record.Checksum || metadata.InputKind != document.RenditionInputOriginalFile ||
	...
	if record.SourceBytes > authorization.MaxInputBytes || record.SourceBytes > client.profile.MaxInputBytes ||
		record.MaxSourceBytes > client.profile.MaxInputBytes {
	...
	if !geminiCapabilitySupported(record) {
		return errors.New("gemini embed: direct-file capability is unsupported or over limit")
	}
	return nil
}

func geminiCapabilitySupported(record document.UploadCapabilityFacts) bool {
	switch record.MediaFamily {
	case "image":
		return (record.MediaType == "image/png" || record.MediaType == "image/jpeg") &&
			record.Pixels > 0 && record.Frames > 0 &&
			record.MaxPixels > 0 && record.MaxFrames > 0
	case "audio":
		return (record.MediaType == "audio/wav" || record.MediaType == "audio/mpeg") &&
			record.DurationMS > 0 && record.DurationMS <= 180_000 &&
			record.MaxDurationMS > 0 && record.MaxDurationMS <= 180_000
	case "video":
		return (record.MediaType == "video/mp4" || record.MediaType == "video/quicktime") &&
			record.DurationMS > 0 && record.DurationMS <= 120_000 &&
			record.MaxDurationMS > 0 && record.MaxDurationMS <= 120_000
	case "pdf":
		return record.MediaType == "application/pdf" && record.Pages > 0 &&
			record.Pages <= 6 && record.MaxPages > 0 && record.MaxPages <= 6
	default:
		return false
	}
}
...
func (client *Client) readDirectFile(ctx context.Context, upload *enrolledUpload, gate *activeSourceGate) (verifiedFile, error) {
	...
	if !ok {
		if contextErr := ctx.Err(); contextErr != nil {
			return verifiedFile{}, fmt.Errorf("gemini embed: source read canceled: %w", contextErr)
		}
...
func (client *Client) executeFile(ctx context.Context, file verifiedFile, secret string, receipt *Receipt) (vector []float32, retErr error) {
	...
	defer func() {
		...
		defer cancel()
		if deleteErr := client.deleteFile(cleanupCtx, created.file.Name, secret, receipt); deleteErr != nil {
			retErr = errors.Join(retErr, fmt.Errorf("gemini embed: provider file deletion failed: %w", deleteErr))
			if receipt != nil {
...
func (client *Client) waitForActiveFile(ctx context.Context, current validatedProviderFile, file verifiedFile, secret string, receipt *Receipt) (validatedProviderFile, error) {
	...
	if current.file.State != "PROCESSING" {
		return validatedProviderFile{}, &ProviderError{Kind: ErrPermanentResponse}
	}
...
func validateWireFile(value wireFile, expected verifiedFile) (validatedProviderFile, bool) {
	// FAILED may carry a processing error. Retain its verified identity so
	// executeFile installs cleanup before waitForActiveFile rejects the state.
	if !validFileName(value.Name) || value.URI != origin+"/v1beta/"+value.Name ||
		value.MIMEType != expected.metadata.MediaType || value.Source != "UPLOADED" ||
		value.Error != nil && value.State != "FAILED" ||
...
func requestFailure(ctx context.Context, operation string, cause error) error {
	if contextErr := ctx.Err(); contextErr != nil {
		return fmt.Errorf("gemini embed: %s canceled: %w", operation, &ProviderError{Kind: contextErr, cause: cause})
	}
	...
	case errors.Is(cause, providerhttp.ErrDestinationDenied), errors.Is(cause, providerhttp.ErrAddressDenied), errors.Is(cause, providerhttp.ErrCertificatePin):
		kind = ErrPermanentResponse
	}
	return fmt.Errorf("gemini embed: %s failed: %w", operation, &ProviderError{Kind: kind, cause: cause})
}
...
func beginReceiptRequest(receipt *Receipt) bool {
	...
	if receipt.RequestCount == math.MaxInt {
		return false
	}
...
func (client *Client) execute(ctx context.Context, payload []byte, secret string, receipt *Receipt) ([]float32, error) {
	...
	response, err := client.http.Do(request)
	if err != nil {
		return nil, requestFailure(ctx, "provider request", err)
	}
	defer func() { _ = response.Body.Close() }()
	if response.StatusCode != http.StatusOK {
		return nil, statusError(response.StatusCode, response.Header.Get("Retry-After"), time.Now().UTC())
...
func readBounded(ctx context.Context, body io.Reader, maximum int64) ([]byte, error) {
	...
	if err != nil {
		clear(data)
		return nil, requestFailure(ctx, "response read", err)
	}

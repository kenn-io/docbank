...
func TestStatusErrorClassifiesGeminiResponses(t *testing.T) {
	for _, testCase := range []struct {
		...
		{name: "unauthorized", status: http.StatusUnauthorized, retryAfter: "2", want: ErrUnauthorized},
		{name: "forbidden", status: http.StatusForbidden, retryAfter: "2", want: ErrUnauthorized},
		{name: "unavailable", status: http.StatusServiceUnavailable, retryAfter: "2", want: ErrTransientResponse, delay: 2 * time.Second, delaySet: true},
		{name: "capacity", status: http.StatusRequestEntityTooLarge, retryAfter: "2", want: ErrCapacityResponse},
		{name: "invalid request", status: http.StatusBadRequest, retryAfter: "2", want: ErrPermanentResponse},
	} {

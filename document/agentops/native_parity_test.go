...
func TestNativeDocumentOperationsPublishReviewedCLIMCPContracts(t *testing.T) {
	for _, check := range []struct {
		...
		{"agent_capabilities", "native_documents", "empty object", "agentops.Capabilities", "none", 0, 0, true},
		...
		{"format_coverage", "native_formats", "ReadFormatCapabilitiesQuery", "api.FormatCoverageResponse", "none", 0, 0, true},
	} {
		t.Run(check.id, func(t *testing.T) {

...
func CurrentOperations() []Operation {
	operations := []Operation{
		...
		currentOperation("agent_capabilities", []string{"GET /api/v1/agent/capabilities"}, "agent capabilities", "get_agent_capabilities", false),
		...
		currentOperation("list_tags", []string{"GET /api/v1/tags"}, "tag list", "list_tags", false),
		...
		currentOperation("list_processing_profiles", []string{"GET /api/v1/processing/profiles"}, "processing profiles", "list_processing_profiles", false),
		...
		currentOperation("export_plan_read", []string{"GET /api/v1/exports/plans/{id}"}, "export show-plan", "get_export_plan", false),
		currentOperation("export_output_problems", []string{"GET /api/v1/exports/plans/{id}/problems"}, "export problems", "list_export_output_problems", false),
	...
	for i := range operations {
		...
		reviewNativeDocumentOperation(&operations[i])
...
// reviewNativeDocumentOperation describes the already bounded CLI and MCP
// adapters for the retained document and processing workflow. ResponseBytes
// is the MCP result cap; the daemon may return a larger paged HTTP response.
func reviewNativeDocumentOperation(operation *Operation) {
	if operation == nil {
		return
	}
	var input, output string
	bound := Bounds{Paging: "none", ResponseBytes: 1 << 20}
	feature := "native_documents"
	idempotent := true
	switch operation.ID {
	case "agent_capabilities":
		input, output = "empty object", "agentops.Capabilities"
	case "list_documents":
		input, output = "api.DocumentQuery|api.ScopedDocumentQuery", "api.DocumentPage"
		bound = Bounds{Paging: "cursor", Default: 50, Maximum: 250, ResponseBytes: 1 << 20}
	case "list_tags":
		input, output = "ListTagsQuery", "api.TagPage"
		bound = Bounds{Paging: "offset", Default: 100, Maximum: 250, ResponseBytes: 1 << 20}
	case "search_documents":
		input, output = "api.DocumentSourceFenceResolveRequest|api.DocumentSearchRequest", "api.DocumentSearchReport"
		bound = Bounds{Paging: "none", ResponseBytes: 1 << 20}
	case "get_document":
		input, output = "GetNodePath", "api.Node"
	case "list_document_versions":
		input, output = "ListContentVersionsQuery", "api.ContentVersionPage"
		bound = Bounds{Paging: "offset", Default: 100, Maximum: 250, ResponseBytes: 1 << 20}
	case "read_rendition_text":
		input, output = "api.RenditionWindowRequest", "api.RenditionTextWindow"
		bound = Bounds{Paging: "none", ResponseBytes: 1 << 20}
	case "get_processing_plan":
		input, output = "api.ProcessingPlanRequest", "api.ProcessingPlan"
		feature = "native_processing"
	case "get_processing_status":
		input, output = "GetDocumentProcessingJobPath", "api.ProcessingStatus"
		feature = "native_processing"
	case "get_processing_coverage":
		input, output = "GetDocumentProcessingCoverageQuery", "api.CoverageReport"
		feature = "native_processing"
	case "list_processing_profiles":
		input, output = "empty object", "[]api.ProcessingProfileSummary"
		feature = "native_processing"
	case "start_processing":
		input, output = "api.StartProcessingRequest", "api.ProcessingJobEvent"
		feature, idempotent = "native_processing", false
	case "format_coverage":
		input, output = "ReadFormatCapabilitiesQuery", "api.FormatCoverageResponse"
		feature = "native_formats"
	default:
		return
	}
	...
	operation.Idempotent, operation.Destructive = idempotent, false
}
...
func reviewNativeReportExportOperation(operation *Operation) {
	...
	case "export_plan_preview":
		...
		feature, idempotent = "native_exports", true
	case "export_plan_read":
		input, output = "GetExportPlanPath", "bundle.Plan"
		feature, idempotent = "native_exports", true
	case "export_output_problems":
		input, output = "GetExportOutputProblemsQuery", "bundle.OutputProblems"
		feature, idempotent = "native_exports", true
		bound = Bounds{Paging: "offset", Default: bundle.InspectionPageSize, Maximum: bundle.InspectionPageSize, ResponseBytes: bundle.MaxMemberBytes}
	case "export_job_start":

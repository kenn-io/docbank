...
func TestAgentCurrentOperationsExposeRealBindingsAndGaps(t *testing.T) {
	...
	catalog := operationByID(t, operations, "list_documents")
	if !slices.Contains(catalog.CLIPaths, "documents list") ||
		!slices.Contains(catalog.RouteIDs, "POST /api/v1/documents/scoped") || hasSurfaceGap(catalog, "cli") {
		t.Fatalf("scoped document catalog CLI binding missing: %+v", catalog)
	}
	for _, binding := range []struct{ operation, cli string }{
		{"read_rendition_text", "rendition text"},
		{"get_processing_coverage", "processing coverage"},
	} {
		operation := operationByID(t, operations, binding.operation)
		if !slices.Contains(operation.CLIPaths, binding.cli) || hasSurfaceGap(operation, "cli") {
			t.Fatalf("native CLI binding missing for %s: %+v", binding.operation, operation)
		}
	}
	formats := operationByID(t, operations, "format_coverage")
	if !slices.Contains(formats.CLIPaths, "formats") ||
		!slices.Contains(formats.MCPTools, "get_format_coverage") || hasSurfaceGap(formats, "mcp") {
		t.Fatal("format coverage MCP binding missing")
	}
	...
	if hasSurfaceGap(artifact, "cli") || hasSurfaceGap(artifact, "mcp") ||
		!slices.Equal(artifact.MCPTools, []string{"open_report_artifact", "download_report_artifact"}) {
		t.Fatal("implemented report artifact bindings missing")
	}
}
...
func TestAgentMultiRouteArtifactIsOneDomainOperation(t *testing.T) {
	...
	if hasSurfaceGap(artifact, "cli") || hasSurfaceGap(artifact, "mcp") ||
		!slices.Equal(artifact.MCPTools, []string{"open_report_artifact", "download_report_artifact"}) {
		t.Fatal("one domain operation does not own both artifact adapters")
	}
}

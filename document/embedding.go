...
func NewEmbeddingDescriptor(value EmbeddingDescriptor) (EmbeddingDescriptor, error) {
	...
	encoded, err := canonicalJSON(embeddingDescriptorIdentity(value))
	if err != nil {
...
func ValidateEmbeddingProviderRequest(provider EmbeddingProvider, inputs []EmbeddingInput, authorization EmbeddingAuthorization) error {
	...
	if next := cloneEmbeddingDescriptor(provider.Descriptor()); !equalEmbeddingDescriptors(descriptor, next) {
		return errors.New("embedding descriptor changed during validation")
	}
	if err := validateEmbeddingAuthorization(descriptor, authorization); err != nil {
		return err
	}
	if err := validateEmbeddingInputs(descriptor, inputs, authorization); err != nil {
		return err
	}
...
func ExecuteEmbedding(ctx context.Context, provider EmbeddingProvider, inputs []EmbeddingInput, authorization EmbeddingAuthorization) (EmbeddingResult, error) {
	if err := ValidateEmbeddingProviderRequest(provider, inputs, authorization); err != nil {
		return EmbeddingResult{}, err
	}
	result, err := provider.Embed(ctx, cloneEmbeddingInputs(inputs), authorization)
	if err != nil {
		return EmbeddingResult{}, err
	}
	if err := ValidateEmbeddingProviderResult(provider.Descriptor(), inputs, authorization, result); err != nil {
		return EmbeddingResult{}, err
	}
	return result, nil
}
...
func validateEmbeddingDescriptorFields(descriptor EmbeddingDescriptor) error {
	...
	if !validVectorMetric(descriptor.Metric) {
...
func validateEmbeddingInputs(descriptor EmbeddingDescriptor, inputs []EmbeddingInput, authorization EmbeddingAuthorization) error {
...
func validVectorMetric(metric string) bool {

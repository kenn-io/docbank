...
func PolicyFingerprint(profile Profile) (string, error) {
	...
	encoded, err := json.Marshal(policyIdentity{
		...
		DisclosurePolicy: normalized.DisclosureFingerprint, Semantics: fixedSemantics(),
		RetentionCeiling: int64(profileRetention(normalized.Transport)), RequestTimeout: int64(normalized.RequestTimeout),
		PollInterval: int64(normalized.PollInterval), MaxPollAttempts: normalized.MaxPollAttempts,
		CleanupTimeout: int64(normalized.CleanupTimeout), MaxInputBytes: normalized.MaxInputBytes,
		MaxRequestBytes: normalized.MaxRequestBytes, MaxResponseBytes: normalized.MaxResponseBytes,
		Egress: egressPolicyIdentity(normalized.EgressPolicy),
...
func normalizeProfile(profile Profile) (Profile, document.EmbeddingDescriptor, error) {
	...
	if !validToken(profile.CompatibilityEpoch) || profile.Descriptor.ModelRevision != profile.CompatibilityEpoch {
		return Profile{}, document.EmbeddingDescriptor{}, errors.New("gemini embed: compatibility epoch does not match descriptor")
	}
...
func minimumFilesAPIRequestCapacity(dimension int) (int64, error) {
	...
	fileData, err := json.Marshal(wireRequest{Model: "models/" + Model,
		Content: wireContent{Parts: []wirePart{{FileData: &wireFileData{
...
func maximumProviderFileURI() string { return origin + "/v1beta/files/" + strings.Repeat("a", 40) }
...
func validateDescriptor(descriptor document.EmbeddingDescriptor, epoch string) error {
	...
	if descriptor.ID != ProviderID || descriptor.ContractVersion != document.EmbeddingProviderContractVersion ||
		...
		descriptor.ModelRevision != epoch || descriptor.Dimension < 128 || descriptor.Dimension > 3072 ||

...
func TestAuthorizeCarriesExactLocalCapabilityValue(t *testing.T) {
	...
	require.True(t, ok)
	carried := carrier.CapabilityRecord()
	assert.Equal(t, record, carried)
	require.NoError(t, media.ValidateCapabilityRecord(carried))
	_, local := carried.InspectionPolicy()
	assert.True(t, local)

	carried.Eligible = false
	assert.Equal(t, record, carrier.CapabilityRecord(), "the carrier must return a value copy")
}

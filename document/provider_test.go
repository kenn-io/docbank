...
type countingRenditionProvider struct {
	descriptor  RenditionDescriptor
	renderCalls int
}

func (provider *countingRenditionProvider) Descriptor() RenditionDescriptor {
	return provider.descriptor
}

func (provider *countingRenditionProvider) Render(
	context.Context, AuthorizedUpload, RenditionAuthorization,
) (RenditionResult, error) {
	provider.renderCalls++
	return RenditionResult{}, errors.New("unexpected render call")
}
...
func TestRenderRenditionClassifiesExpiredRequestBeforeProviderCall(t *testing.T) {
	descriptor := validRenditionDescriptor(t)
	metadata := validAuthorizedUploadMetadata()
	upload := &syntheticAuthorizedUpload{
		ReadCloser: io.NopCloser(bytes.NewReader([]byte("synthetic exact source"))), metadata: metadata,
	}
	provider := &countingRenditionProvider{descriptor: descriptor}
	authorization := validRenditionAuthorization(descriptor, metadata)
	authorization.ExpiresAt = time.Now().UTC().Add(-time.Second).Format(renditionTimestampForm)

	_, err := RenderRendition(t.Context(), provider, upload, authorization)

	providerError, ok := errors.AsType[*RenditionProviderError](err)
	require.True(t, ok, "%T: %v", err, err)
	assert.Equal(t, RenditionErrorPolicyRejected, providerError.Code())
	assert.Zero(t, provider.renderCalls)
}

func TestRenderRenditionWithResumeClassifiesCanceledRequestBeforeProviderCall(t *testing.T) {
	descriptor := validRenditionDescriptor(t)
	metadata := validAuthorizedUploadMetadata()
	upload := &syntheticAuthorizedUpload{
		ReadCloser: io.NopCloser(bytes.NewReader([]byte("synthetic exact source"))), metadata: metadata,
	...
	_, err := RenderRenditionWithResume(ctx, provider, upload, authorization, nil, nil)

	providerError, ok := errors.AsType[*RenditionProviderError](err)
	require.True(t, ok, "%T: %v", err, err)
	assert.Equal(t, RenditionErrorCanceled, providerError.Code())
	assert.Zero(t, provider.renderCalls)
}
...
func TestRenderRenditionWithResumePersistsProviderIssuedHandleAndResumesIt(t *testing.T) {
	...
	require.NoError(t, err)
	provider.result.Receipt.CompletedAt = expiresAt.Add(time.Second).Format(renditionTimestampForm)
	resume := persisted[0]
	upload = &syntheticAuthorizedUpload{
		ReadCloser: io.NopCloser(strings.NewReader("synthetic exact source")), metadata: metadata,
	}

...
func TestBuildEmbeddingInputsChargesByteFittingProbes(t *testing.T) {
	...
	policy.Tokenizer = truncationAdversarialTokenizer{}
	policy.MaxProviderBytes = 11
	policy.MaxFittingWorkBytes = 13
	policy.MaxFittingWorkTokens = 13
	policy.TruncationPolicy = TruncationPolicyTruncateIndivisible
...
func TestEmbeddingInputGenerationRoundTripsAndRejectsMalformedJSON(t *testing.T) {
	...
	for _, testCase := range []struct{ name, value, want string }{
		{"unknown generation field", strings.Replace(string(encoded), `{"version":1`, `{"unknown":1,"version":1`, 1), "unknown field"},
...
func TestEmbeddingInputGenerationDecodePreflightsCallerBounds(t *testing.T) {
	generation, err := BuildEmbeddingInputs(testChunkEvidence(t, []sourceChunkUnit{{text: "AABB"}}), testInputPolicy(t, 1, 0))
	require.NoError(t, err)
	encoded, err := json.Marshal(generation)
	require.NoError(t, err)
	require.Greater(t, len(generation.Inputs), 1)

	caseInsensitiveInputs := []byte(strings.Replace(string(encoded), `"inputs":`, `"Inputs":`, 1))
	bounds := testGenerationDecodeBounds()
	bounds.MaxInputs = len(generation.Inputs) - 1
	_, err = DecodeEmbeddingInputGeneration(caseInsensitiveInputs, bounds)
	require.ErrorContains(t, err, "canonical lowercase spelling")

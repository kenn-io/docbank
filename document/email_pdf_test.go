...
func TestEmailPDFProfilePinsDecodedGenerationAndPaper(t *testing.T) {
	...
	require.Error(t, ValidateEmailPDFBinding(b), "the recipe requires a pinned bubblewrap executable")
	b.Recipe.BubblewrapSHA256 = strings.Repeat("b", 64)
	...
	p.Rendition.EmailPDF.Recipe.BubblewrapSHA256 = strings.Repeat("e", 64)
	_, bubblewrapChanged, err := CanonicalProfile(p)
	require.NoError(t, err)
	require.NotEqual(t, workerChanged.RenditionRequest, bubblewrapChanged.RenditionRequest)

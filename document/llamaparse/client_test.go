...
func TestClientClassifiesExactUploadReadLifecycle(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: document.RenditionErrorCanceled,
		},
		{
		...
		},
		{
			...
			want: document.RenditionErrorTransient,
		},
	}
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
...
func TestClientResumesHistoricalSealedAuthorizationWithRecordedReceiptTimes(t *testing.T) {
	...
	require.NoError(t, err)
	renditionPolicy, err := document.NewRenditionPolicy(normalizePolicy, 1_000)
	require.NoError(t, err)
...
func newFixture(t *testing.T, source []byte) *fixture {
	...
	return &fixture{
		...
		authorization: document.RenditionAuthorization{
			...
			InputKind: metadata.InputKind, MaxProviderMarkdownBytes: 16 << 10,

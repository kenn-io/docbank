...
func New(profile Profile, secrets SecretResolver, transport http.RoundTripper) (*Client, error) {
	...
	return &Client{profile: normalized, descriptor: cloneDescriptor(descriptor), secrets: secrets,
		http: &http.Client{Transport: transport, CheckRedirect: providerhttp.RefuseRedirects}}, nil
}
...
func (client *Client) Render(ctx context.Context, upload document.AuthorizedUpload, authorization document.RenditionAuthorization) (document.RenditionResult, error) {
	...
	source, err := readExact(operationCtx, upload, metadata)
	if err != nil {
		return document.RenditionResult{}, err
	}
	defer clear(source)
	...
	if err != nil {
		return document.RenditionResult{}, fmt.Errorf("marker: fingerprint authorization: %w", err)
	}
	return document.RenditionResult{Evidence: result.evidence, ProviderMarkdown: result.markdown,
		Receipt: document.RenditionReceipt{ProviderID: client.descriptor.ID,
			DescriptorFingerprint: client.descriptor.Fingerprint, PolicyFingerprint: authorization.PolicyFingerprint,
			...
			OperationID: "marker-" + authorization.RenditionRequestFingerprint[:24],
...
func (client *Client) parseResult(body []byte, family string, expectedNaturalUnits int) (parsedResult, []string, error) {
	...
	if !natural {
		evidence = degradedEvidence(family, *wire.Output)
	}
	warnings := []string(nil)
...
func degradedEvidence(family, markdown string) document.SourceEvidenceV1 {
	return document.SourceEvidenceV1{ContractVersion: document.SourceEvidenceContractV1,
		Completeness: document.EvidenceDegradedProvenance, Family: family, UnitKind: document.EvidenceUnitGeneric,
		Omissions: []document.SourceEvidenceOmissionV1{{Kind: document.EvidenceOmissionField,
			Field: "natural_provenance", Reason: "Marker returned no source-native unit mapping"}},
...
func readExact(ctx context.Context, upload io.Reader, metadata document.AuthorizedUploadMetadata) ([]byte, error) {
	if err := ctx.Err(); err != nil {
		return nil, providerError(document.RenditionErrorCanceled, "Marker rendering canceled", err)
	}
	value, err := io.ReadAll(io.LimitReader(upload, metadata.ByteLength+1))
	if err != nil {
		return nil, providerError(document.RenditionErrorTransient, "could not read authorized Marker upload", err)
	}
	digest := sha256.Sum256(value)
	if int64(len(value)) != metadata.ByteLength || hex.EncodeToString(digest[:]) != metadata.SHA256 {
		return nil, providerError(document.RenditionErrorPolicyRejected, "authorized Marker upload identity mismatch", nil)
	}
	return value, nil
}
...
func providerError(code document.RenditionErrorCode, message string, cause error) error {
	value, err := document.NewRenditionProviderError(code, message, 0, cause)
	if err == nil {
		return value
	}
	return fmt.Errorf("marker error classification: %w", err)
}

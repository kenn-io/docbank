...
func RenderRendition(
	...
	descriptor, err := validateCurrentRenditionRequest(ctx, provider, upload, sealed)
	if err != nil {
...
func RenderRenditionWithResume(
	...
	if !supportsResume {
		...
		result, renderErr := provider.Render(ctx, upload, cloneRenditionAuthorization(sealed))
		if renderErr != nil {
			if classified := ValidateRenditionProviderError(renderErr); classified != nil {
				return RenditionResult{}, classified
			}
			return RenditionResult{}, renderErr
		}
		if err := ValidateRenditionResult(descriptor, sealed, result); err != nil {
			return RenditionResult{}, err
		}
	...
	recordCheckpointError := func(err error) error {
		if err == nil {
			return nil
		}
		checkpointMu.Lock()
	...
	checkedCheckpoint := func(handle RenditionResumeHandle) error {
		if err := validateRenditionResumeHandle(handle); err != nil {
			return recordCheckpointError(err)
		}
		return recordCheckpointError(checkpoint(handle))
	}
	...
	result, err := resumable.RenderResumable(
		ctx, providerUpload, cloneRenditionAuthorization(sealed), resumeCopy, checkedCheckpoint,
	)
	checkpointMu.Lock()
	durableErr := checkpointErr
	checkpointMu.Unlock()
	if durableErr != nil {
	...
	if resumeCopy != nil {
		validationErr = validateResumedRenditionResult(descriptor, sealed, result)
	} else {
		validationErr = ValidateRenditionResult(descriptor, sealed, result)
	}
	if validationErr != nil {
		return RenditionResult{}, validationErr
	}
	return result, nil
}

func validateCurrentRenditionRequest(
	ctx context.Context, provider RenditionProvider, upload AuthorizedUpload,
	authorization RenditionAuthorization,
) (RenditionDescriptor, error) {
	if ctx != nil && ctx.Err() != nil {
		return RenditionDescriptor{}, classifiedRenditionRequestError(RenditionErrorCanceled, "rendition request canceled", ctx.Err())
	}
	descriptor, err := ValidateRenditionProviderRequest(provider, upload, authorization)
	if err == nil {
	...
	return RenditionDescriptor{}, classifiedRenditionRequestError(RenditionErrorPolicyRejected, "rendition request rejected", err)
}

func classifiedRenditionRequestError(code RenditionErrorCode, message string, cause error) error {
	providerError, err := NewRenditionProviderError(code, message, 0, cause)
	if err != nil {
...
func validateRenditionResult(
	...
	if err := validateRenditionReceipt(
		descriptor, authorization, result.Receipt, allowCompletionAfterExpiry); err != nil {
		return err
	}
...
func validateRenditionReceipt(
	...
	if startedAt.Before(authorizedAt) || !startedAt.Before(expiresAt) ||
		!allowCompletionAfterExpiry && completedAt.After(expiresAt) {
